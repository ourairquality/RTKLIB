...
MainWindow::MainWindow(QWidget *parent) : QDialog(parent) {
  ...
  for (int i = 0; i < 3; i++) CmdEna[i][0] = CmdEna[i][1] = CmdEna[i][2] = 0;

  TimeSys = SolType = 0;
  for (int i = 0; i < 4; i++) {
    PlotType[i] = FreqType[i] = BLMode[i] = TrkType[i] = 0;
    TrkScale[i] = 5;
  };
  ...
  for (int i = 0; i < 3; i++) TrkOri[i] = 0.0;
...
void MainWindow::showEvent(QShowEvent *event) {
  ...
  connect(systemTray, SIGNAL(activated(QSystemTrayIcon::ActivationReason)), this,
          SLOT(TrayIconClick(QSystemTrayIcon::ActivationReason)));
  ...
  if (parser.isSet(iniFileOption)) IniFile = parser.value(iniFileOption);
...
void MainWindow::BtnPlotClick() {
  ...
  opts << QString(" -p tcpcli://localhost:%1 -t \"%2 %3\"")
              .arg(OpenPort)
              .arg(windowTitle())
              .arg(": RTKPLOT QT");
...
void MainWindow::BtnOutputStrClick() {
  ...
  for (i = 3; i < 5; i++) {
    ...
    str = otype[Stream[i]];
    if (str == STR_SERIAL)
      strncpy(path, qPrintable(Paths[i][0]), 1023);
    else if (str == STR_FILE)
      strncpy(path, qPrintable(Paths[i][2]), 1023);
    else if (str == STR_FTP || str == STR_HTTP)
      strncpy(path, qPrintable(Paths[i][3]), 1023);
    else
      strncpy(path, qPrintable(Paths[i][1]), 1023);
...
void MainWindow::SvrStart(void) {
  ...
  if (RovAntPcvF || RefAntPcvF) free(pcvr.pcv);
  ...
  for (i = 0; i < 8; i++) {
    ...
    if (strs[i] == STR_NONE)
      ...
      strcpy(paths[i], qPrintable(Paths[i][0]));
    else if (strs[i] == STR_FILE)
      strcpy(paths[i], qPrintable(Paths[i][2]));
    else if (strs[i] == STR_FTP || strs[i] == STR_HTTP)
      strcpy(paths[i], qPrintable(Paths[i][3]));
    else
      strcpy(paths[i], qPrintable(Paths[i][1]));
  }
  ...
  for (i = 0; i < 3; i++) {
    ...
    } else if (strs[i] == STR_TCPCLI || strs[i] == STR_TCPSVR || strs[i] == STR_NTRIPCLI) {
      cmds[i] = new char[1024];
      cmds[i][0] = '\0';
      cmds_periodic[i] = new char[1024];
      cmds_periodic[i][0] = '\0';
  ...
  if (!rtksvrstart(&rtksvr, SvrCycle, SvrBuffSize, strs, (const char **)paths, Format, NavSelect,
                   (const char **)cmds, (const char **)cmds_periodic, (const char **)rcvopts,
                   NmeaCycle, NmeaReq, nmeapos, &PrcOpt, solopt, &monistr, errmsg,
                   sizeof(errmsg))) {
...
void MainWindow::SvrStop(void) {
  ...
  n = PSolE - PSolS;
  if (n < 0) n += SolBuffSize;
  m = PSol - PSolS;
  if (m < 0) m += SolBuffSize;
  if (n > 0) {
    ScbSol->setMaximum(n - 1);
    ScbSol->setValue(m);
  }
  Message->setText("");
...
void MainWindow::TimerTimer() {
  ...
  if (SolCurrentStat) {
    ...
    Svr->setStyleSheet(rtksvr.state ? "QLabel {background-color: green; }"
                                    : "QLabel {background-color: gray; }");
  }
...
void MainWindow::UpdateSolType(void) {
  QString label[] = {tr("Lat/Lon/Height"),
                     tr("Lat/Lon/Height"),
                     tr("X/Y/Z-ECEF"),
                     tr("E/N/U-Baseline"),
...
// update log ---------------------------------------------------------------
void MainWindow::UpdateLog(int stat, gtime_t time, double *rr, float *qr, double *rb, int ns,
                           double age, double ratio) {
  int i;
...
void MainWindow::UpdateFont(void) {
  QLabel *label[] = {PlabelA, Plabel1, Plabel2,  Plabel3,  Pos1,
                     Pos2,    Pos3,    Solution, LabelStd, LabelNSat};
  ...
  tmp.setPointSize(8);
  label[8]->setFont(tmp);
  label[8]->setStyleSheet("QLabel {color: gray;}");
  ;
  label[9]->setFont(tmp);
  label[9]->setStyleSheet("QLabel {color: gray;}");
}
// update time --------------------------------------------------------------
void MainWindow::UpdateTime(void) {
  ...
  if (TimeSys == 0) {
    ...
    time = gpst2utc(time);
    if (!(t = localtime(&time.time)))
      str = "2000/01/01 00:00:00.0";
    else
      str = QString("%1/%2/%3 %4:%5:%6.%7")
                ...
                .arg(static_cast<int>(time.sec * 10));
  } else if (TimeSys == 3) {
...
void MainWindow::UpdatePos(void) {
  ...
  QString sol[] = {tr("----"), tr("FIX"),    tr("FLOAT"), tr("SBAS"),
                   tr("DGPS"), tr("SINGLE"), tr("PPP")};
  QString s[9], ext = "";
  QString color[] = {"silver", "green", "rgb(0,170,255)", "rgb(255,0,255)",
                     "blue",   "red",   "rgb(128,0,128)"};
  double *rr = SolRov + PSol * 3, *rb = SolRef + PSol * 3, *qr = Qr + PSol * 9, pos[3] = {0},
         Qe[9] = {0};
  ...
  IndSol->setStyleSheet(QString("QLabel {color: %1; background-color: %1}")
                            .arg(rtksvr.state && stat ? color[stat] : "white"));
  ...
  if (SolType == 0) {
    ...
    s[3] = QString("%1%2 %3' %4\"")
               .arg(fabs(dms1[0]), 0, 'f', 0)
               .arg(degreeChar)
               .arg(dms1[1], 2, 'f', 0, '0')
               .arg(dms1[2], 7, 'f', 4, '0');
    s[4] = QString("%1%2 %3' %4\"")
               .arg(fabs(dms2[0]), 0, 'f', 0)
               .arg(degreeChar)
               .arg(dms2[1], 2, 'f', 0, '0')
               .arg(dms2[2], 7, 'f', 4, '0');
    s[5] = QString("%1 m").arg(pos[2], 0, 'f', 3);
    s[6] = QString(tr("N:%1 E:%2 U:%3 m"))
               .arg(SQRT(Qe[4]), 6, 'f', 3)
               .arg(SQRT(Qe[0]), 6, 'f', 3)
               .arg(SQRT(Qe[8]), 6, 'f', 3);
  } else if (SolType == 1) {
  ...
  s[7] = QString(tr("Age:%1 s Ratio:%2 #Sat:%3"))
             .arg(Age[PSol], 4, 'f', 1)
             .arg(Ratio[PSol], 4, 'f', 1)
             .arg(Nvsat[PSol], 2);
  if (Ratio[PSol] > 0.0) s[8] = QString(" R:%1").arg(Ratio[PSol], 4, 'f', 1);

  for (i = 0; i < 8; i++) label[i]->setText(s[i]);
  for (i = 3; i < 6; i++)
    label[i]->setStyleSheet(
        QString("QLabel {color: %1;}")
            .arg(PrcOpt.mode == PMODE_MOVEB && SolType <= 2 ? "grey" : "black"));
  IndQ->setStyleSheet(IndSol->styleSheet());
  SolS->setText(Solution->text());
  SolS->setStyleSheet(Solution->styleSheet());
  SolQ->setText(ext + " " + label[0]->text() + " " + label[3]->text() + " " + label[1]->text() +
                " " + label[4]->text() + " " + label[2]->text() + " " + label[5]->text() + s[8]);
}
...
void MainWindow::UpdateStr(void) {
  ...
  for (i = 0; i < MAXSTRRTK; i++) {
    ...
    if (sstat[i]) Message->setText(msg);
  }
}
// draw solution plot -------------------------------------------------------
  ...
  for (i = 0; i < 2; i++) {
    if (ns[i] > 0) {
      ...
      for (int j = 0; j < ns[i]; j++) {
        ...
        Az[i][j] = az[i][j];
        El[i][j] = el[i][j];
        for (int k = 0; k < NFREQ; k++) Snr[i][j][k] = snr[i][j][k];
        Vsat[i][j] = vsat[i][j];
      }
    } else {
      for (j = 0; j < Nsat[i]; j++) {
...
QColor MainWindow::SnrColor(int snr) {
  QColor color[] = {
      Qt::green, QColor(255, 128, 0, 255), QColor(0, 255, 128, 255), Qt::blue, Qt::red, Qt::gray};
  uint32_t r1, g1, b1;
  QColor c1, c2;
  double a;
  int i;
...
// draw snr plot ------------------------------------------------------------
void MainWindow::DrawSnr(QPainter *c, int w, int h, int x0, int y0, int index, int freq) {
  static const QColor color[] = {QColor(0, 128, 0),    QColor(0, 128, 128), QColor(0xA0, 0, 0xA0),
                                 QColor(128, 0, 0),    QColor(0, 0, 128),   QColor(128, 128, 0),
                                 QColor(128, 128, 128)};
  static const QColor color_sys[] = {
      Qt::green, QColor(0, 0xAA, 0xFF), QColor(255, 0, 255), Qt::blue, Qt::red, Qt::darkCyan,
      Qt::gray};
  QString s;
...
void MainWindow::DrawSat(QPainter *c, int w, int h, int x0, int y0, int index, int freq) {
  ...
  for (i = 0, k = Nsat[index] - 1; i < Nsat[index] && i < MAXSAT; i++, k--) {
    ...
    if (Vsat[index][k] && (snr[freq] > 0 || freq > NFREQ)) {
      azel[ns * 2] = Az[index][k];
      azel[1 + ns * 2] = El[index][k];
      ns++;
    }
  ...
  DrawText(c, x0 + 3, y0 + h, QString(tr("# Sat: %1/%2")).arg(ns, 2).arg(Nsat[index], 2), Qt::gray,
           1, 1);
  DrawText(c, x0 + w - 3, y0 + h, QString(tr("GDOP: %1")).arg(dop[0], 0, 'f', 1), Qt::gray, 2, 1);
}
// draw baseline plot -------------------------------------------------------
void MainWindow::DrawBL(QPainter *c, int id, int w, int h) {
  QColor color[] = {QColor(0xc0, 0xc0, 0xc0), Qt::green, QColor(0x00, 0xAA, 0xFF),
                    QColor(0xff, 0x00, 0xff), Qt::blue,  Qt::red,
  ...
  double cp, q, az = 0.0;
  QColor col = Qt::white;
  int i, d1 = 10, d2 = 16, d3 = 10, cy = 0, sy = 0, cya = 0, sya = 0, a, x1, x2, y1, y2, r1, digit,
         mode;
  ...
  if (pitch < 0.0) {
    ...
    DrawArrow(c, p.x() + sya, p.y() - cya, d3, static_cast<int>((yaw - az) * R2D),
  ...
  for (a = 0; a < 360; a += 5) {
    ...
    c->setBrush(Qt::white);
    if (a % 90 == 0) DrawText(c, p.x() + x1, p.y() - y1, label[a / 90], Qt::gray, 0, 0);
    if (a == 0) {
      x1 = static_cast<int>((r - d1 * 3 / 2) * sin(a * D2R - az));
      y1 = static_cast<int>((r - d1 * 3 / 2) * cos(a * D2R - az));
      DrawArrow(c, p.x() + x1, p.y() - y1, d3, -static_cast<int>(az * R2D),
                QColor(0xc0, 0xc0, 0xc0));
    }
  }
  pp = pitch >= 0.0 ? p2 : p1;
  ...
  DrawText(c, w - 3, h, QString("P: %1%2").arg(pitch * R2D, 0, 'f', 1).arg(degreeChar), Qt::gray, 2,
           1);
}
// draw track plot ----------------------------------------------------------
void MainWindow::DrawTrk(QPainter *c, int id, QPixmap &buff) {
  QColor mcolor[] = {QColor(0xc0, 0xc0, 0xc0), Qt::green, QColor(0x00, 0xAA, 0xFF),
                     QColor(0xff, 0x00, 0xff), Qt::blue,  Qt::red,
                     QColor(0x80, 0x80, 0x00)};
  ...
  QPoint p1, p2;
  QString label;
  double scale[] = {0.00021, 0.00047, 0.001, 0.0021, 0.0047, 0.01,   0.021,  0.047,
                    0.1,     0.21,    0.47,  1.0,    2.1,    4.7,    10.0,   21.0,
                    47.0,    100.0,   210.0, 470.0,  1000.0, 2100.0, 4700.0, 10000.0};
...
// draw text ----------------------------------------------------------------
void MainWindow::DrawText(QPainter *c, int x, int y, const QString &s, const QColor &color, int ha,
                          int va) {
...
// draw arrow ---------------------------------------------------------------
void MainWindow::DrawArrow(QPainter *c, int x, int y, int siz, int ang, const QColor &color) {
  QPoint p1[4], p2[4];
  int i;
  ...
  for (i = 0; i < 4; i++) {
    p2[i].setX(x +
               static_cast<int>(p1[i].x() * cos(-ang * D2R) - p1[i].y() * sin(-ang * D2R) + 0.5));
    p2[i].setY(y -
               static_cast<int>(p1[i].x() * sin(-ang * D2R) + p1[i].y() * cos(-ang * D2R) + 0.5));
  }
...
void MainWindow::OpenMoniPort(int port) {
  ...
  QMessageBox::critical(
      this, tr("Error"),
      QString(tr("monitor port %1-%2 open error")).arg(port).arg(port + MAXPORTOFF));
  OpenPort = 0;
}
...
void MainWindow::SaveLog(void) {
  ...
  fileTemplate = QString("rtk_%1%2%3%4%5%6.txt")
                     .arg(ep[0], 4, 'f', 0, QChar('0'))
                     .arg(ep[1], 2, 'f', 0, QChar('0'))
                     .arg(ep[2], 2, 'f', 0, QChar('0'))
                     .arg(ep[3], 2, 'f', 0, QChar('0'))
                     .arg(ep[4], 2, 'f', 0, QChar('0'))
                     .arg(ep[5], 2, 'f', 0, QChar('0'));
...
void MainWindow::LoadNav(nav_t *nav) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    for (j = 0; j < 2; j++) {
      if ((str = settings.value(QString("navi/eph_%1_%2").arg(i, 2).arg(j)).toString()).isEmpty())
        continue;
      nav->eph[i][j] = eph0;
      ...
      *p = '\0';
      ...
      sscanf(p + 1,
             "%d,%d,%d,%d,%ld,%ld,%ld,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf,%"
             "lf,%lf,%lf,%lf,%lf,%lf,%d,%d,%lf",
             &nav->eph[i][j].iode, &nav->eph[i][j].iodc, &nav->eph[i][j].sva, &nav->eph[i][j].svh,
             &toe_time, &toc_time, &ttr_time, &nav->eph[i][j].A, &nav->eph[i][j].e,
             &nav->eph[i][j].i0, &nav->eph[i][j].OMG0, &nav->eph[i][j].omg, &nav->eph[i][j].M0,
             &nav->eph[i][j].deln, &nav->eph[i][j].OMGd, &nav->eph[i][j].idot, &nav->eph[i][j].crc,
             &nav->eph[i][j].crs, &nav->eph[i][j].cuc, &nav->eph[i][j].cus, &nav->eph[i][j].cic,
             &nav->eph[i][j].cis, &nav->eph[i][j].toes, &nav->eph[i][j].fit, &nav->eph[i][j].f0,
             &nav->eph[i][j].f1, &nav->eph[i][j].f2, &nav->eph[i][j].tgd[0], &nav->eph[i][j].code,
             &nav->eph[i][j].flag, &nav->eph[i][j].tgd[1]);
...
void MainWindow::LoadOpt(void) {
  ...
  TextViewer::Color1 = QColor(
      static_cast<QRgb>(settings.value("viewer/color1", static_cast<int>(Qt::black)).toInt()));
  TextViewer::Color2 = QColor(
      static_cast<QRgb>(settings.value("viewer/color2", static_cast<int>(Qt::white)).toInt()));
  ...
  resize(settings.value("window/width", 388).toInt(), settings.value("window/height", 284).toInt());
}
...
void MainWindow::SaveOpt(void) {
  ...
  for (i = 0; i < 3; i++) settings.setValue(QString("stream/rcvopt%1").arg(i + 1), RcvOpt[i]);
  for (i = 0; i < 3; i++)
    for (j = 0; j < 3; j++) {
      Cmds[i][j].replace("\r\n", "@@");
  ...
  for (i = 0; i < 10; i++) settings.setValue(QString("tcpopt/history%1").arg(i), History[i]);
  settings.setValue("mapopt/nmappnt", NMapPnt);
  for (i = 0; i < NMapPnt; i++) {
    settings.setValue(QString("mapopt/pntname%1").arg(i + 1), PntName[i]);
    settings.setValue(QString("mapopt/pntpos%1").arg(i + 1), QString("%1,%2,%3")
                                                                 .arg(PntPos[i][0], 0, 'f', 4)
                                                                 .arg(PntPos[i][1], 0, 'f', 4)
                                                                 .arg(PntPos[i][2], 0, 'f', 4));
  }
