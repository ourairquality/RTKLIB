...
class MainWindow : public QDialog, private Ui::MainForm {
 ...
 private:
  ...
  void UpdateLog(int stat, gtime_t time, double *rr, float *qr, double *rb, int ns, double age,
                 double ratio);
  void SvrStart(void);
  void SvrStop(void);
  ...
  void DrawArrow(QPainter *c, int x, int y, int siz, int ang, const QColor &color);
