...
void MonitorDialog::setRtk()
{
    ...
    ui->tWConsole->setRowCount(53 + NFREQ + ANTNFREQ * 2);
...
void MonitorDialog::showRtk()
{
    ...
    *rtk = rtksvr->rtk;
    ...
    if (ui->tWConsole->rowCount() < 53 + NFREQ + ANTNFREQ * 2) {
      free(rtk);
      return;
    }
    ...
    for (j = 0; j < ANTNFREQ; j++) {
        ...
        ui->tWConsole->item(row++, 1)->setText(QStringLiteral("%1 m, %2 m, %3 m").arg(pco[0], 0, 'f', 4).arg(pco[1], 0, 'f', 4).arg(pco[2], 0, 'f', 4));
    }
