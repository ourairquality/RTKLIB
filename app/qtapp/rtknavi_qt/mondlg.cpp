...
void MonitorDialog::setRtk()
{
    ...
    ui->tWConsole->setRowCount(51 + NFREQ * 3);
...
void MonitorDialog::showRtk()
{
    ...
    *rtk = rtksvr->rtk;
    ...
    if (ui->tWConsole->rowCount() < 51 + NFREQ * 3)  {
      free(rtk);
      return;
    }
    ...
    ui->tWConsole->item(row++, 1)->setText(freq[rtk->opt.nf > 6 ? 0 : rtk->opt.nf]);
    ...
    for (j = 0; j < NFREQ; j++) {
        ...
        ui->tWConsole->item(row,   0)->setText(tr("Antenna Phase Center F%1 E/N/U Rover").arg(j+1));
    ...
    for (j = 0; j < NFREQ; j++) {
        ...
        ui->tWConsole->item(row,   0)->setText(tr("Antenna Phase Center F%1 E/N/U Base Station").arg(j+1));
        ui->tWConsole->item(row++, 1)->setText(QStringLiteral("%1 m, %2 m, %3 m").arg(off[0], 0, 'f', 3).arg(off[1], 0, 'f', 3).arg(off[2], 0, 'f', 3));
    }
