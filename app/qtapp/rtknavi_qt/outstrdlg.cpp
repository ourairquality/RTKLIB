    ...
    : QDialog(parent), ui(new Ui::OutputStrDialog)
{
    ...
    connect(ui->btnStream3, &QPushButton::clicked, this, &OutputStrDialog::showStream3Options);
...
{
    if ((index < 0) || (index >= RTKSVRNSOL)) return;
...
void OutputStrDialog::setStreamEnabled(int stream, int enabled)
{
    ...
    if ((stream < 0 ) || (stream >= RTKSVRNSOL)) return;
...
{
    QCheckBox *cBStreamC[] = {ui->cBStream1C, ui->cBStream2C, ui->cBStream3C};
    if ((stream < 0 ) || (stream >= RTKSVRNSOL)) return false;
...
{
    QComboBox *cBStream[] = {ui->cBStream1, ui->cBStream2, ui->cBStream3};
    if ((stream < 0 ) || (stream >= RTKSVRNSOL)) return STR_NONE;
