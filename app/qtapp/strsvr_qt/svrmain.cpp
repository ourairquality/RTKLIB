...
void MainForm::FormCreate() {
  ...
  if (parser.isSet(windowTitleOption)) setWindowTitle(parser.value(windowTitleOption));
...
void MainForm::BtnStartClick() { SvrStart(); }
// callback on button-stop --------------------------------------------------
void MainForm::BtnStopClick() { SvrStop(); }
...
void MainForm::BtnCmdClick() {
  ...
  if (i == 0)
    cmdOptDialog->setWindowTitle("Input Serial/TCP Commands");
  else
    cmdOptDialog->setWindowTitle(QString("Output%1 Serial/TCP Commands").arg(i));
...
void MainForm::MenuStartClick() { SvrStart(); }
// callback on menu-stop ----------------------------------------------------
void MainForm::MenuStopClick() { SvrStop(); }
// callback on menu-exit ----------------------------------------------------
void MainForm::MenuExitClick() { close(); }
// callback on stream-monitor -----------------------------------------------
void MainForm::BtnStrMonClick() { strMonDialog->show(); }
// callback on input type change --------------------------------------------
void MainForm::InputChange() { UpdateEnable(); }
// callback on output type change ------------------------------------------
void MainForm::OutputChange() { UpdateEnable(); }
// callback on interval timer -----------------------------------------------
void MainForm::Timer1Timer() {
  QColor color[] = {
      Qt::red, Qt::white, CLORANGE, Qt::green, QColor(0x00, 0xff, 0x00), QColor(0xff, 0xff, 0x00)};
  ...
  QLabel *e1[] = {InputByte,   Output1Byte, Output2Byte, Output3Byte,
                  Output4Byte, Output5Byte, Output6Byte};
  ...
  ctime -= t[2] * 60.0;
  t[3] = ctime;
  ConTime->setText(QString("%1d %2:%3:%4")
                       .arg(t[0], 0, 'f', 0)
                       .arg(t[1], 2, 'f', 0, QChar('0'))
                       .arg(t[2], 2, 'f', 0, QChar('0'))
                       .arg(t[3], 2, 'f', 2, QChar('0')));
...
void MainForm::SvrStart(void) {
  ...
  int itype[] = {STR_SERIAL, STR_TCPCLI, STR_TCPSVR, STR_NTRIPCLI,
                 STR_UDPSVR, STR_FILE,   STR_FTP,    STR_HTTP};
  int otype[] = {STR_NONE,     STR_SERIAL,   STR_TCPCLI, STR_TCPSVR,
                 STR_NTRIPSVR, STR_NTRIPCAS, STR_UDPCLI, STR_FILE};
  ...
  for (int i = 1; i < MAXSTR; i++) {
    ...
    strcpy(paths[1],
           !type[i]->currentIndex() ? "" : qPrintable(Paths[i][type[i]->currentIndex() - 1]));
  ...
  for (int i = 0; i < MAXSTR; i++) {
    ...
    if (strs[i] == STR_SERIAL) {
      ...
      if (CmdEna[i][0]) rtkstrcpy(cmds[i], 1024, qPrintable(Cmds[i][0]));
      if (CmdEna[i][2]) rtkstrcpy(cmds_periodic[i], 1024, qPrintable(Cmds[i][2]));
    } else if (strs[i] == STR_TCPCLI || strs[i] == STR_NTRIPCLI) {
      cmds[i] = new char[1024];
      cmds_periodic[i] = new char[1024];
      if (CmdEnaTcp[i][0]) rtkstrcpy(cmds[i], 1024, qPrintable(CmdsTcp[i][0]));
      if (CmdEnaTcp[i][2]) rtkstrcpy(cmds_periodic[i], 1024, qPrintable(CmdsTcp[i][2]));
    }
  }
  for (int i = 0; i < 5; i++) {
    opt[i] = SvrOpt[i];
  ...
  for (int i = 1; i < MAXSTR; i++) {  // for each out stream
    ...
    if (QMessageBox::question(this, tr("Overwrite"),
                              tr("File %1 exists. \nDo you want to overwrite?").arg(filepath)) !=
        QMessageBox::Yes)
      return;
  }
  ...
  for (int i = 0; i < MAXSTR - 1; i++) {  // for each out stream
    ...
    if (!(conv[i] = strconvnew(ConvInp[i], ConvOut[i], qPrintable(ConvMsg[i]), StaId, StaSel,
                               qPrintable(ConvOpt[i]))))
      continue;
  ...
  // stream server start
  if (!strsvrstart(&strsvr, opt, strs, (const char **)paths, (const char **)logs, conv,
                   (const char **)cmds, (const char **)cmds_periodic, AntPos)) {
    return;
  }
...
void MainForm::SvrStop(void) {
  char *cmds[MAXSTR];
  QComboBox *type[] = {Input, Output1, Output2, Output3, Output4, Output5, Output6};
  const int itype[] = {STR_SERIAL, STR_TCPCLI, STR_TCPSVR, STR_NTRIPCLI,
                       STR_UDPSVR, STR_FILE,   STR_FTP,    STR_HTTP};
  const int otype[] = {STR_NONE,     STR_SERIAL,   STR_TCPCLI, STR_TCPSVR,
                       STR_NTRIPSVR, STR_NTRIPCAS, STR_FILE};
  int strs[MAXSTR];
...
void MainForm::Timer2Timer() {
  const QString types[] = {tr("None"),       tr("Serial"), tr("File"),        tr("TCP Server"),
                           tr("TCP Client"), tr("UDP"),    tr("Ntrip Sever"), tr("Ntrip Client"),
                           tr("FTP"),        tr("HTTP"),   tr("Ntrip Cast"),  tr("UDP Server"),
                           tr("UDP Client")};
  const QString modes[] = {tr("-"), tr("R"), tr("W"), tr("R/W")};
  const QString states[] = {tr("ERR"), tr("-"), tr("WAIT"), tr("CONN")};
  unsigned char *msg = 0;
  int len, inb, inr, outb, outr;
...
#define MSG_SIZE 16000
    if (!(msg = (unsigned char *)malloc(MSG_SIZE))) return;

    msg[0] = '\0';
    for (int i = 0; i < MAXSTR; i++) {
      rtkcatprintf((char *)msg, MSG_SIZE, "[STREAM %d]\n", i);
      strsum(strsvr.stream + i, &inb, &inr, &outb, &outr);
      strstatx(strsvr.stream + i, (char *)msg, MSG_SIZE);
      if (inb > 0) {
        rtkcatprintf((char *)msg, MSG_SIZE, "  inb     = %d\n", inb);
        rtkcatprintf((char *)msg, MSG_SIZE, "  inr     = %d\n", inr);
      }
      if (outb > 0) {
        rtkcatprintf((char *)msg, MSG_SIZE, "  outb    = %d\n", outb);
        rtkcatprintf((char *)msg, MSG_SIZE, "  outr    = %d\n", outr);
      }
    }
...
void MainForm::UpdateEnable(void) {
  QComboBox *type[] = {Output1, Output2, Output3, Output4, Output5, Output6};
  QLabel *label1[] = {LabelOutput1, LabelOutput2, LabelOutput3,
                      LabelOutput4, LabelOutput5, LabelOutput6};
  ...
  for (int i = 0; i < MAXSTR - 1; i++) {
    label1[i]->setStyleSheet(
        QString("color :%1").arg(color2String(type[i]->currentIndex() > 0 ? Qt::black : Qt::gray)));
    label2[i]->setStyleSheet(
        QString("color :%1").arg(color2String(type[i]->currentIndex() > 0 ? Qt::black : Qt::gray)));
    label3[i]->setStyleSheet(
        QString("color :%1").arg(color2String(type[i]->currentIndex() > 0 ? Qt::black : Qt::gray)));
    btn1[i]->setEnabled(type[i]->currentIndex() > 0);
    btn2[i]->setEnabled(btn1[i]->isEnabled() &&
                        (type[i]->currentIndex() == 1 || type[i]->currentIndex() == 2));
    btn3[i]->setEnabled(btn1[i]->isEnabled() && Input->currentIndex() != 2 &&
                        Input->currentIndex() != 4);
    btn4[i]->setEnabled(btn1[i]->isEnabled() &&
                        (type[i]->currentIndex() == 1 || type[i]->currentIndex() == 2));
  }
}
// set task-tray icon -------------------------------------------------------
...
void MainForm::SaveOpt(void) {
  ...
  for (int i = 0; i < 6; i++) settings.setValue(QString("set/svropt_%1").arg(i), SvrOpt[i]);
  for (int i = 0; i < 3; i++) {
