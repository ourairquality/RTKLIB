...
//---------------------------------------------------------------------------
StaListDialog::StaListDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);
...
void StaListDialog::showEvent(QShowEvent *event) {
  ...
  for (int i = 0; i < mainForm->StaList.count(); i++) StaList->addItem(mainForm->StaList.at(i));
}
//---------------------------------------------------------------------------
void StaListDialog::BtnOkClick() {
  mainForm->StaList.clear();

  for (int i = 0; i < StaList->count(); i++) mainForm->StaList.append(StaList->item(i)->text());
}
//---------------------------------------------------------------------------
...
void StaListDialog::BtnSaveClick() {
  ...
  for (int i = 0; i < StaList->count(); i++) fp.write((StaList->item(i)->text() + "\n").toLatin1());
}
//---------------------------------------------------------------------------
