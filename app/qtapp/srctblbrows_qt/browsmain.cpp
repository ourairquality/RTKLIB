...
static char *getsrctbl(const QString addr) {
  ...
  if (!stropen(&str, STR_NTRIPCLI, STR_MODE_R, qPrintable(addr))) {
    ...
    QMetaObject::invokeMethod(mainForm, "ShowMsg", Qt::QueuedConnection,
                              Q_ARG(QString, QT_TR_NOOP("stream open error")));
    return NULL;
  }
  QMetaObject::invokeMethod(mainForm, "ShowMsg", Qt::QueuedConnection,
                            Q_ARG(QString, QT_TR_NOOP("connecting...")));
  ...
  while (p < buff + MAXSRCTBL - 1) {
    ...
    *p = '\0';
    ...
    if ((int)(tickget() - tick) > NTRIP_TIMEOUT) {
      QMetaObject::invokeMethod(mainForm, "ShowMsg", Qt::QueuedConnection,
                                Q_ARG(QString, QT_TR_NOOP("response timeout")));
      break;
    }
  }
  strclose(&str);
...
void MainForm::showEvent(QShowEvent *event) {
  ...
  parser.addPositionalArgument(
      "URL", QCoreApplication::translate("main", "file with list of Ntrip sources"));
...
void MainForm::closeEvent(QCloseEvent *) {
  ...
  for (int i = 0; i < Address->count(); i++) list = list + Address->itemText(i) + "@";
  ...
  for (int i = 0; i < 10; i++) setting.setValue(QString("sta/station%1").arg(i), StaList.join(","));
}
//---------------------------------------------------------------------------
void MainForm::MenuOpenClick() {
  QString OpenDialog_FileName = QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("Open"), QString(), tr("All File (*.*)")));
...
void MainForm::MenuSaveClick() {
  QString SaveDialog_FileName = QDir::toNativeSeparators(
      QFileDialog::getSaveFileName(this, tr("Save File"), QString(), tr("All File (*.*)")));
...
//---------------------------------------------------------------------------
void MainForm::MenuQuitClick() { close(); }
//---------------------------------------------------------------------------
void MainForm::MenuUpdateCasterClick() { GetCaster(); }
//---------------------------------------------------------------------------
void MainForm::MenuUpdateTableClick() { GetTable(); }
//---------------------------------------------------------------------------
...
void MainForm::Table0SelectCell(int ARow, int ACol) {
  ...
  if (0 <= ARow && ARow < Table0->rowCount()) {
    ...
    googleMapView->setWindowTitle(
        QString(tr("NTRIP Data Stream Map: %1/%2")).arg(Address->currentText()).arg(title));
  }
}
//---------------------------------------------------------------------------
void MainForm::BtnListClick() { GetCaster(); }
//---------------------------------------------------------------------------
void MainForm::BtnUpdateClick() { GetTable(); }
//---------------------------------------------------------------------------
void MainForm::AddressChange() { GetTable(); }
//---------------------------------------------------------------------------
...
void MainForm::ShowTable(void) {
  const QString ti[3][19] = {
      {tr("No"), tr("Mountpoint"), tr("ID"), tr("Format"), tr("Format-Details"), tr("Carrier"),
       tr("Nav-System"), tr("Network"), tr("Country"), tr("Latitude"), tr("Longitude"), tr("NMEA"),
       tr("Solution"), tr("Generator"), "Compr-Encrp", "Authentication", "Fee", "Bitrate", ""},
      {tr("No"), tr("Host"), tr("Port"), tr("ID"), tr("Operator"), tr("NMEA"), tr("Country"),
       tr("Latitude"), tr("Longitude"), tr("Fallback_Host"), tr("Fallback_Port"), "", "", "", "",
       "", "", ""},
      {tr("No"), tr("ID"), tr("Operator"), tr("Authentication"), tr("Fee"), tr("Web-Net"),
       tr("Web-Str"), tr("Web-Reg"), "", "", "", "", "", "", "", "", "", ""}};
  ...
  type = MenuViewStr->isChecked()
             ? 0
             : (MenuViewCas->isChecked() ? 1 : (MenuViewNet->isChecked() ? 2 : 3));
  ...
  foreach (QString line, lines) {
    switch (type) {
      case 0:
        if (line.contains("STR")) ns++;
        break;
      case 1:
        if (line.contains("CAS")) ns++;
        break;
      case 2:
        if (line.contains("NET")) ns++;
        break;
    }
  }
  if (ns <= 0) return;
...
void MainForm::UpdateMap(void) {
  ...
  if (Address->currentText() == "")
    ...
    googleMapView->setWindowTitle(
        QString(tr("NTRIP Data Stream Map: %1")).arg(Address->currentText()));
  ...
  for (int i = 0; i < Table0->rowCount(); i++) {
    ...
    lat = LatText.toDouble(&okay);
    if (!okay) continue;
    lon = LonText.toDouble(&okay);
    if (!okay) continue;
    title = Table0->item(i, 1)->text();
    msg = "<b>" + Table0->item(i, 1)->text() + "</b>: " + Table0->item(i, 2)->text() + " (" +
          Table0->item(i, 8)->text() + ")<br>" + "Format: " + Table0->item(i, 3)->text() + ", " +
          Table0->item(i, 4)->text() + ", <br> " + "Nav-Sys: " + Table0->item(i, 6)->text() +
          "<br>" + "Network: " + Table0->item(i, 7)->text() + "<br>" +
          "Latitude/Longitude: " + Table0->item(i, 9)->text() + ", " + Table0->item(i, 10)->text() +
          "<br>" + "Generator: " + Table0->item(i, 13)->text();
    googleMapView->AddMark(lat, lon, title, msg);
  }
}
//---------------------------------------------------------------------------
void MainForm::BtnStaClick() { staListDialog->exec(); }
//---------------------------------------------------------------------------
void MainForm::UpdateEnable(void) { BtnSta->setEnabled(StaMask->isChecked()); }
//---------------------------------------------------------------------------
void MainForm::StaMaskClick() { UpdateEnable(); }
//---------------------------------------------------------------------------
