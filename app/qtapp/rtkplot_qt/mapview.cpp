...
//---------------------------------------------------------------------------
MapView::MapView(QWidget *parent) : QDialog(parent) {
  loaded = false;
  setupUi(this);
...
//---------------------------------------------------------------------------
void MapView::BtnCloseClick() { close(); }
//---------------------------------------------------------------------------
void MapView::MapSel1Click() { SelectMap(0); }
//---------------------------------------------------------------------------
void MapView::MapSel2Click() { SelectMap(1); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void MapView::BtnZoomOutClick() { ExecFunc(MapSel, "ZoomOut()"); }
//---------------------------------------------------------------------------
void MapView::BtnZoomInClick() { ExecFunc(MapSel, "ZoomIn()"); }
//---------------------------------------------------------------------------
...
void MapView::ShowMapLL(void) {
  ...
  while (!ifp.atEnd()) {
    ...
    out << buff;
    if (buff.contains("<script src=\"qrc:/leaflet/leaflet.js\"></script>"))
...
//---------------------------------------------------------------------------
void MapView::AddMark(int map, int index, double lat, double lon, int state) {
  QString func = QString("AddMark(%1,%2,'SOL%3','SOLUTION %4')")
                     .arg(lat, 0, 'f', 9)
                     .arg(lon, 0, 'f', 9)
                     .arg(index)
                     .arg(index);
  ...
  ExecFunc(map, func);
  if (state)
    func = QString("ShowMark('SOL%1')").arg(index);
  else
    func = QString("HideMark('SOL%1')").arg(index);
  ExecFunc(map, func);
}
//---------------------------------------------------------------------------
...
void MapView::SetMark(int index, double lat, double lon) {
  QString func =
      QString("PosMark(%1,%2,'SOL%3')").arg(lat, 0, 'f', 9).arg(lon, 0, 'f', 9).arg(index);
