...
//---------------------------------------------------------------------------
//---------------------------------------------------------------------------
PlotOptDialog::PlotOptDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);
...
void PlotOptDialog::showEvent(QShowEvent *event) {
  ...
  for (int i = 0; i < 8; i++)
    for (int j = 0; j < 2; j++) MColor[j][i] = plot->MColor[j][i];

  for (int i = 0; i < 4; i++) CColor[i] = plot->CColor[i];
  ...
  Color1->setStyleSheet(
      QString("QLabel {background-color: %1;}").arg(color2String(plot->CColor[0])));
  Color2->setStyleSheet(
      QString("QLabel {background-color: %1;}").arg(color2String(plot->CColor[1])));
  Color3->setStyleSheet(
      QString("QLabel {background-color: %1;}").arg(color2String(plot->CColor[2])));
  Color4->setStyleSheet(
      QString("QLabel {background-color: %1;}").arg(color2String(plot->CColor[3])));
...
void PlotOptDialog::BtnOKClick() {
  ...
  for (int i = 0; i < 8; i++)
    for (int j = 0; j < 2; j++) plot->MColor[j][i] = MColor[j][i];

  for (int i = 0; i < 4; i++) plot->CColor[i] = CColor[i];
  ...
  plot->NavSys = (NavSys1->isChecked() ? SYS_GPS : 0) | (NavSys2->isChecked() ? SYS_GLO : 0) |
...
void PlotOptDialog::BtnColor1Click() {
  ...
  if (dialog.result() != QDialog::Accepted) return;
  Color1->setStyleSheet(
      QString("QLabel {background-color: %1;}").arg(color2String(dialog.currentColor())));
...
void PlotOptDialog::BtnTLEFileClick() {
  TLEFile->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Open"), QString(),
      tr("Text Files (*.txt);;Position Files (*.pos *.snx);;All (*.*)"))));
}
//---------------------------------------------------------------------------
void PlotOptDialog::BtnTLESatFileClick() {
  TLESatFile->setText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Open"), QString(),
      tr("Text Files (*.txt);;Position Files (*.pos *.snx);;All (*.*)"))));
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void PlotOptDialog::AutoScaleChange() { UpdateEnable(); }
//---------------------------------------------------------------------------
void PlotOptDialog::OriginChange() { UpdateEnable(); }
//---------------------------------------------------------------------------
...
void PlotOptDialog::UpdateEnable(void) {
  ...
  LabelRefPos->setEnabled(Origin->currentIndex() == 5 || Origin->currentIndex() == 6 ||
                          RcvPos->currentIndex() == 1);
  BtnRefPos->setEnabled(Origin->currentIndex() == 5 || Origin->currentIndex() == 6 ||
                        RcvPos->currentIndex() == 1);
  EditTimeSync->setEnabled(ChkTimeSync->isChecked());
}
//---------------------------------------------------------------------------
void PlotOptDialog::RcvPosChange() { UpdateEnable(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void PlotOptDialog::ChkTimeSyncClick() { UpdateEnable(); }
//---------------------------------------------------------------------------
