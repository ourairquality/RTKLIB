...
Plot::Plot(QWidget *parent) : QMainWindow(parent) {
  ...
  for (int i = 0; i < 2; i++) GraphE[i] = new Graph(Disp);
  ...
  RangeList->setWindowFlags(Qt::Window | Qt::WindowStaysOnTopHint | Qt::X11BypassWindowManagerHint |
...
Plot::~Plot() {
  ...
  delete GraphR;
  delete GraphS;
  for (int i = 0; i < 2; i++) delete GraphE[i];
  for (int i = 0; i < 3; i++) delete GraphG[i];
  delete RangeList;
...
bool Plot::eventFilter(QObject *obj, QEvent *event) {
  if ((obj == Disp) && (event->type() == QEvent::MouseMove)) {
    ...
    if (childAt(mouseEvent->pos()) == Disp) mouseMove(mouseEvent);
  }
...
void Plot::showEvent(QShowEvent *event) {
  ...
  if (parser.isSet(rawOption)) OpenRaw = 1;
  if (parser.isSet(titleOption)) Title = parser.value(titleOption);
  if (parser.isSet(pathOption)) path1 = parser.value(pathOption);
  if (parser.isSet(path1Option)) path1 = parser.value(path1Option);
  if (parser.isSet(path2Option)) path2 = parser.value(path2Option);
  if (parser.isSet(traceOption)) Trace = parser.value(traceOption).toInt();

  const QStringList args = parser.positionalArguments();
  QString file;
  foreach (file, args) OpenFiles.append(file);
  ...
  if (path1 != "" || path2 != "") {
    ...
    setWindowTitle(
        Title != "" ? Title
                    : QString(tr("%1 ver. %2 %3")).arg(PRGNAME).arg(VER_RTKLIB).arg(PATCH_LEVEL));
  }
  if (ShapeFile != "") {
    QStringList files;
    ...
    int n = expath(qPrintable(ShapeFile), paths, 1024, MAXSHAPEFILE);
  ...
  if (TLEFile != "") tle_read(qPrintable(TLEFile), &TLEData);
  if (TLESatFile != "") tle_name_read(qPrintable(TLESatFile), &TLEData);
  ...
  if (OpenFiles.count() > 0) {
    if (CheckObs(OpenFiles.at(0)) || OpenRaw)
      ReadObs(OpenFiles);
    else
      ReadSol(OpenFiles, 0);
  }
...
void Plot::paintEvent(QPaintEvent *) { UpdateDisp(); }
...
void Plot::dropEvent(QDropEvent *event) {
  ...
  if (ConnectState || !event->mimeData()->hasUrls()) return;
...
void Plot::MenuOpenSol1Click() {
  ...
  ReadSol(QStringList(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
              this, tr("Open Solution 1"), QString(),
              tr("Solution File (*.pos *.stat *.nmea *.txt *.ubx);;All (*.*)")))),
          0);
}
...
void Plot::MenuOpenSol2Click() {
  ...
  ReadSol(QStringList(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
              this, tr("Open Solution 2"), QString(),
              tr("Solution File (*.pos *.stat *.nmea *.txt *.ubx);;All (*.*)")))),
          1);
}
...
void Plot::MenuOpenMapImageClick() {
  ...
  ReadMapData(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Open Map Image"), MapImageFile, tr("JPEG File (*.jpg *.jpeg);;All (*.*)"))));
}
// callback on menu-open-track-points ---------------------------------------
void Plot::MenuOpenShapeClick() {
  trace(3, "MenuOpenShapePath\n");

  QStringList files = QFileDialog::getOpenFileNames(this, tr("Open Shape File"), QString(),
                                                    tr("Shape File (*.shp);;All (*.*)"));
  for (int i = 0; i < files.size(); i++) files[i] = QDir::toNativeSeparators(files.at(i));
...
void Plot::MenuOpenSkyImageClick() {
  ...
  ReadSkyData(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Open Sky Image"), SkyImageFile, tr("JPEG File (*.jpg *.jpeg);;All (*.*)"))));
}
// callback on menu-oepn-waypoint -------------------------------------------
void Plot::MenuOpenWaypointClick() {
  trace(3, "MenuOpenWaypointClick\n");

  ReadWaypoint(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Open Waypoint"), SkyImageFile, tr("Waypoint File (*.gpx, *.pos);;All (*.*)"))));
}
// callback on menu-open-obs-data -------------------------------------------
void Plot::MenuOpenObsClick() {
  trace(3, "MenuOpenObsClick\n");
  ReadObs(QFileDialog::getOpenFileNames(
      this, tr("Open Obs/Nav Data"), QString(),
      tr("RINEX OBS (*.obs *.*o *.*d *.O.rnx *.*o.gz *.*o.Z *.d.gz *.d.Z);;All (*.*)")));
}
// callback on menu-open-nav-data -------------------------------------------
void Plot::MenuOpenNavClick() {
  trace(3, "MenuOpenNavClick\n");

  ReadNav(QFileDialog::getOpenFileNames(
      this, tr("Open Raw Obs/Nav Messages"), QString(),
      tr("RINEX NAV (*.nav *.gnav *.hnav *.qnav *.*n *.*g *.*h *.*q *.*p *N.rnx);;All (*.*)")));
}
// callback on menu-open-elev-mask ------------------------------------------
void Plot::MenuOpenElevMaskClick() {
  trace(3, "MenuOpenElevMaskClick\n");

  ReadElMaskData(QDir::toNativeSeparators(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Opene Elevation Mask"), QString(), tr("Text File (*.txt);;All (*.*)")))));
}
...
void Plot::MenuSaveImageClick() {
  Buff.save(QDir::toNativeSeparators(QFileDialog::getSaveFileName(
      this, tr("Save Image"), QString(), tr("JPEG  (*.jpg);;Windows Bitmap (*.bmp)"))));
}
// callback on menu-save-waypoint -------------------------------------------
void Plot::MenuSaveWaypointClick() {
  trace(3, "MenuSaveWaypointClick\n");

  SaveWaypoint(QDir::toNativeSeparators(QFileDialog::getSaveFileName(
      this, tr("Save Waypoint"), QString(), tr("GPX File (*.gpx, *.pos);;All (*.*)"))));
}
...
void Plot::MenuSaveDopClick() {
  ...
  SaveDop(QDir::toNativeSeparators(QFileDialog::getSaveFileName(
      this, tr("Save Data"), QString(), tr("All (*.*);;Text File (*.txt)"))));
}
...
void Plot::MenuSaveElMaskClick() {
  ...
  SaveElMask(QDir::toNativeSeparators(QFileDialog::getSaveFileName(
      this, tr("Save Data"), QString(), tr("All (*.*);;Text File (*.txt)"))));
}
...
void Plot::MenuTimeClick() {
  ...
  for (i = 0; i < 3; i++) spanDialog->TimeEna[i] = TimeEna[i];
  ...
  if (TimeEna[0] != spanDialog->TimeEna[0] || TimeEna[1] != spanDialog->TimeEna[1] ||
      TimeEna[2] != spanDialog->TimeEna[2] || timediff(TimeStart, spanDialog->TimeStart) != 0.0 ||
...
void Plot::MenuSrcObsClick() {
  ...
  cstat = rtk_uncompress(file, tmpfile, sizeof(tmpfile));
...
void Plot::MenuAnimStartClick() { trace(3, "MenuAnimStartClick\n"); }
// callback on menu-animation-stop ------------------------------------------
void Plot::MenuAnimStopClick() { trace(3, "MenuAnimStopClick\n"); }
...
void Plot::mousePressEvent(QMouseEvent *event) {
  ...
  Drag = event->buttons().testFlag(Qt::LeftButton)
             ? 1
             : (event->buttons().testFlag(Qt::RightButton) ? 11 : 0);
...
void Plot::mouseMove(QMouseEvent *event) {
  ...
  if ((abs(Disp->mapFromGlobal(event->globalPosition()).x() - Xn) < 1) &&
      (abs(Disp->mapFromGlobal(event->globalPosition()).y() - Yn) < 1))
    return;
  ...
  if ((abs(Disp->mapFromGlobal(event->globalPos()).x() - Xn) < 1) &&
      (abs(Disp->mapFromGlobal(event->globalPos()).y() - Yn) < 1))
    return;
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 0, 0)
  trace(3, "DispMouseUp: X=%d Y=%d\n", Disp->mapFromGlobal(event->globalPosition()).x(),
        Disp->mapFromGlobal(event->globalPosition()).y());
#else
  trace(3, "DispMouseUp: X=%d Y=%d\n", Disp->mapFromGlobal(event->globalPos()).x(),
        Disp->mapFromGlobal(event->globalPos()).y());
#endif
  Drag = 0;
...
void Plot::MouseMoveTrk(int X, int Y, double dx, double dy, double dxs, double dys) {
...
void Plot::MouseMoveSol(int X, int Y, double dx, double dy, double dxs, double dys) {
  QPoint p1, p2, p(X, Y);
  double x, y, xs, ys;
  int i, sel = !BtnSol1->isChecked() && BtnSol2->isChecked() ? 1 : 0;
  ...
  if (Drag <= 4) {
    for (i = 0; i < 3; i++) {
      ...
      if (!MenuFixVert->isChecked() || !MenuFixVert->isEnabled()) y = i == Drag - 1 ? Yc + dy : y;
    ...
    if (!MenuFixVert->isChecked() || !MenuFixVert->isEnabled()) y = Yc + dy;
...
void Plot::MouseMoveObs(int X, int Y, double dx, double dy, double dxs, double dys) {
  QPoint p1, p2, p(X, Y);
  double x, y, xs, ys;
  int i;
...
void Plot::TimerTimer() {
  const QColor color[] = {Qt::red, Qt::gray, QColor(0x00, 0xAA, 0xFF), Qt::green,
  ...
  if (ConnectState) {  // real-time input mode
    for (i = 0; i < 2; i++) {
      ...
      strstatus[i]->setStyleSheet(
          QStringLiteral("QLabel {color %1;}").arg(color2String(color[stat < 3 ? stat + 1 : 3])));
      ...
      while ((n = strread(Stream + i, buff, sizeof(buff))) > 0) {
        for (j = 0; j < n; j++) {
          ...
          if (Week == 0 && SolData[i].n == 1) {  // first data
            if (PlotType > PLOT_NSAT) UpdateType(PLOT_TRK);
      ...
      if (nmsg[i] > 0) {
        strstatus[i]->setStyleSheet(
            QStringLiteral("QLabel {color %1;}").arg(color2String(color[4])));
    ...
    while (strread(&StrTimeSync, (uint8_t *)StrBuff + NStrBuff, 1)) {
      ...
      if (StrBuff[NStrBuff - 1] == '\n') {
        ...
        if (sscanf(StrBuff, "%lf/%lf/%lf %lf:%lf:%lf", ep, ep + 1, ep + 2, ep + 3, ep + 4,
                   ep + 5) >= 6) {
          time = epoch2time(ep);
        }
        NStrBuff = 0;
...
void Plot::UpdateSize(void) {
  ...
  GraphS->resize();
  GraphR->resize();
  for (int i = 0; i < 3; i++) GraphG[i]->resize();
  for (int i = 0; i < 2; i++) GraphE[i]->resize();
...
void Plot::UpdateSatMask(void) {
  ...
  if (ExSats != "") {
    ...
    for (p = strtok(buff, " "); p; p = strtok(NULL, " ")) {
      if (*p == '+' && (sat = satid2no(p + 1)))
        SatMask[sat - 1] = 0;  // included
      else if ((sat = satid2no(p)))
        SatMask[sat - 1] = 1;  // excluded
    }
  }
}
...
void Plot::UpdateSatSel(void) {
  ...
  if (SatListText == "G")
    sys = SYS_GPS;
  else if (SatListText == "R")
    sys = SYS_GLO;
  else if (SatListText == "E")
    sys = SYS_GAL;
  else if (SatListText == "J")
    sys = SYS_QZS;
  else if (SatListText == "C")
    sys = SYS_CMP;
  else if (SatListText == "I")
    sys = SYS_IRN;
  else if (SatListText == "S")
    sys = SYS_SBS;
...
void Plot::UpdateEnable(void) {
  ...
  BtnSol12->setEnabled(!ConnectState && PlotType <= PLOT_SOLA && SolData[0].n > 0 &&
                       SolData[1].n > 0);

  QFlag->setVisible(PlotType == PLOT_TRK || PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
  ...
  SatList->setVisible(PlotType == PLOT_RES || PlotType == PLOT_RESE || PlotType >= PLOT_OBS ||
                      PlotType == PLOT_SKY || PlotType == PLOT_DOP || PlotType == PLOT_SNR ||
                      PlotType == PLOT_SNRE || PlotType == PLOT_MPS);
  QFlag->setEnabled(data);
  ObsType->setEnabled(data && !SimObs);
  ObsType2->setEnabled(data && !SimObs);

  Panel102->setVisible(PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA ||
                       PlotType == PLOT_NSAT || PlotType == PLOT_RES || PlotType == PLOT_RESE ||
  ...
  BtnCenterOri->setVisible(PlotType == PLOT_TRK || PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
                           PlotType == PLOT_SOLA || PlotType == PLOT_NSAT);
  BtnCenterOri->setEnabled(PlotType != PLOT_NSAT);
  BtnRangeList->setVisible(PlotType == PLOT_TRK || PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
                           PlotType == PLOT_SOLA || PlotType == PLOT_NSAT);
  BtnRangeList->setEnabled(PlotType != PLOT_NSAT);

  BtnFitHoriz->setVisible(PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA ||
                          PlotType == PLOT_NSAT || PlotType == PLOT_RES || PlotType == PLOT_OBS ||
                          PlotType == PLOT_DOP || PlotType == PLOT_SNR || PlotType == PLOT_SNRE);
  BtnFitVert->setVisible(PlotType == PLOT_TRK || PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
                         PlotType == PLOT_SOLA);
  ...
  BtnFixHoriz->setVisible(PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA ||
                          PlotType == PLOT_NSAT || PlotType == PLOT_RES || PlotType == PLOT_OBS ||
                          PlotType == PLOT_DOP || PlotType == PLOT_SNR);
  BtnFixVert->setVisible(PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA);
  ...
  BtnShowImg->setVisible(PlotType == PLOT_TRK || PlotType == PLOT_SKY || PlotType == PLOT_MPS);
...
void Plot::LoadOpt(void) {
  ...
  MapStrs[0][1] =
      settings.value("mapview/mapstrs_0_1", "https://tile.openstreetmap.org/{z}/{x}/{y}.png")
          .toString();
...
//---------------------------------------------------------------------------
void Plot::DirSelSelected(QModelIndex) { DirSelector->setVisible(false); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void Plot::BtnFreqClick() { freqDialog->exec(); }
//---------------------------------------------------------------------------
