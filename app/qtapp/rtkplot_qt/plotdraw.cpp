...
void Plot::DrawTrkImage(QPainter &c, int level) {
  ...
  pos[1] = MapLon * D2R;
  pos2ecef(pos, rr);
  if (norm(OPos, 3) > 0.0) PosToXyz(time, rr, 0, xyz);
...
#define B_IN_B(bound1, bound2)                                                   \
  (bound1[0] <= bound2[1] && bound1[1] >= bound2[0] && bound1[2] <= bound2[3] && \
   bound1[3] >= bound2[2])
...
void Plot::DrawTrkMap(QPainter &c, int level) {
  ...
  enu[3][0] = xl[1];
  enu[3][1] = yl[1];
  enu[4][0] = (xl[0] + xl[1]) / 2.0;
  enu[4][1] = yl[0];
  enu[5][0] = (xl[0] + xl[1]) / 2.0;
  enu[5][1] = yl[1];
  enu[6][0] = xl[0];
  enu[6][1] = (yl[0] + yl[1]) / 2.0;
  enu[7][0] = xl[1];
  enu[7][1] = (yl[0] + yl[1]) / 2.0;
  ...
  for (i = MAXMAPLAYER - 1; i >= 0; i--) {
    ...
    for (data = Gis.data[i]; data; data = data->next) {
      if (data->type == 1) {  // point
        ...
        if ((n = poly->npnt) <= 0 || !B_IN_B(poly->bound, bound)) continue;
        p = new QPoint[n];
        for (j = m = 0; j < n; j++) {
          ...
          GraphT->ToPoint(xyz[0], xyz[1], p1);
          if (m == 0 || p1.x() != p[m - 1].x() || p1.y() != p[m - 1].y()) p[m++] = p1;
        }
        GraphT->DrawPoly(c, p, m, MapColor[i], 0);
        delete[] p;
      } else if (level && data->type == 3) {  // polygon
        polygon = (gis_polygon_t *)data->data;
        if ((n = polygon->npnt) <= 0 || !B_IN_B(polygon->bound, bound)) continue;
        p = new QPoint[n];
        for (j = m = 0; j < n; j++) {
...
void Plot::DrawTrkPnt(QPainter &c, const TIMEPOS *pos, int level, int style) {
  ...
  if (!(PlotStyle % 2)) GraphT->DrawPoly(c, pos->x, pos->y, pos->n, CColor[3], style);
...
void Plot::DrawTrkPos(QPainter &c, const double *rr, int type, int siz, QColor color,
                      const QString &label) {
  gtime_t time = {0, 0};
  QPoint p1;
  double xyz[3], xs, ys;
...
void Plot::DrawTrkStat(QPainter &c, const TIMEPOS *pos, const QString &header, int p) {
  ...
  if (pos->n > 0) {
    ...
    for (i = 0; i < pos->n; i++) d[i] = SQRT(SQR(pos->x[i]) + SQR(pos->y[i]));
    ...
    s[n++] = QString("AVE=E:%1m N:%2m U:%3m")
                 .arg(ave[0], 7, 'f', 4)
                 .arg(ave[1], 7, 'f', 4)
                 .arg(ave[2], 7, 'f', 4);
    s[n++] = QString("STD=E:%1m N:%2m U:%3m")
                 .arg(std[0], 7, 'f', 4)
                 .arg(std[1], 7, 'f', 4)
                 .arg(std[2], 7, 'f', 4);
  ...
  for (i = 0; i < n; i++, p1.ry() += fonth) DrawLabel(GraphT, c, p1, s[i], 2, 2);
}
// draw error-circle on track-plot ------------------------------------------
void Plot::DrawTrkError(QPainter &c, const TIMEPOS *pos, int style) {
  const double sint[36] = {0.0000,  0.1736,  0.3420,  0.5000,  0.6428,  0.7660,  0.8660,  0.9397,
                           0.9848,  1.0000,  0.9848,  0.9397,  0.8660,  0.7660,  0.6428,  0.5000,
                           0.3420,  0.1736,  0.0000,  -0.1736, -0.3420, -0.5000, -0.6428, -0.7660,
                           -0.8660, -0.9397, -0.9848, -1.0000, -0.9848, -0.9397, -0.8660, -0.7660,
                           -0.6428, -0.5000, -0.3420, -0.1736};
...
void Plot::DrawTrkVel(QPainter &c, const TIMEPOS *vel) {
  ...
  if (vel && vel->n > 0) {
    if ((v = SQRT(SQR(vel->x[0]) + SQR(vel->y[0]))) > 1.0) dir = ATAN2(vel->x[0], vel->y[0]) * R2D;
  }
...
void Plot::DrawSol(QPainter &c, int level, int type) {
  QString label[] = {tr("E-W"), tr("N-S"), tr("U-D")},
          unit[] = {"m", "m/s", QString("m/s%1").arg(up2Char)};
  ...
  if (BtnShowTrack->isChecked() && (BtnFixHoriz->isChecked() || BtnFixVert->isChecked())) {
    ...
    for (i = 0; i < 3 && pos->n > 0; i++) {
      ...
      if (BtnFixVert->isChecked()) yc = i == 0 ? pos->x[0] : (i == 1 ? pos->y[0] : pos->z[0]);
  ...
  if (BtnShowTrack->isChecked() &&
      (BtnSol1->isChecked() || BtnSol2->isChecked() || BtnSol12->isChecked())) {
    pos = SolToPos(SolData + sel, SolIndex[sel], 0, type);
    ...
    for (j = k = 0; j < 3 && pos->n > 0; j++) {
      ...
      if (BtnSol2->isChecked() && pos2->n > 0 &&
          (time1.time == 0 || fabs(timediff(time1, time2)) < DTTOL * 2.0)) {
...
void Plot::DrawSolPnt(QPainter &c, const TIMEPOS *pos, int level, int style) {
  ...
  for (i = 0; i < pos->n; i++) x[i] = TimePos(pos->t[i]);
  ...
  for (i = 0; i < 3; i++) {
    ...
    if (!level || !(PlotStyle % 2)) DrawPolyS(GraphG[i], c, x, y, pos->n, CColor[3], style);
    ...
    if (level && ShowErr && PlotType <= PLOT_SOLA && PlotStyle < 2) {
      ...
      if (ShowErr == 1) {
        for (j = 0; j < pos->n; j++)
          GraphG[i]->DrawMark(c, x[j], y[j], 12, CColor[1], static_cast<int>(SQRT(s[j]) * 2.0 / ys),
                              0);
      } else {
        yy = new double[pos->n];
...
void Plot::DrawSolStat(QPainter &c, const TIMEPOS *pos, const QString &unit, int p) {
  ...
  for (i = 0; i < 3; i++) {
    ...
    s = QString("AVE=%1%2 STD=%3%2 RMS=%4%2")
            .arg(ave, 0, 'f', 4)
            .arg(unit)
            .arg(std, 0, 'f', 4)
            .arg(rms, 0, 'f', 4);
...
void Plot::DrawNsat(QPainter &c, int level) {
  QString label[] = {tr("# of Valid Satellites"), tr("Age of Differential (s)"),
...
void Plot::DrawObs(QPainter &c, int level) {
  ...
  if (level && PlotStyle <= 2) DrawObsEphem(c, yp);
  ...
  if (level && PlotStyle <= 2) DrawObsSlip(c, yp);
  ...
  if (BtnShowTrack->isChecked() && 0 <= ind && ind < NObs) {
    ...
    if (!BtnFixHoriz->isChecked()) GraphR->DrawMark(c, xl[0], yl[1] - 1E-6, 1, CColor[2], 9, 0);
  }
}
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (m = 0; m < NSATGLO; m++) {
      for (j = 0; j < Nav.ng[m]; j++) {
        ...
        if (in)
          GraphR->DrawMark(c, ps[2], 0, Nav.geph[m][j].svh ? MColor[0][5] : CColor[1],
    ...
    for (m = 0; m < NSATSBS; m++) {
      for (j = 0; j < Nav.ns[m]; j++) {
        ...
        if (in)
          GraphR->DrawMark(c, ps[2], 0, Nav.seph[m][j].svh ? MColor[0][5] : CColor[1],
...
void Plot::DrawSky(QPainter &c, int level) {
  ...
  if (BtnShowImg->isChecked()) DrawSkyImage(c, level);
  ...
  if (ShowSlip && PlotStyle <= 2) {
    ...
    for (i = 0; i < Obs.n; i++) {
      ...
      GraphS->DrawMark(c, x, y, 4, MColor[0][5], MarkSize * 3 + 2,
  ...
  if (BtnShowTrack->isChecked() && 0 <= ind && ind < NObs) {
    for (i = IndexObs[ind]; i < Obs.n && i < IndexObs[ind + 1]; i++) {
      ...
      GraphS->DrawMark(c, x, y, 1, col == Qt::black ? MColor[0][0] : CColor[2],
                       Disp->font().pointSize() * 2 + 5, 0);
  ...
  if (ShowStats && BtnShowTrack->isChecked() && 0 <= ind && ind < NObs && !SimObs) {
    ...
    if (obstype == "ALL") {
      s = QString::asprintf("%3s: %*s %*s%*s %*s", "SAT", NFREQ, "PR", NFREQ, "CP", NFREQ * 3,
                            "CN0", NFREQ, "LLI");
    } else {
    ...
    for (i = IndexObs[ind]; i < Obs.n && i < IndexObs[ind + 1]; i++) {
      ...
      if (HideLowSat && ElMaskP && El[i] < ElMaskData[static_cast<int>(Az[i] * R2D + 0.5)])
        continue;
      ...
      if (obstype == "ALL") {
        ...
        for (j = 0; j < NFREQ; j++) {
          if (obs->P[j] == 0.0 && obs->L[j] == 0.0)
            ...
            s += QString("%1 ").arg(obs->SNR[j] * SNR_UNIT, 2, 'f', 0, QChar('0'));
        }
        for (j = 0; j < NFREQ; j++) {
          if (obs->L[j] == 0.0)
            s += "-";
          else
            s += QString("%1").arg(obs->LLI[j]);
        }
      } else if (ok) {
        if (!obs->code[freq - 1]) continue;
        s += QString("%1  %2 %3 %4  ")
                 .arg(code2obs(obs->code[freq - 1]))
                 .arg(obs->P[freq - 1] == 0.0 ? "-" : "C")
                 .arg(obs->L[freq - 1] == 0.0 ? "-" : "L")
                 .arg(obs->D[freq - 1] == 0.0 ? "-" : "D");
        if (obs->P[freq - 1] == 0.0 && obs->L[freq - 1] == 0.0)
          s += "---- ";
        else
          s += QString("%1 ").arg(obs->SNR[freq - 1] * SNR_UNIT, 4, 'f', 1);
        if (obs->L[freq - 1] == 0.0)
          s += " -";
        else
        ...
        if (obs->P[j] == 0.0 && obs->L[j] == 0.0)
          ...
          s += QString("%1 ").arg(obs->SNR[j] * SNR_UNIT, 4, 'f', 1);
        if (obs->L[j] == 0.0)
          s += " -";
        else
...
void Plot::DrawDop(QPainter &c, int level) {
  ...
  for (i = 0; i < 4; i++) {
    ...
    if (!(PlotStyle % 2)) DrawPolyS(GraphR, c, x, y, n, CColor[3], 0);
    if (level && PlotStyle < 2) {
      for (j = 0; j < n; j++) {
        if (y[j] == 0.0) continue;
  ...
  if (doptype == 0 || doptype == 1) {
    ...
    if (!(PlotStyle % 2)) DrawPolyS(GraphR, c, x, y, n, CColor[3], 1);
    if (level && PlotStyle < 2) {
      for (i = 0; i < n; i++) GraphR->DrawMark(c, x[i], y[i], 0, MColor[0][1], MarkSize, 0);
    }
  }
...
void Plot::DrawDopStat(QPainter &c, double *dop, int *ns, int n) {
  ...
  if (DopType->currentIndex() == 0 || DopType->currentIndex() >= 2) {
    ...
    s2[m++] = QString("NDOP=%1(%2%%) %3(%4%%) %5(%6%%) %7(%8f%%)")
                  .arg(ndop[0])
                  .arg(n > 0 ? ndop[0] * 100.0 / n : 0.0, 4, 'f', 1)
                  .arg(ndop[1])
                  .arg(n > 0 ? ndop[1] * 100.0 / n : 0.0, 4, 'f', 1)
                  .arg(ndop[2])
                  .arg(n > 0 ? ndop[2] * 100.0 / n : 0.0, 4, 'f', 1)
                  .arg(ndop[3])
                  .arg(n > 0 ? ndop[3] * 100.0 / n : 0.0, 4, 'f', 1);
  }
  ...
  if (NObs > 0 && BtnSol1->isChecked()) {
    ...
    for (int i = 0, l = 0; i < 3; i++) {
        ...
        for (int j = n = 0; j < Obs.n; j++) {
          ...
          if (i == 0) {
            ...
            if (SimObs)
              ...
              col[n] = SnrColor(obs->SNR[k] * SNR_UNIT);
      ...
      if (level && i == 1 && nrms > 0 && ShowStats && !BtnShowTrack->isChecked()) {
        ...
        p1.rx() = p2.x() - 8;
        p1.ry() += 3;
        DrawLabel(GraphG[i], c, p1,
                  QString("AVE=%1m RMS=%2m").arg(ave, 0, 'f', 4).arg(rms, 0, 'f', 4), 2, 2);
      }
      if (BtnShowTrack->isChecked() && 0 <= ObsIndex && ObsIndex < NObs && BtnSol1->isChecked()) {
        ...
        p1.ry() += 3;
        DrawLabel(GraphG[i], c, p1,
                  QString("%1 %2").arg(yp[0], 0, 'f', i == 1 ? 4 : 1).arg(unit[i]), 2, 2);
      }
    }
    delete[] x;
    delete[] y;
...
void Plot::DrawSnrE(QPainter &c, int level) {
  ...
  if (NObs > 0 && BtnSol1->isChecked()) {
    ...
    for (int i = 0; i < 2; i++) {
      x[i] = new double[NObs], y[i] = new double[NObs];
...
void Plot::DrawRes(QPainter &c, int level) {
  ...
  if (n > 0 && ((sel == 0 && BtnSol1->isChecked()) || (sel == 1 && BtnSol2->isChecked()))) {
    ...
    for (int i = 0; i < 4; i++) {
      x[i] = new double[n], y[i] = new double[n];
      col[i] = new QColor[n];
    }
    ...
    if (ShowStats) {
      for (int i = 0; i < 2; i++) {
        ...
        std = ns[i] <= 1 ? 0.0
                         : SQRT((sum2[i] - 2.0 * sum[i] * ave + ns[i] * ave * ave) / (ns[i] - 1));
        rms = ns[i] <= 0 ? 0.0 : SQRT(sum2[i] / ns[i]);
        GraphG[i]->GetPos(p1, p2);
        p1.setX(p2.x() - 5);
        p1.ry() += 3;
        str = tr("AVE=%1m STD=%2m RMS=%3m")
                  .arg(ave, 0, 'f', 3)
                  .arg(std, 0, 'f', 3)
                  .arg(rms, 0, 'f', 3);
    ...
    if (BtnShowTrack->isChecked() && 0 <= ind && ind < SolData[sel].n &&
        (BtnSol1->isChecked() || BtnSol2->isChecked())) {
  ...
  if (n > 0 && ((sel == 0 && BtnSol1->isChecked()) || (sel == 1 && BtnSol2->isChecked()))) {
    ...
    if (ShowStats) {
      for (int i = 0; i < 2; i++) {
        ...
        p1.setX(p2.x() - 5);
        p1.ry() += 3;
        str = QString("AVE=%1m STD=%2m RMS=%3m")
                  .arg(ave, 0, 'f', 3)
                  .arg(std, 0, 'f', 3)
                  .arg(rms, 0, 'f', 3);
        DrawLabel(GraphG[i], c, p1, str, 2, 2);
...
void Plot::DrawPolyS(Graph *graph, QPainter &c, double *x, double *y, int n, const QColor &color,
                     int style) {
  int i, j;
...
void Plot::DrawMark(Graph *g, QPainter &c, const QPoint &p, int mark, const QColor &color, int size,
                    int rot) {
...
void Plot::Refresh_MapView(void) {
  ...
  if (BtnShowTrack->isChecked()) {
    if (BtnSol2->isChecked() && SolData[1].n > 0 && (sol = getsol(SolData + 1, SolIndex[1]))) {
      ecef2pos(sol->rr, pos);
    ...
    if (BtnSol1->isChecked() && SolData[0].n > 0 && (sol = getsol(SolData, SolIndex[0]))) {
      ecef2pos(sol->rr, pos);
