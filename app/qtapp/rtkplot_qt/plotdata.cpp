...
void Plot::ReadSol(const QStringList &files, int sel) {
  ...
  for (i = 0; i < files.count() && n < MAXNFILE; i++) strcpy(paths[n++], qPrintable(files.at(i)));
  TimeSpan(&ts, &te, &tint);
  ...
  if (SolFiles[sel] != files) SolFiles[sel] = files;
  setWindowTitle("");
...
void Plot::ReadObs(const QStringList &files) {
  ...
  if (ObsFiles != files) ObsFiles = files;
  NavFiles.clear();
...
int Plot::ReadObsRnx(const QStringList &files, obs_t *obs, nav_t *nav, sta_t *sta) {
  gtime_t ts, te;
  double tint;
  int i, n;
...
void Plot::ReadNav(const QStringList &files) {
  ...
  if (NavFiles != files) NavFiles = files;
...
void Plot::ReadElMaskData(const QString &file) {
  ...
  while (!fp.atEnd()) {
    ...
    bool okay;
    az1 = tokens.at(0).toDouble(&okay);
    if (!okay) continue;
    el1 = tokens.at(1).toDouble(&okay);
    if (!okay) continue;
...
#define ResPixelBL(img1, x, y, b1, pix)                                                           \
  {                                                                                               \
    ...
    pix = qRgb(                                                                                   \
        static_cast<quint8>(a1 * qRed(p1) + a2 * qRed(p2) + a3 * qRed(p1) + a4 * qRed(p2)),       \
...
void Plot::UpdateSky(void) {
  ...
  if (norm(SkyFov, 3) > 1e-12) RPY(SkyFov, R);
  ...
  for (j = 0; j < h2; j++)
    for (i = 0; i < w2; i++) {
      ...
      if (!SkyRes)
        ResPixelNN(bm1, x, y, b1, pix) else ResPixelBL(bm1, x, y, b1, pix) bm2.setPixel(i, j, pix);
      if (SkyBinarize) {
        YCrCb(&pix, Y);
...
void Plot::ReadMapTag(const QString &file) {
  ...
  while (!fp.atEnd()) {
    ...
    if (tokens.at(0) == "scalex")
      MapScaleX = tokens.at(1).toDouble();
    else if (tokens.at(0) == "scaley")
      MapScaleY = tokens.at(1).toDouble();
    else if (tokens.at(0) == "scaleeq")
      MapScaleEq = tokens.at(1).toInt();
    else if (tokens.at(0) == "lat")
      MapLat = tokens.at(1).toDouble();
    else if (tokens.at(0) == "lon")
      MapLon = tokens.at(1).toDouble();
  }
}
// read shapefile -----------------------------------------------------------
...
void Plot::SaveGpxFile(const QString &file) {
  ...
  for (i = 0; i < NWayPnt; i++) {
    ...
    if (PntPos[i][2] != 0.0) stream.writeTextElement("ele", QString::number(PntPos[i][2], 'f', 4));
...
void Plot::SavePosFile(const QString &file) {
  ...
  for (i = 0; i < NWayPnt; i++) {
    ...
    stream << QString("%13.9f %14.9f %10.4f %s\n")
                  .arg(PntPos[i][0], 13, 'f', 9)
                  .arg(PntPos[i][1], 14, 'f', 9)
                  .arg(PntPos[i][2], 10, 'f', 4)
                  .arg(str);
  }
  fp.close();
}
...
void Plot::ReadStaPos(const QString &file, const QString &sta, double *rr) {
  QFile fp(file);
  QByteArray buff;
  QString code;
  double pos[3];
...
void Plot::SaveDop(const QString &file) {
  ...
  tlabel =
      TimeLabel <= 1 ? tr("TIME (GPST)") : (TimeLabel <= 2 ? tr("TIME (UTC)") : tr("TIME (JST))"));

  data = QString(tr("%% %1 %2 %3 %4 %5 %6 (EL>=%7deg)\n"))
             .arg(tlabel, TimeLabel == 0 ? 13 : 19)
             .arg("NSAT", 6)
             .arg("GDOP", 8)
             .arg("PDOP", 8)
             .arg("HDOP", 8)
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < Obs.n; j++) {
      ...
      data = QString("%1 %2 %3 %4 %5 %6f\n")
                 ...
                 .arg(Az[j] * R2D, 8, 'f', 1)
                 .arg(El[j] * R2D, 8, 'f', 1)
                 .arg(Obs.data[j].SNR[k] * SNR_UNIT, 9, 'f', 2)
                 .arg(!Mp[k] ? 0.0 : Mp[k][j], 10, 'f', 4);
      fp.write(data.toLatin1());
    }
  }
}
...
void Plot::Connect(void) {
  ...
  for (i = 0; i < 2; i++) {
    if (RtStream[i] == STR_NONE)
      ...
      strcpy(path, qPrintable(StrPaths[i][0]));
    else if (RtStream[i] == STR_FILE)
      strcpy(path, qPrintable(StrPaths[i][2]));
    else if (RtStream[i] <= STR_NTRIPCLI)
      strcpy(path, qPrintable(StrPaths[i][1]));
    else
      continue;
  ...
  if (Title != "")
    ...
    setWindowTitle(tr("CONNECT %1 %2").arg(name[0]).arg(name[1]));
...
int Plot::CheckObs(const QString &file) {
  ...
  if (file.indexOf(".z") || file.indexOf(".gz") || file.indexOf(".zip") || file.indexOf(".Z") ||
      file.indexOf(".GZ") || file.indexOf(".ZIP"))
    return file.at(p - 1) == 'o' || file.at(p - 1) == 'O' || file.at(p - 1) == 'd' ||
           file.at(p - 1) == 'D';
  return file.indexOf(".obs") || file.indexOf(".OBS") || file.mid(p + 3) == "o" ||
         file.mid(p + 3) == "O" || file.mid(p + 3) == "d" || file.mid(p + 3) == "D";
}
...
void Plot::Clear(void) {
  ...
  if (!ConnectState) {
    ...
    setWindowTitle(Title != "" ? Title
                               : tr("%1 ver.%2 %3").arg(PRGNAME).arg(VER_RTKLIB).arg(PATCH_LEVEL));
  } else {
