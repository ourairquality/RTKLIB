...
//---------------------------------------------------------------------------
PntDialog::PntDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);
  QStringList labels;
  ...
  connect(PntList, SIGNAL(itemDoubleClicked(QTableWidgetItem *)), this,
          SLOT(PntListDblClick(QTableWidgetItem *)));
}
//---------------------------------------------------------------------------
void PntDialog::showEvent(QShowEvent *event) {
  ...
  for (int i = 0; i < 4; i++) PntList->setColumnWidth(i, width[i] * FontScale / 96);
}
//---------------------------------------------------------------------------
void PntDialog::BtnAddClick() {
  ...
  PntList->setItem(PntList->rowCount() - 1, 0,
                   new QTableWidgetItem(QString("%1").arg(pos[0] * R2D, 0, 'f', 9)));
  PntList->setItem(PntList->rowCount() - 1, 1,
                   new QTableWidgetItem(QString("%1").arg(pos[1] * R2D, 0, 'f', 9)));
  PntList->setItem(PntList->rowCount() - 1, 2, new QTableWidgetItem(QString("%1").arg(pos[2])));
  PntList->setItem(PntList->rowCount() - 1, 4,
                   new QTableWidgetItem(QString("Point%1").arg(PntList->rowCount(), 2)));
  noUpdate = false;
...
//---------------------------------------------------------------------------
void PntDialog::BtnUpdateClick() { UpdatePoint(); }
//---------------------------------------------------------------------------
void PntDialog::PntListSetEditText() { UpdatePoint(); }
