...
class Plot : public QMainWindow, public Ui::Plot {
 ...
 private:
  ...
  void DrawTrkPos(QPainter &g, const double *rr, int type, int siz, QColor color,
                  const QString &label);
  ...
  void DrawMark(Graph *, QPainter &g, const QPoint &p, int mark, const QColor &color, int size,
                int rot);
  ...
  void CovToXyz(const double *rr, const float *qr, int type, double *xyzs);
