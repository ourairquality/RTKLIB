...
void Plot::UpdateTimeSol(void) {
  ...
  if ((BtnSol1->isChecked() || BtnSol2->isChecked() || BtnSol12->isChecked()) &&
    ...
    if (PLOT_SOLP <= PlotType && PlotType <= PLOT_SOLA) {
      ...
      msg += QString("NS=%1 AGE=%2 RATIO=%3 Q=")
                 .arg(data->ns)
                 .arg(data->age, 0, 'f', 1)
...
void Plot::UpdateInfoSol(void) {
  ...
  if (n > 0) {
    ...
    for (i = 1; i <= 6; i++) {
      if (nq[i] <= 0) continue;
      msgs[i - 1] = QString("%1:%2(%3%) ")
                        .arg(i)
                        .arg(nq[i])
                        .arg(static_cast<double>(nq[i]) / n * 100.0, 0, 'f', 1);
    }
  }
  ShowMsg(msg);
  ShowLegend(msgs);
...
void Plot::UpdateObsType(void) {
  ...
  for (i = 0; i < Obs.n; i++)
    for (j = 0; j < NFREQ + NEXOBS; j++) cmask[Obs.data[i].code[j]] = 1;
