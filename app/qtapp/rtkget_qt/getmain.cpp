...
extern int showmsg(const char *format, ...) {
  ...
  if ((p = strstr(buff, "STAT="))) {
    ...
    buff2.append(p + 5);
    QMetaObject::invokeMethod(mainForm->MsgLabel3, "setText", Qt::QueuedConnection,
                              Q_ARG(QString, QString(buff2)));
  } else if ((p = strstr(buff, "->"))) {
    *p = '\0';
    QMetaObject::invokeMethod(mainForm->MsgLabel1, "setText", Qt::QueuedConnection,
                              Q_ARG(QString, QString(buff)));
    QMetaObject::invokeMethod(mainForm->MsgLabel2, "setText", Qt::QueuedConnection,
                              Q_ARG(QString, QString(p + 2)));
  }
  return abortf;
}
...
class DownloadThread : public QThread {
 ...
 protected:
  void run() {
    if (LogFile != "") {
      reppath(qPrintable(LogFile), path, sizeof(path), utc2gpst(timeget()), "", "");
      fp = fopen(path, append ? "a" : "w");
    }
    if (test)
      dl_test(ts, te, ti, urls, nurl, (const char **)stas, nsta, dir, 35, 0, fp);
    else
      dl_exec(ts, te, ti, seqnos, seqnoe, urls, nurl, (const char **)stas, nsta, dir,
              qPrintable(usr), qPrintable(pwd), qPrintable(proxy), opts, msg, sizeof(msg), fp);
    if (fp) fclose(fp);
  }
};
...
MainForm::MainForm(QWidget *parent) : QWidget(parent) {
  ...
  for (int i = 0; i < 8; i++) Images[i].load(QString(":/buttons/wait%1.bmp").arg(i + 1));
...
void MainForm::BtnFileClick() {
  ...
  GetTime(&ts, &te, &ti);
  if (str != "") reppath(qPrintable(str), path, sizeof(path), ts, "", "");
...
void MainForm::BtnTestClick() {
  ...
  if (LocalDir->isChecked()) strcpy(thread->dir, qPrintable(Dir->currentText()));
...
void MainForm::BtnDirClick() {
  ...
  dir = QDir::toNativeSeparators(
      QFileDialog::getExistingDirectory(this, tr("Output Directory"), dir));
  Dir->insertItem(0, dir);
  Dir->setCurrentIndex(0);
}
//---------------------------------------------------------------------------
void MainForm::DirChange() { UpdateMsg(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void MainForm::HidePasswdClick() { UpdateEnable(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void MainForm::DataTypeChange() { UpdateType(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void MainForm::StaListClick() { UpdateStaList(); }
//---------------------------------------------------------------------------
...
void MainForm::LoadOpt(void) {
  ...
  Dir->insertItem(0, setting.value("opt/localdir", "").toString());
  Dir->setCurrentIndex(0);
  DataType->insertItem(0, setting.value("opt/datatype", "").toString());
...
void MainForm::LoadUrl(QString file) {
  ...
  for (i = 0; i < n; i++) {
    ...
    if (DataType->findText(basetype) == -1) DataType->addItem(basetype);

    subtype = Types.last().mid(p + 1);
    if ((p = subtype.indexOf('_')) != -1) subtype = subtype.mid(0, p);

    if (SubType->findText(subtype) == -1) SubType->addItem(subtype);
  }
  DataType->setCurrentIndex(0);
...
void MainForm::GetTime(gtime_t *ts, gtime_t *te, double *ti) {
  ...
  eps[0] = dateTime1->date().year();
  eps[1] = dateTime1->date().month();
  eps[2] = dateTime1->date().day();
  eps[3] = dateTime1->time().hour();
  eps[4] = dateTime1->time().minute();
  epe[0] = dateTime2->date().year();
  epe[1] = dateTime2->date().month();
  epe[2] = dateTime2->date().day();
  epe[3] = dateTime2->time().hour();
  epe[4] = dateTime2->time().minute();
