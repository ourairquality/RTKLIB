...
extern void settime(gtime_t time) {
  ...
  if (tend_.time != 0 && tstart_.time != 0 && (tt = timediff(tend_, tstart_)) > 0.0) {
    QMetaObject::invokeMethod(mainForm->Progress, "setValue", Qt::QueuedConnection,
...
void MainForm::showEvent(QShowEvent *event) {
  ...
  QCommandLineOption baseStationOption(
      QStringList() << "b", QCoreApplication::translate("main", "rinex obs base station <path>"),
      QCoreApplication::translate("main", "file"));
  parser.addOption(baseStationOption);

  QCommandLineOption navFileOption(
      QStringList() << "n"
                    << "file",
  ...
  if (parser.isSet(navFileOption)) {
    ...
    for (int n = 0; n < files.size() && n < 4; n++) ifile[n]->setCurrentText(files.at(n));
  }
  ...
  if (parser.isSet(timeStartOption)) {
    ...
    dateTime1->setDateTime(
        QDateTime::fromString(parser.value(timeStartOption), "yyyy/MM/dd hh:mm:ss"));
  }
  if (parser.isSet(timeEndOption)) {
    TimeEnd->setChecked(true);
    dateTime2->setDateTime(
        QDateTime::fromString(parser.value(timeEndOption), "yyyy/MM/dd hh:mm:ss"));
  }
...
void MainForm::closeEvent(QCloseEvent *) { SaveOpt(); }
...
void MainForm::BtnExecClick() {
  ...
  if (OutputFile_Text.contains(".obs", Qt::CaseInsensitive) ||
      ...
      OutputFile_Text.contains(
          QRegularExpression(R"(\.\d\d[ondg])", QRegularExpression::CaseInsensitiveOption))) {
...
void MainForm::BtnExitClick() { close(); }
...
void MainForm::BtnInputFile1Click() {
  InputFile1->setCurrentText(QDir::toNativeSeparators(
      QFileDialog::getOpenFileName(this, tr("RINEX OBS (Rover) File"), InputFile1->currentText(),
                                   tr("All (*.*);;RINEX OBS (*.rnx *.obs *.*O *.*D)"))));
  SetOutFile();
}
// callback on button-inputfile-2 -------------------------------------------
void MainForm::BtnInputFile2Click() {
  InputFile2->setCurrentText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("RINEX OBS (Base Station) File"), InputFile2->currentText(),
      tr("All (*.*);;RINEX OBS (*.rnx *.obs *.*O *.*D)"))));
}
// callback on button-inputfile-3 -------------------------------------------
void MainForm::BtnInputFile3Click() {
  InputFile3->setCurrentText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("RINEX NAV/CLK,SP3,FCB,IONEX or SBAS/EMS File"), InputFile3->currentText(),
      tr("All (*.*);;RINEX NAV (*.rnx *.*nav *.*N *.*P *.*G *.*H *.*Q)"))));
}
// callback on button-inputfile-4 -------------------------------------------
void MainForm::BtnInputFile4Click() {
  InputFile4->setCurrentText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("RINEX NAV/CLK,SP3,FCB,IONEX or SBAS/EMS File"), InputFile4->currentText(),
      tr("All (*.*);;Precise Ephemeris/Clock (*.SP3 *.sp3 *.eph* *.clk*)"))));
}
// callback on button-inputfile-5 -------------------------------------------
void MainForm::BtnInputFile5Click() {
  InputFile5->setCurrentText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("RINEX NAV/CLK,SP3,FCB,IONEX or SBAS/EMS File"), InputFile5->currentText(),
      tr("All (*.*);;Precise Ephemeris/Clock (*.CLK *.sp3 *.eph* *.clk*)"))));
}
// callback on button-inputfile-6 -------------------------------------------
void MainForm::BtnInputFile6Click() {
  InputFile6->setCurrentText(QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("RINEX NAV/CLK,SP3,FCB,IONEX or SBAS/EMS File"), InputFile6->currentText(),
      tr("All (*.*);;FCB (*.fcb),IONEX (*.*i *.ionex),SBAS (*.sbs *.ems)"))));
}
// callback on button-outputfile --------------------------------------------
void MainForm::BtnOutputFileClick() {
  OutputFile->setCurrentText(QDir::toNativeSeparators(
      QFileDialog::getSaveFileName(this, tr("Output File"), OutputFile->currentText(),
...
void MainForm::BtnInputPlot1Click() {
  ...
  opts << "-r";
  for (int i = 0; i < 5; i++) opts << "\"" + files[i] + "\"";
...
void MainForm::TimeStartClick() { UpdateEnable(); }
// callback on time-end check -----------------------------------------
void MainForm::TimeEndClick() { UpdateEnable(); }
// callback on time-interval check ------------------------------------------
void MainForm::TimeIntFClick() { UpdateEnable(); }
// callback on time-unit check ----------------------------------------------
void MainForm::TimeUnitFClick() { UpdateEnable(); }
// callback on inputfile-1 change -------------------------------------------
void MainForm::InputFile1Change() { SetOutFile(); }
...
void MainForm::OutDirChange() { SetOutFile(); }
...
void MainForm::ExecProc(void) {
  ...
  if (!GetOption(thread->prcopt, thread->solopt, thread->filopt)) {
  ...
  if (!TimeStart->isChecked() || !TimeEnd->isChecked()) {
    if (QFileInfo::exists(thread->outfile)) {
      if (QMessageBox::question(this, tr("Overwrite"),
                                QString(tr("Overwrite existing file %1.")).arg(thread->outfile)) !=
          QMessageBox::Yes) {
        ProcessingFinished(0);
        return;
      }
...
int MainForm::GetOption(prcopt_t &prcopt, solopt_t &solopt, filopt_t &filopt) {
  ...
  prcopt.dynamics = PosMode == PMODE_KINEMA || PosMode == PMODE_PPP_KINEMA;
  ...
  strcpy(solopt.prog,
         qPrintable(QString("%1 ver.%2 %3").arg(PRGNAME).arg(VER_RTKLIB).arg(PATCH_LEVEL)));
...
QString MainForm::FilePath(const QString &file) {
  ...
  if (p != -1)
    strcpy(rov, qPrintable(RovList.mid(p)));
  else
    strcpy(rov, qPrintable(RovList));
  ...
  if (p != -1)
    strcpy(base, qPrintable(BaseList.mid(p)));
  else
    strcpy(base, qPrintable(RovList));

  reppath(qPrintable(file), path, sizeof(path), ts, rov, base);
...
void MainForm::ViewFile(const QString &file) {
  ...
  if (file == "") return;
  cstat = rtk_uncompress(qPrintable(file), tmpfile, sizeof(tmpfile));
...
// show message in message area ---------------------------------------------
void MainForm::ShowMsg(const QString &msg) { Message->setText(msg); }
