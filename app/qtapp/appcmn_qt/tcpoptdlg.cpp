...
//---------------------------------------------------------------------------
TcpOptDialog::TcpOptDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);
...
void TcpOptDialog::showEvent(QShowEvent *event) {
  QString ti[] = {tr("TCP Server Options "),     tr("TCP Client Options"),
                  ...
                  "NTRIP Caster Client Options", "NTRIP Caster Server Options",
  ...
  if (Opt == 2 || Opt == 4) {
    MntPnt->insertItem(0, url.path().mid(1));
    MntPnt->setCurrentText(url.path().mid(1));
  }
