...
//---------------------------------------------------------------------------
FtpOptDialog::FtpOptDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);
...
void FtpOptDialog::showEvent(QShowEvent *event) {
  ...
  if (tokens.size() > 1) {
    ...
    if (t.contains("T=")) {
      ...
      for (int i = 0; (i < 4) || (i < values.size()); i++) topts[i] = values.at(i).toInt();
    }
  }
  ...
  PathOffset->insertItem(0, QString::number(topts[0] / 3600.0, 'g', 2));
  PathOffset->setCurrentIndex(0);
  Interval->insertItem(0, QString::number(topts[1] / 3600.0, 'g', 2));
  Interval->setCurrentIndex(0);
  Offset->insertItem(0, QString::number(topts[2] / 3600.0, 'g', 2));
  Offset->setCurrentIndex(0);
...
//---------------------------------------------------------------------------
void FtpOptDialog::BtnKeyClick() { keyDlg->exec(); }
//---------------------------------------------------------------------------
