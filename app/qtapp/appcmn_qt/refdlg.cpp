...
//---------------------------------------------------------------------------
RefDialog::RefDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);
  Opt = 0;
...
void RefDialog::showEvent(QShowEvent *event) {
  ...
  QStringList columns;
  columns << tr("No") << tr("Latitude(%1)").arg(degreeChar) << tr("Longitude(%1)").arg(degreeChar)
          << tr("Height(m)") << tr("Id") << tr("Name") << tr("Dist(km)");
  StaList->setColumnCount(columns.size());
  StaList->setRowCount(2);

  for (int i = 0; i < columns.size(); i++)
    for (int j = 0; j < 2; j++) StaList->setItem(i, j, new QTableWidgetItem(""));

  FontScale = 2 * physicalDpiX();
  for (int i = 0; i < columns.size(); i++) StaList->setColumnWidth(i, width[i] * FontScale / 96);
...
void RefDialog::BtnLoadClick() {
  StaPosFile = QDir::toNativeSeparators(QFileDialog::getOpenFileName(
      this, tr("Load Station List..."), StaPosFile, tr("Position File (*.pos *.snx);;All (*.*)")));

  LoadList();
}
//---------------------------------------------------------------------------
void RefDialog::BtnFindClick() { FindList(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void RefDialog::AddRef(int n, double *pos, const QString code, const QString name) {
  int row = StaList->rowCount();

  for (int i = 0; i < 7; i++) StaList->setItem(row - 1, i, new QTableWidgetItem());
