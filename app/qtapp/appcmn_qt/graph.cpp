...
double Graph::AutoTickTime(double scale) {
  const double t[] = {0.1,         0.2,         0.5,          1.0,          3.0,         6.0,
...
void Graph::DrawGrid(QPainter &c, double xt, double yt) {
  ...
  if (XGrid) {
    for (int i = (int)ceil(xl[0] / xt); i * xt <= xl[1]; i++) {
      ToPoint(i * xt, 0.0, p);
      pen.setStyle(i != 0 ? Qt::DotLine : Qt::SolidLine);
      c.setPen(pen);
...
//---------------------------------------------------------------------------
void Graph::DrawMark(QPainter &c, const QPoint &p, int mark, const QColor &color, int size,
                     int rot) {
  ...
  switch (mark) {
    ...
    case 12:  // vscale
      ...
      for (int i = 0; i < n; i++) {
        ...
        ps[i].setX(mark == 11 ? x : y);
        ps[i].setY(mark == 11 ? y : x);
      }
      break;
    case 13:  // compass
      n = 7;
...
//---------------------------------------------------------------------------
void Graph::DrawMark(QPainter &c, double x, double y, int mark, const QColor &color, int size,
                     int rot) {
  QPoint p;
...
//---------------------------------------------------------------------------
void Graph::DrawMark(QPainter &c, const QPoint &p, int mark, const QColor &color,
                     const QColor &bgcolor, int size, int rot) {
  QPoint p1;

  p1 = p;
  p1.setX(p1.x() - 1);
  DrawMark(c, p1, mark, bgcolor, size, rot);  // draw with hemming
  p1 = p;
  p1.setX(p1.x() + 1);
  DrawMark(c, p1, mark, bgcolor, size, rot);
  p1 = p;
  p1.setY(p1.y() - 1);
  DrawMark(c, p1, mark, bgcolor, size, rot);
  p1 = p;
...
//---------------------------------------------------------------------------
void Graph::DrawMark(QPainter &c, double x, double y, int mark, const QColor &color,
                     const QColor &bgcolor, int size, int rot) {
  QPoint p;
...
//---------------------------------------------------------------------------
void Graph::DrawText(QPainter &c, const QPoint &p, const QString &str, const QColor &color, int ha,
                     int va, int rot) {
  // str = UTF-8 string
...
//---------------------------------------------------------------------------
void Graph::DrawText(QPainter &c, const QPoint &p, const QString &str, const QColor &color, int ha,
                     int va, int rot, const QFont &font) {
  // str = UTF-8 string
  // ha  = horizontal alignment (0: center, 1: left,   2: right)
  // va  = vertical alignment   (0: center, 1: bottom, 2: top  )
  // rot = rotation angle (deg)
...
void Graph::DrawText(QPainter &c, const QPoint &p, const QString &str, const QColor &color,
                     const QColor &bgcolor, int ha, int va, int rot) {
  QPoint p1;

  p1 = p;
  p1.setX(p1.x() - 1);
  DrawText(c, p1, str, bgcolor, ha, va, rot);  // draw with hemming
  p1 = p;
  p1.setX(p1.x() + 1);
  DrawText(c, p1, str, bgcolor, ha, va, rot);
  p1 = p;
  p1.setY(p1.y() - 1);
  DrawText(c, p1, str, bgcolor, ha, va, rot);
  p1 = p;
...
//---------------------------------------------------------------------------
void Graph::DrawCircle(QPainter &c, const QPoint &p, const QColor &color, int rx, int ry,
                       int style) {
  Qt::PenStyle ps[] = {Qt::SolidLine, Qt::DotLine, Qt::DashLine, Qt::DashDotLine,
                       Qt::DashDotDotLine};
...
//---------------------------------------------------------------------------
void Graph::DrawCircle(QPainter &c, double x, double y, const QColor &color, double rx, double ry,
                       int style) {
  QPoint p;
...
void Graph::DrawCircles(QPainter &c, int label) {
  ...
  for (int i = imin; i <= imax; i++) DrawCircle(c, 0.0, 0.0, Color[1], i * xt, i * xt, 1);
  ToPoint(0.0, 0.0, p);
...
int Graph::OnAxis(const QPoint &p) {
  ...
  return (p.x() < xmin ? 1 : (p.x() <= xmax ? 0 : 2)) +
         (p.y() < ymin ? 4 : (p.y() <= ymax ? 0 : 8));
}
//---------------------------------------------------------------------------
...
void Graph::DrawPatch(QPainter &c, QPoint *p, int n, const QColor &color1, const QColor &color2,
  ...
  if (xmax < X || xmin > X + Width - 1 || ymax < Y || ymin > Y + Height - 1) return;
