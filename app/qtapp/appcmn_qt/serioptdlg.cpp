...
//---------------------------------------------------------------------------
SerialOptDialog::SerialOptDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);
  Opt = 0;
...
void SerialOptDialog::BtnOkClick() {
  ...
  Path = QString("%1:%2:%3:%4:%5:%6")
             .arg(Port_Text)
             .arg(BitRate_Text)
             .arg(ByteSize->currentIndex() ? 8 : 7)
             .arg(parity[Parity->currentIndex()])
             .arg(StopBits->currentIndex() ? 2 : 1)
             .arg(fctr[FlowCtr->currentIndex()]);

  if (OutTcpPort->isChecked()) Path += QString("#%1").arg(TcpPort->value());
...
void SerialOptDialog::UpdatePortList(void) {
  ...
  for (int i = 0; i < ports.size(); i++) Port->addItem(ports.at(i).portName());
}
//---------------------------------------------------------------------------
void SerialOptDialog::UpdateEnable(void) { TcpPort->setEnabled(OutTcpPort->isChecked()); }
//---------------------------------------------------------------------------
void SerialOptDialog::OutTcpPortClick() { UpdateEnable(); }
