    ...
    : QDialog(parent), options(opts), ui(new Ui::OptDialog)
{
    ...
    QStringList freq_tooltips = {tr("Single Frequency"), tr("Dual-Frequency"),
                                 tr("Triple-Frequency"),
    ...
    ui->lESatelliteMetaFile->setCompleter(new QCompleter(fileModel, this));
    ...
    if (MAXPRNBDS <= 0) { ui->cBNavSys6->setEnabled(false); ui->cBNavSys7->setEnabled(false); }
...
void OptDialog::updateOptions()
{
    ...
    processingOptions.maxagessr = ui->sBMaxAgeSSR->value();
...
void OptDialog::updateUi(const prcopt_t &prcopt, const solopt_t &solopt, const filopt_t &filopt)
{
    ...
    ui->sBMaxAgeSSR->setValue(prcopt.maxagessr);
...
void OptDialog::save(const QString &file)
{
    ...
    procOpts.maxagessr = ui->sBMaxAgeSSR->value();
    ...
    sprintf(comment, "rtk options (%s, v.%s %s)", tstr, VER_RTKLIB, PATCH_LEVEL);
...
void OptDialog::saveOptions(QSettings &settings)
{
    ...
    settings.setValue("prcopt/maxagessr", ui->sBMaxAgeSSR->value());
...
void OptDialog::loadOptions(QSettings &settings)
{
    ...
    int navsys = settings.value("prcopt/navsys", SYS_GPS | SYS_GLO | SYS_GAL | SYS_BDS).toInt();
    ...
    ui->sBMaxAgeSSR->setValue(settings.value("prcopt/maxagessr", 90.0).toDouble());
...
void OptDialog::updateEnable()
{
    ...
    ui->cBTideCorrection->setEnabled(true);
