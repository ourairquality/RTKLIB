...
#include <QShowEvent>
//---------------------------------------------------------------------------
MaskOptDialog::MaskOptDialog(QWidget *parent) : QDialog(parent) {
  setupUi(this);

  Mask.ena[0] = 0;
  Mask.ena[1] = 0;
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 9; j++) Mask.mask[i][j] = 0.0;
...
void MaskOptDialog::BtnOkClick() {
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 9; j++) Mask.mask[i][j] = mask[i][j]->value();

  accept();
}
//---------------------------------------------------------------------------
void MaskOptDialog::MaskEnaClick() { UpdateEnable(); }
//---------------------------------------------------------------------------
