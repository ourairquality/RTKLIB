...
int main(int argc, char **argv) {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (gpx) {
      stat =
          convgpx(infile[i], outfile, ts, te, tint, qflg, offset, tcolor, pcolor, outalt, outtime);
    } else {
      stat =
          convkml(infile[i], outfile, ts, te, tint, qflg, offset, tcolor, pcolor, outalt, outtime);
    }
    switch (stat) {
      case -1:
