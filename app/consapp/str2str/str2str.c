...
int main(int argc, char **argv) {
  ...
  char cmd_periodic_strs[MAXSTR][MAXRCVCMD];
  char *paths[MAXSTR], *logs[MAXSTR];
  char *cmds[MAXSTR], *cmds_periodic[MAXSTR];
  ...
  int n = 0, dispint = 5000, trlevel = 0, opts[] = {10000, 10000, 2000, 32768, 10, 0, 30, 0};
  int types[MAXSTR] = {STR_FILE, STR_FILE}, stat[MAXSTR] = {0}, log_stat[MAXSTR] = {0};
  int fmts[MAXSTR] = {0}, sta = 0;
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-in") && i + 1 < argc) {
      ...
      if (!decodepath(argv[++i], types + n + 1, paths[n + 1], sizeof(s1[0]), fmts + n + 1))
        return EXIT_FAILURE;
      n++;
  ...
  for (int i = 0; i < n; i++) {
    ...
    rtkstrcpy(buff, sizeof(buff), antinfo);
    char *r, *p = strtok_r(buff, ",", &r);
    for (int j = 0; p && j < 3; p = strtok_r(NULL, ",", &r)) ant[j++] = p;
    rtkstrcpy(conv[i]->out.sta.antdes, sizeof(conv[0]->out.sta.antdes), ant[0]);
    rtkstrcpy(conv[i]->out.sta.antsno, sizeof(conv[0]->out.sta.antsno), ant[1]);
    conv[i]->out.sta.antsetup = atoi(ant[2]);
    rtkstrcpy(buff, sizeof(buff), rcvinfo);
    p = strtok_r(buff, ",", &r);
    for (int j = 0; p && j < 3; p = strtok_r(NULL, ",", &r)) rcv[j++] = p;
    rtkstrcpy(conv[i]->out.sta.rectype, sizeof(conv[i]->out.sta.rectype), rcv[0]);
    rtkstrcpy(conv[i]->out.sta.recver, sizeof(conv[i]->out.sta.recver), rcv[1]);
    rtkstrcpy(conv[i]->out.sta.recsno, sizeof(conv[i]->out.sta.recsno), rcv[2]);
