...
static int startsvr(vt_t *vt)
{
    ...
    for (unsigned i = 0; i < ninfiles && i < MAXINFILES; i++) {
...
static void prnavidata(vt_t *vt, int prev, int set)
{
    ...
    for (int i=0;i<MAXSAT;i++) {
        ...
        if ((sys & (SYS_GPS | SYS_GAL | SYS_QZS | SYS_BDS) & prcopt.navsys) == 0 ||
...
static void prssr(vt_t *vt, unsigned ri, int cbias, int pbias, unsigned prev)
{
    ...
    for (i=0;i<MAXSAT;i++) {
        ...
        if ((satsyst(i + 1, time, NULL) & prcopt.navsys) == 0) continue;
        satno2id(i+1,id);
...
//
// Note this does not change the initial set of input files and so that set
// will be reloaded if restarted. Rather it queues a new set of files to load
// in a running server and it cancels and files to load that were pending.
static void cmd_precdata(char **args, int narg, vt_t *vt)
{
  ...
  if (narg > 1) {
    ...
    if (state) {
      svr.ninfiles = 0;
      for (unsigned i = 0; i < MAXINFILES; i++) svr.infiles[i][0] = '\0';
      for (unsigned i = 1; i < narg && svr.ninfiles < MAXINFILES; i++) {
        snprintf(svr.infiles[svr.ninfiles], sizeof(svr.infiles[0]), "%s", args[i]);
        svr.ninfiles++;
        n++;
      }
    }
    rtksvrunlock(&svr);
    if (!state) vt_printf(vt, "Warning: load not queued as server stopped\n");
    if (n > 0)  vt_printf(vt, "queued load of %u files\n", n);
    sleepms(1000);
  }
