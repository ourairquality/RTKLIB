...
static void prstatus(vt_t *vt) {
  ...
  vt_printf(vt, "%-28s: %.3Lf,%.3Lf,%.3Lf\n", "pos xyz single (m) rover", rtk.sol.rr[0],
...
static void prnavidata(vt_t *vt) {
  ...
  vt_printf(vt, "ION: %9.2LE %9.2LE %9.2LE %9.2LE %9.2LE %9.2LE %9.2LE %9.2LE\n", ion[0], ion[1],
            ion[2], ion[3], ion[4], ion[5], ion[6], ion[7]);
...
static void prssr(vt_t *vt) {
  ...
  for (int i = 0; i < MAXSAT; i++) {
    ...
    rtkcatprintf(buff, sizeof(buff),
                 ...
                 ssr[i].ddeph[1] * 1E3L, ssr[i].ddeph[2] * 1E3L, ssr[i].dclk[0],
                 ssr[i].dclk[1] * 1E3L, ssr[i].dclk[2] * 1E3L, ssr[i].hrclk);
  }
  vt_puts(vt, buff);
}
/* Start command -------------------------------------------------------------*/
