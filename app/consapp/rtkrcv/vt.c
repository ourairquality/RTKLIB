...
extern vt_t *vt_open(int sock, const char *dev) {
  ...
  vt_t *vt = (vt_t *)malloc(sizeof(vt_t));
  if (!vt) {
    return NULL;
  }
...
static int clear_buff(vt_t *vt) {
  ...
  int len = vt->n;
  for (int i = 0; i < vt->cur; i++) *p++ = '\b';
  for (int i = 0; i < len; i++) *p++ = ' ';
  for (int i = 0; i < len; i++) *p++ = '\b';
...
static int hist_add(vt_t *vt, const char *buff) {
  ...
  /* Note the use of strcpy here is safe as the destination was just allocated to fit */
...
static int hist_prev(vt_t *vt) {
  ...
  for (char *p = vt->hist[vt->cur_h++]; *p; p++)
