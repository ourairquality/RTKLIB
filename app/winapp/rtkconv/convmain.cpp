...
void __fastcall TMainWindow::FormClose(TObject *Sender, TCloseAction &Action) { SaveOpt(); }
...
void __fastcall TMainWindow::Panel2Resize(TObject *Sender) {
  TButton *btns1[] = {BtnOutFile1, BtnOutFile2, BtnOutFile3, BtnOutFile4, BtnOutFile5,
                      BtnOutFile6, BtnOutFile7, BtnOutFile8, BtnOutFile9};
  TSpeedButton *btns2[] = {BtnOutFileView1, BtnOutFileView2, BtnOutFileView3,
                           BtnOutFileView4, BtnOutFileView5, BtnOutFileView6,
                           BtnOutFileView7, BtnOutFileView8, BtnOutFileView9};
  TEdit *inps[] = {OutFile1, OutFile2, OutFile3, OutFile4, OutFile5,
...
void __fastcall TMainWindow::SetOutFiles(AnsiString infile) {
  TEdit *edit[] = {OutFile1, OutFile2, OutFile3, OutFile4, OutFile5,
                   OutFile6, OutFile7, OutFile8, OutFile9};
...
TStringList *__fastcall TMainWindow::ReadList(TIniFile *ini, AnsiString cat, AnsiString key) {
...
void __fastcall TMainWindow::WriteList(TIniFile *ini, AnsiString cat, AnsiString key,
                                       TStrings *list) {
  AnsiString s;
  int i;
...
void __fastcall TMainWindow::BtnPlotClick(TObject *Sender) {
  ...
  TCheckBox *cb[] = {OutFileEna1, OutFileEna2, OutFileEna3, OutFileEna4, OutFileEna5,
                     OutFileEna6, OutFileEna7, OutFileEna8, OutFileEna9};
  int i, ena[9];
...
//---------------------------------------------------------------------------
void __fastcall TMainWindow::BtnAbortClick(TObject *Sender) { abortf = 1; }
// callback on button-convert -----------------------------------------------
void __fastcall TMainWindow::BtnConvertClick(TObject *Sender) { ConvertFile(); }
// callback on button-exit --------------------------------------------------
void __fastcall TMainWindow::BtnExitClick(TObject *Sender) { Close(); }
...
void __fastcall TMainWindow::OutDirChange(TObject *Sender) { SetOutFiles(InFile->Text); }
...
void __fastcall TMainWindow::TimeStartFClick(TObject *Sender) { UpdateEnable(); }
// callback on button-time-end ----------------------------------------------
void __fastcall TMainWindow::TimeEndFClick(TObject *Sender) { UpdateEnable(); }
// callback on button-time-interval -----------------------------------------
void __fastcall TMainWindow::TimeIntFClick(TObject *Sender) { UpdateEnable(); }
...
void __fastcall TMainWindow::InFileChange(TObject *Sender) { SetOutFiles(InFile->Text); }
//-------------------------------------------------------------------
void __fastcall TMainWindow::InFileCloseUp(TObject *Sender) { DisableHighlight(); }
// callback on format change ------------------------------------------------
void __fastcall TMainWindow::FormatChange(TObject *Sender) { UpdateEnable(); }
// get time -----------------------------------------------------------------
void __fastcall TMainWindow::GetTime(gtime_t *ts, gtime_t *te, double *tint, double *tunit) {
...
void __fastcall TMainWindow::TimeY1UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
...
void __fastcall TMainWindow::TimeH1UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
                                                TUpDownDirection Direction) {
...
void __fastcall TMainWindow::TimeY2UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
                                                TUpDownDirection Direction) {
...
void __fastcall TMainWindow::TimeH2UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
                                                TUpDownDirection Direction) {
...
AnsiString __fastcall TMainWindow::RepPath(AnsiString File) {
  AnsiString Path;
  char path[1024];
  reppath(File.c_str(), path, sizeof(path), timeadd(RnxTime, TSTARTMARGIN), RnxCode.c_str(), "");
  return Path = path;
}
// execute command ----------------------------------------------------------
...
void __fastcall TMainWindow::ConvertFile(void) {
  ...
  if (Format->ItemIndex == 0) {  // auto
    if (!strcmp(p, ".rtcm2"))
      ...
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "p"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "P"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "g"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "G"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "h"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "H"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "q"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "Q"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "l"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "L"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "c"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "C"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "i"))
      format = STRFMT_RINEX;
    else if (!strcmp(p + 3, "I"))
      format = STRFMT_RINEX;
...
void __fastcall TMainWindow::LoadOpt(void) {
  ...
  AnsiString opt, mask = "11111111111111111111111111111111111111111111111111111111111111111111";
