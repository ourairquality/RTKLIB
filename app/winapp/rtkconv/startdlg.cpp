...
//---------------------------------------------------------------------------
__fastcall TStartDialog::TStartDialog(TComponent *Owner) : TForm(Owner) {
  Time.time = 0;
  Time.sec = 0.0;
}
//---------------------------------------------------------------------------
void __fastcall TStartDialog::FormShow(TObject *Sender) {
  ...
  paths[0] = path;
  if (expath(file.c_str(), paths, sizeof(path), 1)) {
...
//---------------------------------------------------------------------------
void __fastcall TStartDialog::TimeY1UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
...
//---------------------------------------------------------------------------
void __fastcall TStartDialog::TimeH1UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
                                                 TUpDownDirection Direction) {
...
void __fastcall TStartDialog::BtnFileTimeClick(TObject *Sender) {
  ...
  if ((h = CreateFile(FileName, GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0)) ==
