...
//---------------------------------------------------------------------------
__fastcall TMonitorDialog::TMonitorDialog(TComponent *Owner) : TForm(Owner) {
  int i;
...
//---------------------------------------------------------------------------
void __fastcall TMonitorDialog::BtnCloseClick(TObject *Sender) { Close(); }
//---------------------------------------------------------------------------
...
void __fastcall TMonitorDialog::ShowRtk(void) {
  ...
  AnsiString mode[] = {"Single",      "DGPS",  "Kinematic",     "Static",     "Static-Start",
  ...
  char file[1024] = "";
  const char *ionoopt[] = {"OFF",           "Broadcast", "SBAS",     "Dual-Frequency",
                           "Estimate STEC", "IONEX TEC", "QZSS LEX", ""};
  const char *tropopt[] = {"OFF", "Saastamoinen", "SBAS", "Estimate ZTD", "Estimate ZTD+Grad", ""};
  const char *ephopt[] = {
      "Broadcast", "Precise", "Broadcast+SBAS", "Broadcat+SSR APC", "Broadcast+SSR CoM",
      "QZSS LEX",  ""};
  ...
  Tbl->Cells[1][i++] = !rtk.opt.snrmask.ena[0]
                           ? s.sprintf("")
                           : s.sprintf("%.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f",
                                       rtk.opt.snrmask.mask[0][0], rtk.opt.snrmask.mask[0][1],
                                       rtk.opt.snrmask.mask[0][2], rtk.opt.snrmask.mask[0][3],
                                       rtk.opt.snrmask.mask[0][4], rtk.opt.snrmask.mask[0][5],
                                       rtk.opt.snrmask.mask[0][6], rtk.opt.snrmask.mask[0][7],
                                       rtk.opt.snrmask.mask[0][8]);

  Tbl->Cells[0][i] = "SNR Mask L2 (dBHz)";
  Tbl->Cells[1][i++] = !rtk.opt.snrmask.ena[0]
                           ? s.sprintf("")
                           : s.sprintf("%.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f",
                                       rtk.opt.snrmask.mask[1][0], rtk.opt.snrmask.mask[1][1],
                                       rtk.opt.snrmask.mask[1][2], rtk.opt.snrmask.mask[1][3],
                                       rtk.opt.snrmask.mask[1][4], rtk.opt.snrmask.mask[1][5],
                                       rtk.opt.snrmask.mask[1][6], rtk.opt.snrmask.mask[1][7],
                                       rtk.opt.snrmask.mask[1][8]);

  Tbl->Cells[0][i] = "SNR Mask L5 (dBHz)";
  Tbl->Cells[1][i++] = !rtk.opt.snrmask.ena[0]
                           ? s.sprintf("")
                           : s.sprintf("%.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f, %.0f",
                                       rtk.opt.snrmask.mask[2][0], rtk.opt.snrmask.mask[2][1],
                                       rtk.opt.snrmask.mask[2][2], rtk.opt.snrmask.mask[2][3],
                                       rtk.opt.snrmask.mask[2][4], rtk.opt.snrmask.mask[2][5],
                                       rtk.opt.snrmask.mask[2][6], rtk.opt.snrmask.mask[2][7],
                                       rtk.opt.snrmask.mask[2][8]);

  Tbl->Cells[0][i] = "Rec Dynamic/Earth Tides Correction";
  Tbl->Cells[1][i++] =
      s.sprintf("%s, %s", rtk.opt.dynamics ? "ON" : "OFF", rtk.opt.tidecorr ? "ON" : "OFF");
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("Obs(%d), Nav(%d), Ion(%d), Sbs(%d), Pos(%d), Dgps(%d), Ssr(%d), Err(%d)",
                nmsg[0][0], nmsg[0][1] + nmsg[0][6], nmsg[0][2], nmsg[0][3], nmsg[0][4], nmsg[0][5],
                nmsg[0][7], nmsg[0][9]);

  Tbl->Cells[0][i] = "# of Input Data Base Station";
  Tbl->Cells[1][i++] =
      s.sprintf("Obs(%d), Nav(%d), Ion(%d), Sbs(%d), Pos(%d), Dgps(%d), Ssr(%d), Err(%d)",
                nmsg[1][0], nmsg[1][1] + nmsg[1][6], nmsg[1][2], nmsg[1][3], nmsg[1][4], nmsg[1][5],
                nmsg[1][7], nmsg[1][9]);

  Tbl->Cells[0][i] = "# of Input Data Ephemeris";
  Tbl->Cells[1][i++] =
      s.sprintf("Obs(%d), Nav(%d), Ion(%d), Sbs(%d), Pos(%d), Dgps(%d), Ssr(%d), Err(%d)",
                nmsg[2][0], nmsg[2][1] + nmsg[2][6], nmsg[2][2], nmsg[2][3], nmsg[2][4], nmsg[2][5],
                nmsg[2][7], nmsg[2][9]);
  ...
  Tbl->Cells[1][i++] = s.sprintf("%.3f, %.3f, %.3f, %.3f", rtk.sol.dtr[1] * 1E9,
  ...
  Tbl->Cells[1][i++] = s.sprintf("%.3f, %.3f, %.3f", rtk.x ? rtk.x[0] : 0, rtk.x ? rtk.x[1] : 0,
                                 rtk.x ? rtk.x[2] : 0);

  Tbl->Cells[0][i] = "Pos X/Y/Z Float Std (m) Rover";
  Tbl->Cells[1][i++] =
      s.sprintf("%.3f, %.3f, %.3f", rtk.P ? SQRT(rtk.P[0]) : 0,
                rtk.P ? SQRT(rtk.P[1 + 1 * rtk.nx]) : 0, rtk.P ? SQRT(rtk.P[2 + 2 * rtk.nx]) : 0);

  Tbl->Cells[0][i] = "Pos X/Y/Z Fixed (m) Rover";
  Tbl->Cells[1][i++] = s.sprintf("%.3f, %.3f, %.3f", rtk.xa ? rtk.xa[0] : 0, rtk.xa ? rtk.xa[1] : 0,
                                 rtk.xa ? rtk.xa[2] : 0);

  Tbl->Cells[0][i] = "Pos X/Y/Z Fixed Std (m) Rover";
  Tbl->Cells[1][i++] = s.sprintf("%.3f, %.3f, %.3f", rtk.Pa ? SQRT(rtk.Pa[0]) : 0,
...
void __fastcall TMonitorDialog::SetSat(void) {
  int i, j = 0;
  AnsiString s, label[] = {"SAT",      "Status",   "Azimuth (deg)", "Elevation (deg)", "LG (m)",
                           "PHW(cyc)", "P1-P2(m)", "P1-C1(m)",      "P2-C2(m)"};
...
void __fastcall TMonitorDialog::SetEst(void) {
  AnsiString label[] = {"State", "Estimate Float", "Std Float", "Estimate Fixed", "Std Fixed"};
...
void __fastcall TMonitorDialog::ShowEst(void) {
  ...
  for (i = 0, n = 1; i < nx; i++) {
    ...
    Tbl->Cells[j++][n] =
        i >= na || Pa[i + i * na] == 0.0 ? s0 : s.sprintf("%.3f", SQRT(Pa[i + i * na]));
    n++;
  }
  free(x);
  free(P);
...
void __fastcall TMonitorDialog::ShowCov(void) {
  ...
  for (i = 0, n = 1; i < nx; i++) {
    ...
    for (j = 0, m = 1; j < nx; j++) {
      ...
      Tbl->Cells[m][n] = P[i + j * nx] == 0.0 ? s0 : s.sprintf("%.5f", SQRT(P[i + j * nx]));
      m++;
    }
    n++;
  }
...
void __fastcall TMonitorDialog::SetObs(void) {
  ...
  for (i = 0; i < NFREQ + nex; i++) {
    ...
    Tbl->Cells[j][0] =
        i < NFREQ ? s.sprintf("P%d (m)", i + 1) : s.sprintf("PX%d (m)", i - NFREQ + 1);
    Tbl->Cells[j++][1] = "";
  }
  for (i = 0; i < NFREQ + nex; i++) {
    Tbl->ColWidths[j] = 85 * FontScale / 96;
    Tbl->Cells[j][0] =
        i < NFREQ ? s.sprintf("L%d (cycle)", i + 1) : s.sprintf("LX%d (cycle)", i - NFREQ + 1);
    Tbl->Cells[j++][1] = "";
  }
  for (i = 0; i < NFREQ + nex; i++) {
    Tbl->ColWidths[j] = 60 * FontScale / 96;
    Tbl->Cells[j][0] =
        i < NFREQ ? s.sprintf("D%d (Hz)", i + 1) : s.sprintf("DX%d (Hz)", i - NFREQ + 1);
    Tbl->Cells[j++][1] = "";
  }
  for (i = 0; i < NFREQ + nex; i++) {
...
void __fastcall TMonitorDialog::ShowObs(void) {
  ...
  for (i = 0; i < n; i++) {
    ...
    for (k = 0; k < NFREQ + nex; k++) {
      ...
      if (*code)
        Tbl->Cells[j++][i + 1] = s.sprintf("%s", code);
      else
        Tbl->Cells[j++][i + 1] = "-";
    }
    for (k = 0; k < NFREQ + nex; k++) {
      if (obs[i].SNR[k])
        Tbl->Cells[j++][i + 1] = s.sprintf("%.1f", obs[i].SNR[k] * SNR_UNIT);
      else
        Tbl->Cells[j++][i + 1] = s.sprintf("-");
    }
    for (k = 0; k < NFREQ + nex; k++) {
...
void __fastcall TMonitorDialog::SetNav(void) {
  ...
  int i, width[] = {25, 25, 30, 30, 30, 25, 25, 115, 115, 115, 80, 80, 60, 60, 60, 60,
                    70, 70, 70, 70, 50, 50, 50, 50,  70,  70,  50, 50, 70, 70, 30, 30};
  Tbl->ColCount = 32;
  Tbl->RowCount = 2;
  for (i = 0; i < Tbl->ColCount; i++) {
    Tbl->ColWidths[i] = width[i] * FontScale / 96;
...
void __fastcall TMonitorDialog::ShowNav(void) {
  ...
  for (k = 0, n = 1; k < MAXSAT; k++) {
    ...
    if (eph[k].iode < 0)
      ...
      s.sprintf("%d", eph[k].iode);
    Tbl->Cells[j++][n] = s;
    if (eph[k].iodc < 0)
      s = "-";
    else
      s.sprintf("%d", eph[k].iodc);
    Tbl->Cells[j++][n] = s;
...
void __fastcall TMonitorDialog::ShowIonUtc(void) {
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.0f, %.0f, %.0f, %.0f", utc_gps[4], utc_gps[5], utc_gps[6], utc_gps[7]);

  Tbl->Cells[0][i] = "GPST-UTC Ref Week, Time(s), A0(ns), A1(ns/s)";
  Tbl->Cells[1][i++] = s.sprintf("%.0f, %.0f, %.3f, %.5E", utc_gps[3], utc_gps[2], utc_gps[0] * 1E9,
                                 utc_gps[1] * 1E9);
  ...
  Tbl->Cells[1][i++] = s.sprintf("%.0f, %.0f, %.3f, %.5E", utc_gal[3], utc_gal[2], utc_gal[0] * 1E9,
                                 utc_gal[1] * 1E9);

  Tbl->Cells[0][i] = "QZSST-UTC Ref Week, Time(s), A0(ns), A1(ns/s)";
  Tbl->Cells[1][i++] = s.sprintf("%.0f, %.0f, %.3f, %.5E", utc_qzs[3], utc_qzs[2], utc_qzs[0] * 1E9,
                                 utc_qzs[1] * 1E9);

  Tbl->Cells[0][i] = "BDST-UTC Ref Week, Time(s), A0(ns), A1(ns/s)";
  Tbl->Cells[1][i++] = s.sprintf("%.0f, %.0f, %.3f, %.5E", utc_cmp[3], utc_cmp[2], utc_cmp[0] * 1E9,
                                 utc_cmp[1] * 1E9);

  Tbl->Cells[0][i] = "IRNT-UTC Ref Week,Time(s), A0(ns), A1(ns/s), A2(ns/s2)";
  Tbl->Cells[1][i++] = s.sprintf("%.0f, %.0f, %.3f, %.5E, %.5E", utc_irn[3], utc_irn[2],
                                 utc_irn[0] * 1E9, utc_irn[1] * 1E9, utc_irn[8] * 1E9);

  Tbl->Cells[0][i] = "GPS Iono Parameters Alpha0-3";
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_gps[0], ion_gps[1], ion_gps[2], ion_gps[3]);

  Tbl->Cells[0][i] = "GPS Iono Parameters Beta0-3";
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_gps[4], ion_gps[5], ion_gps[6], ion_gps[7]);
  ...
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_qzs[0], ion_qzs[1], ion_qzs[2], ion_qzs[3]);

  Tbl->Cells[0][i] = "QZSS Iono Parameters Beta0-3";
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_qzs[4], ion_qzs[5], ion_qzs[6], ion_qzs[7]);

  Tbl->Cells[0][i] = "BDS Iono Parameters Alpha0-3";
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_cmp[0], ion_cmp[1], ion_cmp[2], ion_cmp[3]);

  Tbl->Cells[0][i] = "BDS Iono Parameters Beta0-3";
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_cmp[4], ion_cmp[5], ion_cmp[6], ion_cmp[7]);

  Tbl->Cells[0][i] = "NavIC Iono Parameters Alpha0-3";
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_irn[0], ion_irn[1], ion_irn[2], ion_irn[3]);

  Tbl->Cells[0][i] = "NavIC Iono Parameters Beta0-3";
  Tbl->Cells[1][i++] =
      s.sprintf("%.5E, %.5E, %.5E, %.5E", ion_irn[4], ion_irn[5], ion_irn[6], ion_irn[7]);
}
//---------------------------------------------------------------------------
void __fastcall TMonitorDialog::SetStr(void) {
  AnsiString label[] = {"STR",           "Stream",      "Type",         "Format",
                        "Mode",          "State",       "Input(bytes)", "Input(bps)",
...
void __fastcall TMonitorDialog::ShowStr(void) {
  AnsiString ch[] = {"Input Rover",       "Input Base",        "Input Correction",
                     "Output Solution 1", "Output Solution 2", "Log Rover",
                     "Log Base",          "Log Correction",    "Monitor"};
  AnsiString type[] = {"-",
                       "Serial",
                       "File",
                       "TCP Server",
                       ...
                       "UDP Client",
                       ""};
  AnsiString outformat[] = {"Lat/Lon/Height", "X/Y/Z-ECEF", "E/N/U-Baseline", "NMEA-0183"};
...
void __fastcall TMonitorDialog::SetSbsMsg(void) {
  AnsiString label[] = {"Trcv", "PRN", "STR", "Type", "Message", "Contents"};
...
void __fastcall TMonitorDialog::ShowSbsMsg(void) {
  ...
  const char *content[] = {"For Testing",
                           "PRN Mask",
                           "Fast Corrections",
                           "Fast Corrections",
                           "Fast Corrections",
                           ...
                           "WAAS Service Messages",
                           "Clock-Ephemeris Covariance Matrix Message",
                           "Internal Test Message",
                           "Null Message",
                           "QZSS: DC Report (JMA)",
                           "QZSS: DC Report (Other)",
                           "QZSS: Monitoring Station Info",
                           "QZSS: PRN Mask",
                           "QZSS: Data Issue Number",
                           "QZSS: DGPS Correction",
...
void __fastcall TMonitorDialog::SetSbsLong(void) {
  AnsiString label[] = {"SAT",       "Status",    "IODE",      "dX (m)",    "dY (m)",      "dZ (m)",
                        "dVX (m/s)", "dVY (m/s)", "dVZ (m/s)", "daf0 (ns)", "daf1 (ns/s)", "T0"};
...
void __fastcall TMonitorDialog::SetSbsIono(void) {
  AnsiString label[] = {"IODI", "Lat (deg)", "Lon (deg)", "GIVEI", "Delay (m)", "T0"};
...
void __fastcall TMonitorDialog::SetSbsFast(void) {
  AnsiString label[] = {"SAT", "Status", "PRC (m)", "RRC (m)", "IODF", "UDREI", "AI", "Tof"};
...
void __fastcall TMonitorDialog::SetRtcmDgps(void) {
  AnsiString label[] = {"SAT", "Status", "PRC (m)", "RRC (m)", "IOD", "UDRE", "T0"};
...
void __fastcall TMonitorDialog::SetRtcmSsr(void) {
  AnsiString s, label[] = {"SAT",      "Status",     "UDI(s)",     "UDHR(s)",      "IOD",
                           "URA",      "Datum",      "T0",         "D0-A(m)",      "D0-C(m)",
                           "D0-R(m)",  "D1-A(mm/s)", "D1-C(mm/s)", "D1-R(mm/s)",   "C0(m)",
                           "C1(mm/s)", "C2(mm/s2)",  "C-HR(m)",    "Code Bias(m)", "Phase Bias(m)"};
  int i,
      width[] = {25, 30, 30, 30, 30, 25, 15, 115, 50, 50, 50, 50, 50, 50, 50, 50, 50, 50, 180, 180};
  char *code;
...
void __fastcall TMonitorDialog::ShowRefSta(void) {
  ...
  Tbl->Cells[1][i++] = s.sprintf("%.2f, %.2f, %.2f, %.2f", sta.glo_cp_bias[0], sta.glo_cp_bias[1],
                                 sta.glo_cp_bias[2], sta.glo_cp_bias[3]);
}
//---------------------------------------------------------------------------
