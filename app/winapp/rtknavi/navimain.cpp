...
void __fastcall TMainForm::BtnInputFilesClick(TObject *Sender)
{
    ...
    if (rtksvr.state) {
      rtksvr.ninfiles = 0;
      for (unsigned i = 0; i < MAXINFILES; i++) rtksvr.infiles[i][0] = '\0';
      for (unsigned i = 0; i < MAXINFILES; i++) {
        if (InputFiles[i] == "") continue;
        snprintf(rtksvr.infiles[rtksvr.ninfiles], sizeof(rtksvr.infiles[0]), "%s", InputFiles[i].c_str());
        rtksvr.ninfiles++;
      }
    }
    rtksvrunlock(&rtksvr);
