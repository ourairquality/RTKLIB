...
int __fastcall TMainForm::ExecCmd(AnsiString cmd, int show) {
  ...
  si.cb = sizeof(si);
  char *p = cmd.c_str();
  if (!CreateProcess(NULL, p, NULL, NULL, false, show ? 0 : CREATE_NO_WINDOW, NULL, NULL, &si,
                     &info))
    return 0;
...
void __fastcall TMainForm::BtnPlotClick(TObject *Sender) {
  ...
  cmd.sprintf("rtkplot -p tcpcli://localhost:%d -t \"%s %s\"", OpenPort, Ansi_Caption.c_str(),
              ": RTKPLOT");
  if (!ExecCmd(cmd, 1)) {
...
void __fastcall TMainForm::BtnOutputStrClick(TObject *Sender) {
  ...
  for (i = 3; i < 5; i++) {
    ...
    str = otype[Stream[i]];
    if (str == STR_SERIAL)
      path = Paths[i][0].c_str();
    else if (str == STR_FILE)
      path = Paths[i][2].c_str();
    else if (str == STR_FTP || str == STR_HTTP)
      path = Paths[i][3].c_str();
    else
      path = Paths[i][1].c_str();
...
void __fastcall TMainForm::SvrStart(void) {
  ...
  for (i = 0; i < 8; i++) {
    if (strs[i] == STR_NONE)
      ...
      paths[i] = Paths[i][0].c_str();
    else if (strs[i] == STR_FILE)
      paths[i] = Paths[i][2].c_str();
    else if (strs[i] == STR_FTP || strs[i] == STR_HTTP)
      paths[i] = Paths[i][3].c_str();
    else
      paths[i] = Paths[i][1].c_str();
  }
  for (i = 0; i < 3; i++) {
  ...
  if (!rtksvrstart(&rtksvr, SvrCycle, SvrBuffSize, strs, (const char **paths), Format, NavSelect,
                   (const char **paths)cmds, (const char **paths)cmds_periodic,
                   (const char **paths)rcvopts, NmeaCycle, NmeaReq, nmeapos, &PrcOpt, solopt,
                   &monistr, errmsg, sizeof(errmsg))) {
...
void __fastcall TMainForm::UpdateSolType(void) {
  AnsiString label[] = {"Lat/Lon/Height",
                        "Lat/Lon/Height",
                        "X/Y/Z-ECEF",
                        "E/N/U-Baseline",
...
// update log ---------------------------------------------------------------
void __fastcall TMainForm::UpdateLog(int stat, gtime_t time, double *rr, float *qr, double *rb,
                                     int ns, double age, double ratio) {
  int i, ena;
...
void __fastcall TMainForm::UpdateFont(void) {
  TLabel *label[] = {PlabelA, Plabel1, Plabel2,  Plabel3,  Pos1,
  ...
  label[0]->Font->Size = 9;
  label[7]->Font->Color = color;
  label[8]->Font->Size = 8;
  label[8]->Font->Color = clGray;
  label[9]->Font->Size = 8;
  label[9]->Font->Color = clGray;
}
// update time --------------------------------------------------------------
void __fastcall TMainForm::UpdateTime(void) {
  ...
  if (TimeSys == 0)
    ...
    if (!(t = localtime(&time.time)))
      strcpy(tstr, "2000/01/01 00:00:00.0");
    else
      snprintf(tstr, sizeof(tstr), "%04d/%02d/%02d %02d:%02d:%02d.%d", t->tm_year + 1900,
               t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec, (int)(time.sec * 10));
  } else if (TimeSys == 3) {
    tow = time2gpst(time, &week);
    sprintf(tstr, "week %04d %8.1f s", week, tow);
  }
...
void __fastcall TMainForm::UpdatePos(void) {
  ...
  SolQ->Caption = ext + L" " + label[0]->Caption + L" " + label[3]->Caption + L" " +
                  label[1]->Caption + L" " + label[4]->Caption + L" " + label[2]->Caption + L" " +
                  label[5]->Caption + s[8];
}
...
// draw snr plot ------------------------------------------------------------
void __fastcall TMainForm::DrawSnr(TCanvas *c, int w, int h, int x0, int y0, int index, int freq) {
  static const TColor color[] = {(TColor)0x00008000, (TColor)0x00008080, (TColor)0x00A000A0,
                                 (TColor)0x00800000, (TColor)0x00000080, (TColor)0x00808000,
                                 (TColor)0x00808080};
  static const TColor color_sys[] = {clGreen, (TColor)0xAAFF, clFuchsia, clBlue,
                                     clRed,   clTeal,         clGray};
  UTF8String s;
  ...
  for (i = j = 0; i < 7; i++) {
    if (!mask[i]) continue;
    sprintf(id, "%c", sys[i]);
    DrawText(c, x0 + w - tm * 3 / 2 + PanelFont->Size * 9 / 8 * (-n + j++), y0 + tm, (s = id),
             color[i], 0, 2);
  }
}
// draw satellites in skyplot -----------------------------------------------
void __fastcall TMainForm::DrawSat(TCanvas *c, int w, int h, int x0, int y0, int index, int freq) {
  static const TColor color_sys[] = {clGreen, (TColor)0xAAFF, clFuchsia, clBlue,
                                     clRed,   clTeal,         clGray};
  TColor color_text;
  UTF8String s;
  ...
  for (i = 0, k = Nsat[index] - 1; i < Nsat[index] && i < MAXSAT; i++, k--) {
    ...
    c->Brush->Color =
        !Vsat[index][k] ? clSilver : (freq < NFREQ + 1 ? SnrColor(snr[freq]) : color_sys[l]);
    c->Brush->Style = bsSolid;
    c->Pen->Color = clGray;
    color_text = clWhite;
...
void __fastcall TMainForm::DrawBL(TImage *plot, int w, int h) {
  ...
  if (plot->Name == "Plot1")
    mode = BLMode1;
  else if (plot->Name == "Plot2")
    mode = BLMode2;
  else if (plot->Name == "Plot3")
    mode = BLMode3;
  else
    mode = BLMode4;
...
void __fastcall TMainForm::DrawTrk(TImage *plot) {
  ...
  p2.y -= 2;
  if (xt < 0.01)
    label.sprintf("%.0f mm", xt * 1000.0);
  else if (xt < 1.0)
    label.sprintf("%.0f cm", xt * 100.0);
  else if (xt < 1000.0)
    label.sprintf("%.0f m", xt);
  else
    label.sprintf("%.0f km", xt / 1000.0);
...
// draw text ----------------------------------------------------------------
void __fastcall TMainForm::DrawText(TCanvas *c, int x, int y, UTF8String s, TColor color, int ha,
                                    int va) {
...
// draw arrow ---------------------------------------------------------------
void __fastcall TMainForm::DrawArrow(TCanvas *c, int x, int y, int siz, int ang, TColor color) {
  TPoint p1[4], p2[4];
  int i;
...
void __fastcall TMainForm::SaveLog(void) {
  ...
  sprintf(file, "rtk_%04.0f%02.0f%02.0f%02.0f%02.0f%02.0f.txt", ep[0], ep[1], ep[2], ep[3], ep[4],
          ep[5]);
...
void __fastcall TMainForm::LoadNav(nav_t *nav) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    for (j = 0; j < 2; j++) {
      if ((str = ini->ReadString("navi", s.sprintf("eph_%02d_%d", i, j), "")) == "") continue;
      nav->eph[i][j] = eph0;
  ...
  sscanf(str.c_str(), "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf", nav->ion_gps, nav->ion_gps + 1,
         nav->ion_gps + 2, nav->ion_gps + 3, nav->ion_gps + 4, nav->ion_gps + 5, nav->ion_gps + 6,
         nav->ion_gps + 7);

  str = ini->ReadString("navi", "utc", "");
  for (i = 0; i < 8; i++) nav->utc_gps[i] = 0.0;
  sscanf(str.c_str(), "%lf,%lf,%lf,%lf,%lf,%lf,%lf,%lf", nav->utc_gps, nav->utc_gps + 1,
         nav->utc_gps + 2, nav->utc_gps + 3, nav->utc_gps + 4, nav->utc_gps + 5, nav->utc_gps + 6,
         nav->utc_gps + 7);
...
void __fastcall TMainForm::SaveNav(nav_t *nav) {
  ...
  for (i = 0; i < MAXSAT * 2; i++) {
    for (j = 0; j < nav->n[i]; j++) {
      ...
      str = str + s.sprintf("%d,", nav->eph[i][j].iode);
      str = str + s.sprintf("%d,", nav->eph[i][j].iodc);
      str = str + s.sprintf("%d,", nav->eph[i][j].sva);
      str = str + s.sprintf("%d,", nav->eph[i][j].svh);
      str = str + s.sprintf("%d,", (int)nav->eph[i][j].toe.time);
      str = str + s.sprintf("%d,", (int)nav->eph[i][j].toc.time);
      str = str + s.sprintf("%d,", (int)nav->eph[i][j].ttr.time);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].A);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].e);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].i0);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].OMG0);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].omg);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].M0);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].deln);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].OMGd);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].idot);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].crc);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].crs);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].cuc);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].cus);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].cic);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].cis);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].toes);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].fit);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].f0);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].f1);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].f2);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].tgd[0]);
      str = str + s.sprintf("%d,", nav->eph[i][j].code);
      str = str + s.sprintf("%d,", nav->eph[i][j].flag);
      str = str + s.sprintf("%.14E,", nav->eph[i][j].tgd[1]);
      ini->WriteString("navi", s.sprintf("eph_%02d_%d", i, j), str);
    }
  }
  str = "";
