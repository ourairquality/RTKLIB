...
#define FMTOPT                                                                                    \
  "0:rtcm2,1:rtcm3,2:oem4,4:ubx,5:swift,6:hemis,7:skytraq,8:javad,9:nvs,10:binex,11:rt17,12:sbf," \
  "15:sp3"
...
void __fastcall TOptDialog::LoadOpt(AnsiString file) {
  ...
  resetsysopts();
  if (!loadopts(file.c_str(), sysopts) || !loadopts(file.c_str(), rcvopts)) return;
...
void __fastcall TOptDialog::SaveOpt(AnsiString file) {
  ...
  if (!saveopts(file.c_str(), "w", comment, sysopts) || !saveopts(file.c_str(), "a", "", rcvopts))
    return;
}
//---------------------------------------------------------------------------
...
void __fastcall TOptDialog::ObsWeightChange(TObject *Sender) { UpdateEnable(); }
//---------------------------------------------------------------------------
