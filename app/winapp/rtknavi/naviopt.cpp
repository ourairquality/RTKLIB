	...
	: TForm(Owner)
{
	...
	int nbds=MAXPRNBDS,nirn=MAXPRNIRN;
	...
	if (nbds<=0) { NavSys6->Enabled=false; NavSys7->Enabled=false; }
...
void __fastcall TOptDialog::LoadOpt(AnsiString file)
{
	...
	for (int i=0;i<MAXSTRRTK;i++) {
		...
		if (i<3 || i>=6) MainForm->Format[i]=strfmt[i];
