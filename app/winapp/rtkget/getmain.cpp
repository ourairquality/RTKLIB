...
//---------------------------------------------------------------------------
void __fastcall TMainForm::FormShow(TObject *Sender) { ::DragAcceptFiles(Handle, true); }
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::Panel3Resize(TObject *Sender) {
  TButton *btn[] = {BtnFile, BtnLog, BtnOpts, BtnTest, BtnDownload, BtnExit};
...
void __fastcall TMainForm::BtnFileClick(TObject *Sender) {
  ...
  GetTime(&ts, &te, &ti);
  if (str != "") reppath(str.c_str(), path, sizeof(path), ts, "", "");
...
void __fastcall TMainForm::BtnDownloadClick(TObject *Sender) {
  ...
  if (LogFile != "") {
    reppath(LogFile.c_str(), path, sizeof(path), utc2gpst(timeget()), "", "");
  ...
  dl_exec(ts, te, ti, seqnos, seqnoe, urls, nurl, (const char **)stas, nsta, dir, usr.c_str(),
          pwd.c_str(), proxy.c_str(), opts, msg, sizeof(msg), fp);
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::DirChange(TObject *Sender) { UpdateMsg(); }
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::DropFiles(TWMDropFiles msg) {
  ...
  if (StaList->Left <= x && x < StaList->Left + StaList->Width && StaList->Top <= y &&
      y < StaList->Top + StaList->Height) {
    LoadSta(file = str);
  }
}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::HidePasswdClick(TObject *Sender) { UpdateEnable(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::DataTypeChange(TObject *Sender) { UpdateType(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::StaListClick(TObject *Sender) { UpdateStaList(); }
//---------------------------------------------------------------------------
...
void __fastcall TMainForm::GetTime(gtime_t *ts, gtime_t *te, double *ti) {
  ...
  str = TimeY1->Text;
  sscanf(str.c_str(), "%lf/%lf/%lf", eps, eps + 1, eps + 2);
  str = TimeH1->Text;
  sscanf(str.c_str(), "%lf:%lf", eps + 3, eps + 4);
  str = TimeY2->Text;
  sscanf(str.c_str(), "%lf/%lf/%lf", epe, epe + 1, epe + 2);
  str = TimeH2->Text;
  sscanf(str.c_str(), "%lf:%lf", epe + 3, epe + 4);
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::TimeIntChange(TObject *Sender) { UpdateEnable(); }
//---------------------------------------------------------------------------
