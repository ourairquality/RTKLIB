...
//---------------------------------------------------------------------------
__fastcall TRefDialog::TRefDialog(TComponent *Owner) : TForm(Owner) {
  Opt = 0;
...
//---------------------------------------------------------------------------
void __fastcall TRefDialog::StaListMouseDown(TObject *Sender, TMouseButton Button,
...
//---------------------------------------------------------------------------
void __fastcall TRefDialog::BtnFindClick(TObject *Sender) { FindList(); }
//---------------------------------------------------------------------------
...
void __fastcall TRefDialog::FindList(void) {
  ...
  for (;;) {
    if (StaList->Cells[4][j].Pos(str) > 0 || StaList->Cells[5][j].Pos(str) > 0) {
      r.Top = j;
      r.Bottom = j;
      r.Left = 0;
      r.Right = 6;
      ...
      if (j < StaList->TopRow + 1 || j >= StaList->TopRow + nmax) {
        if (j < nmax - 1)
          ...
          StaList->TopRow = n - nmax;
        else
          StaList->TopRow = j - nmax / 2;
      }
      break;
    }
    if (i == j) break;
...
void __fastcall TRefDialog::LoadList(void) {
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    code[0] = '\0';
    name[0] = '\0';
    if (sscanf(buff, "%lf %lf %lf %255s %255s", pos, pos + 1, pos + 2, code, name) < 3) continue;
...
//---------------------------------------------------------------------------
void __fastcall TRefDialog::AddRef(int n, double *pos, const char *code, const char *name) {
  AnsiString s;
