...
int TGraph::ToPoint(double x, double y, TPoint &p) {
  ...
  if (x < -1E6)
    x = -1E6;  // clip to avoid numerical exception
  else if (x > 1E6)
    x = 1E6;
  if (y < -1E6)
    y = -1E6;
...
double TGraph::AutoTick(double scale) {
  ...
  for (int i = 0; i < 4; i++)
    if (tick <= t[i] * order) return t[i] * order;
  return 10.0;
}
//---------------------------------------------------------------------------
double TGraph::AutoTickTime(double scale) {
  double t[] = {0.1,         0.2,         0.5,          1.0,          3.0,         6.0,
                12.0,        30.0,        60.0,         300.0,        900.0,       1800.0,
                3600.0,      7200.0,      10800.0,      21600.0,      43200.0,     86400.0,
                86400.0 * 2, 86400.0 * 7, 86400.0 * 14, 86400.0 * 35, 86400.0 * 70};
  double tick = 60.0 * scale;
  for (int i = 0; i < (int)(sizeof(t) / sizeof(*t)); i++)
    if (tick <= t[i]) return t[i];
  return 86400.0 * 140;
}
//---------------------------------------------------------------------------
...
void TGraph::DrawMark(TPoint p, int mark, TColor color, int size, int rot) {
  ...
  switch (mark) {
    ...
    case 13:  // compass
      n = 7;
      for (int i = 0; i < n; i++) {
        ps[i].x = xs3[i] * size / 40;
        ps[i].y = ys3[i] * size / 40;
      }
...
//---------------------------------------------------------------------------
void TGraph::DrawMark(double x, double y, int mark, TColor color, int size, int rot) {
  TPoint p;
  if (ToPoint(x, y, p)) DrawMark(p, mark, color, size, rot);
}
//---------------------------------------------------------------------------
void TGraph::DrawMark(TPoint p, int mark, TColor color, TColor bgcolor, int size, int rot) {
  TPoint p1;
  p1 = p;
  p1.x--;
  DrawMark(p1, mark, bgcolor, size, rot);  // draw with hemming
  p1 = p;
  p1.x++;
  DrawMark(p1, mark, bgcolor, size, rot);
  p1 = p;
  p1.y--;
  DrawMark(p1, mark, bgcolor, size, rot);
  p1 = p;
...
//---------------------------------------------------------------------------
void TGraph::DrawMark(double x, double y, int mark, TColor color, TColor bgcolor, int size,
                      int rot) {
  TPoint p;
  if (ToPoint(x, y, p)) DrawMark(p, mark, color, bgcolor, size, rot);
}
//---------------------------------------------------------------------------
void TGraph::DrawMarks(const double *x, const double *y, const TColor *color, int n, int mark,
                       int size, int rot) {
  TPoint p, pp;
  for (int i = 0; i < n; i++) {
...
//---------------------------------------------------------------------------
void TGraph::DrawText(TPoint p, UTF8String str, TColor color, int ha, int va, int rot,
                      AnsiString font) {
  // str = UTF-8 string
...
//---------------------------------------------------------------------------
void TGraph::DrawText(double x, double y, UTF8String str, TColor color, int ha, int va, int rot,
                      AnsiString font) {
  TPoint p;
  ToPoint(x, y, p);
  DrawText(p, str, color, ha, va, rot, font);
...
//---------------------------------------------------------------------------
void TGraph::DrawText(TPoint p, UTF8String str, TColor color, TColor bgcolor, int ha, int va,
                      int rot) {
  TPoint p1;
  p1 = p;
  p1.x--;
  DrawText(p1, str, bgcolor, ha, va, rot);  // draw with hemming
  p1 = p;
  p1.x++;
  DrawText(p1, str, bgcolor, ha, va, rot);
  p1 = p;
  p1.y--;
  DrawText(p1, str, bgcolor, ha, va, rot);
  p1 = p;
...
//---------------------------------------------------------------------------
void TGraph::DrawText(double x, double y, UTF8String str, TColor color, TColor bgcolor, int ha,
                      int va, int rot) {
  TPoint p;
  ToPoint(x, y, p);
...
//---------------------------------------------------------------------------
void TGraph::DrawCircle(double x, double y, TColor color, double rx, double ry, int style) {
  TPoint p;
  ToPoint(x, y, p);
...
//---------------------------------------------------------------------------
void TGraph::DrawPatch(TPoint *p, int n, TColor color1, TColor color2, int style) {
  TCanvas *c = Canvas;
  TPenStyle ps[] = {psSolid, psDot, psDash, psDashDot, psDashDotDot};
...
//---------------------------------------------------------------------------
void TGraph::DrawPatch(double *x, double *y, int n, TColor color1, TColor color2, int style) {
  TPoint *p = new TPoint[n];
  for (int i = 0; i < n; i++) {
    ToPoint(x[i], y[i], p[i]);
  }
...
//---------------------------------------------------------------------------
void TGraph::DrawSkyPlot(double x, double y, TColor color1, TColor color2, double size) {
  TPoint p;
  ToPoint(x, y, p);
  DrawSkyPlot(p, color1, color2, size / XScale);
}
//---------------------------------------------------------------------------
void TGraph::DrawSkyPlot(TPoint p, TColor color1, TColor color2, TColor bgcolor, int size) {
  TCanvas *c = Canvas;
  c->Pen->Color = color1;
  c->Brush->Style = bsClear;
  AnsiString s, dir[] = {"N", "E", "S", "W"};
...
//---------------------------------------------------------------------------
void TGraph::DrawSkyPlot(double x, double y, TColor color1, TColor color2, TColor bgcolor,
                         double size) {
  TPoint p;
  ToPoint(x, y, p);
