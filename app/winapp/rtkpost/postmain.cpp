...
__fastcall TMainForm::TMainForm(TComponent *Owner) : TForm(Owner) {
  ...
  RejectPhase = 30.0;
  RejectCode = 30.0;
  MeasErrR1 = MeasErrR2 = MeasErrR5 = 100.0;
  MeasErr2 = 0.004;
  MeasErr3 = 0.003;
  MeasErr4 = 1.0;
  SatClkStab = 1E-11;
...
void __fastcall TMainForm::FormShow(TObject *Sender) {
  ...
  for (int i = 1; i < argc; i++) {
    if (!strcmp(argv[i], "-t") && i + 1 < argc)
      ...
      InputFile2->Text = argv[++i];
    else if (!strcmp(argv[i], "-d") && i + 1 < argc) {
      OutDirEna->Checked = true;
      OutDir->Text = argv[++i];
    } else if (!strcmp(argv[i], "-o") && i + 1 < argc)
      OutputFile->Text = argv[++i];
...
void __fastcall TMainForm::FormClose(TObject *Sender, TCloseAction &Action) { SaveOpt(); }
...
void __fastcall TMainForm::BtnExecClick(TObject *Sender) {
  ...
  if ((p = strrchr(OutputFile_Text.c_str(), '.'))) {
    if (!strcmp(p, ".obs") || !strcmp(p, ".OBS") || !strcmp(p, ".nav") || !strcmp(p, ".NAV") ||
        !strcmp(p, ".gnav") || !strcmp(p, ".GNAV") || !strcmp(p, ".gz") || !strcmp(p, ".Z") ||
        !strcmp(p + 3, "o") || !strcmp(p + 3, "O") || !strcmp(p + 3, "d") || !strcmp(p + 3, "D") ||
        !strcmp(p + 3, "n") || !strcmp(p + 3, "N") || !strcmp(p + 3, "g") || !strcmp(p + 3, "G")) {
...
void __fastcall TMainForm::BtnExitClick(TObject *Sender) { Close(); }
...
void __fastcall TMainForm::BtnInputPlot1Click(TObject *Sender) {
  ...
  opts = " -r \"" + files[0] + "\" \"" + files[2] + "\" \"" + files[3] + "\" \"" + files[4] +
         "\" \"" + files[5] + "\"";
...
void __fastcall TMainForm::BtnInputPlot2Click(TObject *Sender) {
  ...
  opts = " -r \"" + files[1] + "\" \"" + files[2] + "\" \"" + files[3] + "\" \"" + files[4] +
         "\" \"" + files[5] + "\"";
...
void __fastcall TMainForm::TimeStartClick(TObject *Sender) { UpdateEnable(); }
// callback on time-interval check ------------------------------------------
void __fastcall TMainForm::TimeIntFClick(TObject *Sender) { UpdateEnable(); }
// callback on time-unit check ----------------------------------------------
void __fastcall TMainForm::TimeUnitFClick(TObject *Sender) { UpdateEnable(); }
// callback on time-ymd-1 updown --------------------------------------------
void __fastcall TMainForm::TimeY1UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
...
void __fastcall TMainForm::TimeH1UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
                                              TUpDownDirection Direction) {
...
void __fastcall TMainForm::TimeY2UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
                                              TUpDownDirection Direction) {
...
void __fastcall TMainForm::TimeH2UDChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
                                              TUpDownDirection Direction) {
...
void __fastcall TMainForm::InputFile1Change(TObject *Sender) { SetOutFile(); }
...
void __fastcall TMainForm::OutDirChange(TObject *Sender) { SetOutFile(); }
...
int __fastcall TMainForm::ExecProc(void) {
  ...
  if ((stat = postpos(ts, te, ti, tu, &prcopt, &solopt, &filopt, (const char **)infile, n,
                      (const char **)outfile, (const char *)rov, base)) == 1) {
    showmsg((char *)"aborted");
  }
  delete[] rov;
  delete[] base;
...
int __fastcall TMainForm::GetOption(prcopt_t &prcopt, solopt_t &solopt, filopt_t &filopt) {
...
AnsiString __fastcall TMainForm::FilePath(AnsiString file) {
  ...
  reppath(file.c_str(), path, sizeof(path), ts, rov, base);

  return (s = path);
}
// read history -------------------------------------------------------------
TStringList *__fastcall TMainForm::ReadList(TIniFile *ini, AnsiString cat, AnsiString key) {
...
// write history ------------------------------------------------------------
void __fastcall TMainForm::WriteList(TIniFile *ini, AnsiString cat, AnsiString key,
                                     TStrings *list) {
  AnsiString s;
  int i;
...
// show message in message area ---------------------------------------------
void __fastcall TMainForm::ShowMsg(char *msg) { Message->Caption = msg; }
...
void __fastcall TMainForm::LoadOpt(void) {
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 9; j++) {
      SnrMask.mask[i][j] = ini->ReadFloat("opt", s.sprintf("snrmask_%d_%d", i + 1, j + 1), 0.0);
    }
...
void __fastcall TMainForm::SaveOpt(void) {
  ...
  for (int i = 0; i < 3; i++)
    for (int j = 0; j < 9; j++) {
      ini->WriteFloat("opt", s.sprintf("snrmask_%d_%d", i + 1, j + 1), SnrMask.mask[i][j]);
    }
...
void __fastcall TMainForm::Panel4Resize(TObject *Sender) {
  TButton *btns[] = {BtnInputFile1, BtnInputFile2, BtnInputFile3,
                     BtnInputFile4, BtnInputFile5, BtnInputFile6};
  TComboBox *boxes[] = {InputFile1, InputFile2, InputFile3, InputFile4, InputFile5, InputFile6};
  int w = Panel4->Width;
...
void __fastcall TMainForm::Panel2Resize(TObject *Sender) {
  TBitBtn *btns[] = {BtnPlot, BtnView, BtnToKML, BtnOption, BtnExec, BtnExit};
