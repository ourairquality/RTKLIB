...
static char *getsrctbl(const char *path) {
  ...
  while (p < buff + MAXSRCTBL - 1) {
    ...
    *p = '\0';
    Sleep(NTRIP_CYCLE);
    msg[0] = '\0';
    stat = strstat(&str, msg, sizeof(msg));
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::MenuQuitClick(TObject *Sender) { Close(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::MenuUpdateCasterClick(TObject *Sender) { UpdateCaster(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::MenuUpdateTableClick(TObject *Sender) { UpdateTable(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::BtnListClick(TObject *Sender) { UpdateCaster(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::BtnUpdateClick(TObject *Sender) { UpdateTable(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::TypeStrClick(TObject *Sender) { ShowTable(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::TypeCasClick(TObject *Sender) { ShowTable(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::TypeNetClick(TObject *Sender) { ShowTable(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::TypeSrcClick(TObject *Sender) { ShowTable(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::AddressKeyPress(TObject *Sender, char &Key) {
  if (Key == '\r') UpdateTable();
}
//---------------------------------------------------------------------------
void __fastcall TMainForm::AddressChange(TObject *Sender) { ShowTable(); }
//---------------------------------------------------------------------------
void __fastcall TMainForm::TypeChange(TObject *Sender) { ShowTable(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::Table0MouseDown(TObject *Sender, TMouseButton Button, TShiftState Shift,
                                           int X, int Y) {
  AnsiString title;
  double lat, lon;
  int col, row;
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::Table1MouseDown(TObject *Sender, TMouseButton Button, TShiftState Shift,
                                           int X, int Y) {
  int col, row;
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::Table2MouseDown(TObject *Sender, TMouseButton Button, TShiftState Shift,
                                           int X, int Y) {
  int col, row;
...
void __fastcall TMainForm::ShowTable(void) {
  const char *ti[][19] = {
      {"No", "Mountpoint", "ID", "Format", "Format-Details", "Carrier", "Nav-System", "Network",
       "Country", "Latitude", "Longitude", "NMEA", "Solution", "Generator", "Compr-Encrp",
  ...
  for (i = 0; i < 3; i++) table[i]->Visible = false;
  ...
  for (p = SrcTable.c_str(), ns = 0; *p; p = q + 1) {
    ...
    switch (type) {
      case 0:
        if (!strncmp(p, "STR", 3)) ns++;
        break;
      case 1:
        if (!strncmp(p, "CAS", 3)) ns++;
        break;
      case 2:
        if (!strncmp(p, "NET", 3)) ns++;
        break;
    }
  }
  if (ns <= 0) return;
  ...
  for (p = SrcTable.c_str(), j = 1; *p; p = q + 1) {
    ...
    switch (type) {
      case 0:
        if (!strncmp(buff, "STR", 3))
          break;
        else
          continue;
      case 1:
        if (!strncmp(buff, "CAS", 3))
          break;
        else
          continue;
      case 2:
        if (!strncmp(buff, "NET", 3))
          break;
        else
          continue;
    }
...
void __fastcall TMainForm::UpdateMap(void) {
  ...
  for (int i = 1; i < Table0->RowCount; i++) {
    ...
    msg = "<b>" + Table0->Cells[1][i] + "</b>: " + Table0->Cells[2][i] + " (" +
          Table0->Cells[8][i] + ")<br>" + "Format: " + Table0->Cells[3][i] + ", " +
          Table0->Cells[4][i] + ",<br>" + "Nav-Sys: " + Table0->Cells[6][i] + "<br>" +
...
//---------------------------------------------------------------------------
void __fastcall TMainForm::UpdateEnable(void) { BtnSta->Enabled = StaMask->Checked; }
//---------------------------------------------------------------------------
void __fastcall TMainForm::StaMaskClick(TObject *Sender) { UpdateEnable(); }
//---------------------------------------------------------------------------
