...
void __fastcall TMainForm::BtnStartClick(TObject *Sender) { SvrStart(); }
// callback on button-stop --------------------------------------------------
void __fastcall TMainForm::BtnStopClick(TObject *Sender) { SvrStop(); }
...
void __fastcall TMainForm::BtnCmdClick(TObject *Sender) {
  ...
  if (i == 0)
    CmdOptDialog->Caption = s.sprintf("Input Serial/TCP Commands");
  else
    CmdOptDialog->Caption = s.sprintf("Output%d Serial/TCP Commands", i);
...
void __fastcall TMainForm::TrayIconMouseDown(TObject *Sender, TMouseButton Button,
...
void __fastcall TMainForm::MenuStartClick(TObject *Sender) { SvrStart(); }
// callback on menu-stop ----------------------------------------------------
void __fastcall TMainForm::MenuStopClick(TObject *Sender) { SvrStop(); }
// callback on menu-exit ----------------------------------------------------
void __fastcall TMainForm::MenuExitClick(TObject *Sender) { Close(); }
// callback on stream-monitor -----------------------------------------------
void __fastcall TMainForm::BtnStrMonClick(TObject *Sender) { StrMonDialog->Show(); }
// callback on output1 enable -----------------------------------------------
void __fastcall TMainForm::EnaOut1Click(TObject *Sender) { UpdateEnable(); }
// callback on output2 enable -----------------------------------------------// callback on output3
// enable -----------------------------------------------// callback on input type change
// --------------------------------------------
void __fastcall TMainForm::InputChange(TObject *Sender) { UpdateEnable(); }
// callback on output1 type change ------------------------------------------
void __fastcall TMainForm::OutputChange(TObject *Sender) { UpdateEnable(); }
...
void __fastcall TMainForm::SvrStart(void) {
  ...
  for (int i = 0; i < MAXSTR - 1; i++) {  // for each out stream
    ...
    if (!(conv[i] = strconvnew(ConvInp[i], ConvOut[i], ConvMsg[i].c_str(), StaId, StaSel,
                               ConvOpt[i].c_str())))
      continue;
...
void __fastcall TMainForm::Timer2Timer(TObject *Sender) {
  const char *types[] = {"None",       "Serial",      "File",         "TCP Server",
                         "TCP Client", "Ntrip Sever", "Ntrip Client", "FTP",
                         "HTTP",       "Ntrip Cast",  "UDP Sever",    "UDP Client"};
...
#define MSG_SIZE 16000
    if (!(msg = (char *)malloc(MSG_SIZE))) return;
...
void __fastcall TMainForm::UpdateEnable(void) {
  TComboBox *type[] = {Output1, Output2, Output3, Output4, Output5, Output6};
  TLabel *label1[] = {LabelOutput1, LabelOutput2, LabelOutput3,
