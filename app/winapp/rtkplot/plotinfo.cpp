...
void __fastcall TPlot::UpdateInfo(void) {
  int showobs = (PLOT_OBS <= PlotType && PlotType <= PLOT_DOP) || PlotType == PLOT_SNR ||
...
void __fastcall TPlot::UpdateTimeSol(void) {
  ...
  if ((BtnSol1->Down || BtnSol2->Down || BtnSol12->Down) && (data = getsol(SolData + sel, ind))) {
    if (!ConnectState)
      msg.sprintf("[%d]", sel + 1);
    else
      msg = "[R]";
    ...
    if (PLOT_SOLP <= PlotType && PlotType <= PLOT_SOLA) {
      ...
      msg += s.sprintf("E=%7.4f%s N=%7.4f%s U=%7.4f%s Q=", p->x[0], u, p->y[0], u, p->z[0], u);
      delete p;
    } else if (PlotType == PLOT_NSAT) {
      msg += s.sprintf("NS=%d AGE=%.1f RATIO=%.1f Q=", data->ns, data->age, data->ratio);
    } else if (!data->type) {
      ...
      msg += s.sprintf("B=%.3fm D=%6.2f" CHARDEG " %5.2f" CHARDEG "  Q=", r,
                       az < 0.0 ? az + 360.0 : az, el);
    }
