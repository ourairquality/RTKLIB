...
//---------------------------------------------------------------------------
__fastcall TMapOptDialog::TMapOptDialog(TComponent *Owner) : TForm(Owner) {}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::BtnUpdateClick(TObject *Sender) { UpdateMap(); }
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::BtnCloseClick(TObject *Sender) { Close(); }
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::ScaleXUpDownChangingEx(TObject *Sender, bool &AllowChange,
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::ScaleYUpDownChangingEx(TObject *Sender, bool &AllowChange,
                                                      int NewValue, TUpDownDirection Direction) {
  AnsiString s;
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::LatUpDownChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
...
//---------------------------------------------------------------------------
void __fastcall TMapOptDialog::LonUpDownChangingEx(TObject *Sender, bool &AllowChange, int NewValue,
                                                   TUpDownDirection Direction) {
  AnsiString s;
...

{
  ...
  if (Key == VK_UP || Key == VK_DOWN || Key == VK_RIGHT || Key == VK_LEFT) {
    ScaleXUpDownChangingEx(Sender, allowchange, 0,
                           Key == VK_UP || Key == VK_RIGHT ? updUp : updDown);
    Key = 0;
  }
}
//---------------------------------------------------------------------------
...

{
  bool allowchange;
  if (Key == VK_UP || Key == VK_DOWN || Key == VK_RIGHT || Key == VK_LEFT) {
    ScaleYUpDownChangingEx(Sender, allowchange, 0,
                           Key == VK_UP || Key == VK_RIGHT ? updUp : updDown);
    Key = 0;
  }
}
