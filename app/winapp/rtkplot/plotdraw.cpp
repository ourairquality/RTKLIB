...
void __fastcall TPlot::DrawTrkPos(const double *rr, int type, int siz, TColor color,
                                  UTF8String label) {
  gtime_t time = {0};
  TPoint p1;
  double xyz[3], xs, ys;
...
void __fastcall TPlot::DrawTrkStat(const TIMEPOS *pos, UTF8String header, int p) {
  ...
  if (pos->n > 0) {
    ...
    s[n++].sprintf("RMS=E:%7.4fm N:%7.4fm U:%7.4fm 2D:%7.4fm", rms[0], rms[1], rms[2],
                   2.0 * rms[3]);
    delete[] d;
  }
  GraphT->GetPos(p1, p2);
...
void __fastcall TPlot::DrawNsat(int level) {
  UTF8String label[] = {"# of Valid Satellites", "Age of Differential (s)",
...
void __fastcall TPlot::DrawObsEphem(double *yp) {
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < Nav.n[i]; j++) {
      ...
      for (k = 0; k < 3; k++) ps[k].y += MarkSize + 2 + off[Nav.eph[i][j].sat - 1];
      ps[0].y -= 2;
    ...
    for (m = 0; m < NSATGLO; m++) {
      for (j = 0; j < Nav.ng[m]; j++) {
        ...
        for (k = 0; k < 3; k++) ps[k].y += MarkSize + 2 + off[Nav.geph[m][j].sat - 1];
        ps[0].y -= 2;

        GraphR->DrawPoly(ps, 3, Nav.geph[m][j].svh ? MColor[0][5] : CColor[1], 0);

        if (in)
          GraphR->DrawMark(ps[2], 0, Nav.geph[m][j].svh ? MColor[0][5] : CColor[1],
    ...
    for (m = 0; m < NSATSBS; m++) {
      for (j = 0; j < Nav.ns[m]; j++) {
        ...
        for (k = 0; k < 3; k++) ps[k].y += MarkSize + 2 + off[Nav.seph[m][j].sat - 1];
        ps[0].y -= 2;

        GraphR->DrawPoly(ps, 3, Nav.seph[m][j].svh ? MColor[0][5] : CColor[1], 0);

        if (in)
          GraphR->DrawMark(ps[2], 0, Nav.seph[m][j].svh ? MColor[0][5] : CColor[1],
...
void __fastcall TPlot::DrawSky(int level) {
  ...
  if (ShowStats && BtnShowTrack->Down && 0 <= ind && ind < NObs && !SimObs) {
    ...
    if (!strcmp(obstype, "ALL")) {
      ustr.sprintf("%3s: %*s %*s%*s %*s", "SAT", NFREQ, "PR", NFREQ, "CP", NFREQ * 3, "CN0", NFREQ,
                   "LLI");
    } else {
      ...
      if (!strcmp(obstype, "ALL")) {
        ...
        for (j = 0; j < NFREQ; j++) {
          if (obs->P[j] == 0.0 && obs->L[j] == 0.0)
            ...
            ustr += ss.sprintf("%02.0f ", obs->SNR[j] * SNR_UNIT);
        }
        for (j = 0; j < NFREQ; j++) {
          if (obs->L[j] == 0.0)
            ustr += "-";
          else
            ustr += ss.sprintf("%d", obs->LLI[j]);
        }
        ...
        if (obs->P[freq - 1] == 0.0 && obs->L[freq - 1] == 0.0)
          ...
          ustr += ss.sprintf("%4.1f ", obs->SNR[freq - 1] * SNR_UNIT);
        if (obs->L[freq - 1] == 0.0)
          ustr += " -";
        else
          ustr += ss.sprintf("%2d", obs->LLI[freq - 1]);
      } else {
        for (j = 0; j < NFREQ + NEXOBS; j++) {
          if (!strcmp(code2obs(obs->code[j]), obstype)) break;
        }
        if (j >= NFREQ + NEXOBS) continue;
        ustr += ss.sprintf("%3s  %s %s %s  ", code2obs(obs->code[j]), obs->P[j] == 0.0 ? "-" : "C",
                           obs->L[j] == 0.0 ? "-" : "L", obs->D[j] == 0.0 ? "-" : "D");
        if (obs->P[j] == 0.0 && obs->L[j] == 0.0)
          ustr += "---- ";
        else
          ustr += ss.sprintf("%4.1f ", obs->SNR[j] * SNR_UNIT);
        if (obs->L[j] == 0.0)
          ustr += " -";
        else
          ustr += ss.sprintf("%2d", obs->LLI[j]);
      }
      TColor col = ObsColor(obs, Az[i], El[i]);
      p2.y += hh;
...
void __fastcall TPlot::DrawDopStat(double *dop, int *ns, int n) {
  ...
  if (DopType->ItemIndex == 0 || DopType->ItemIndex >= 2) {
    s2[m++].sprintf("AVE= GDOP:%4.1f PDOP:%4.1f HDOP:%4.1f VDOP:%4.1f", ave[0], ave[1], ave[2],
                    ave[3]);
    s2[m++].sprintf("NDOP=%d(%4.1f%%) %d(%4.1f%%) %d(%4.1f%%) %d(%4.1f%%)", ndop[0],
                    n > 0 ? ndop[0] * 100.0 / n : 0.0, ndop[1], n > 0 ? ndop[1] * 100.0 / n : 0.0,
                    ndop[2], n > 0 ? ndop[2] * 100.0 / n : 0.0, ndop[3],
                    n > 0 ? ndop[3] * 100.0 / n : 0.0);
  }
  ...
  if (NObs > 0 && BtnSol1->Down) {
    ...
    for (int i = 0, l = 0; i < 3; i++) {
      ...
      if (BtnShowTrack->Down && 0 <= ObsIndex && ObsIndex < NObs && BtnSol1->Down) {
        ...
        p1.x = p2.x - 8;
        p1.y += 3;
        DrawLabel(GraphG[i], p1, s.sprintf("%.*f %s", i == 1 ? 4 : 1, yp[0], unit[i].c_str()), 2,
                  2);
      }
    }
    delete[] x;
...
void __fastcall TPlot::DrawPolyS(TGraph *graph, double *x, double *y, int n, TColor color,
                                 int style) {
  int i, j;
...
void __fastcall TPlot::DrawLabel(TGraph *g, TPoint p, UTF8String label, int ha, int va) {
  g->DrawText(p, label, CColor[2], CColor[0], ha, va, 0);
}
// draw mark with hemming ---------------------------------------------------
void __fastcall TPlot::DrawMark(TGraph *g, TPoint p, int mark, TColor color, int size, int rot) {
...
void __fastcall TPlot::Refresh_MapView(void) {
  ...
  if (BtnShowTrack->Down) {
    if (BtnSol2->Down && SolData[1].n > 0 && (sol = getsol(SolData + 1, SolIndex[1]))) {
      ecef2pos(sol->rr, pos);
    ...
    if (BtnSol1->Down && SolData[0].n > 0 && (sol = getsol(SolData, SolIndex[0]))) {
      ecef2pos(sol->rr, pos);
