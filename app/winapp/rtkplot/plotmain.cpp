...
__fastcall TPlot::TPlot(TComponent *Owner) : TForm(Owner) {
  ...
  obs0.data = NULL;
  obs0.n = obs0.nmax = 0;
  for (int i = 0; i < MAXSAT; i++) nav0.eph[i] = NULL;
  nav0.n[i] = nav0.nmax[i] = 0;
  for (int i = 0; i < NSATGLO; i++) nav0.geph[i] = NULL;
  nav0.ng[i] = nav0.ngmax[i] = 0;
  for (int i = 0; i < NSATSBS; i++) nav0.seph[i] = NULL;
  nav0.ns[i] = nav0.nsmax[i] = 0;
  ObsIndex = 0;
  Obs = obs0;
  Nav = nav0;
  Sta = sta0;
...
void __fastcall TPlot::FormCreate(TObject *Sender) { ::DragAcceptFiles(Handle, true); }
...
void __fastcall TPlot::FormShow(TObject *Sender) {
  ...
  if (ShapeFile != "") {
    ...
    int n = expath(ShapeFile.c_str(), paths, 1024, MAXSHAPEFILE);
...
void __fastcall TPlot::DropFiles(TWMDropFiles msg) {
  ...
  if (ConnectState || (n = DragQueryFile((HDROP)msg.Drop, 0xFFFFFFFF, NULL, 0)) <= 0) {
    delete files;
    return;
  }
  for (int i = 0; i < n; i++) {
  ...
  if (n == 1 && (ext = strrchr(file.c_str(), '.')) &&
      (!strcmp(ext, ".jpg") || !strcmp(ext, ".jpeg") || !strcmp(ext, ".JPG") ||
       !strcmp(ext, ".JPEG"))) {
...
void __fastcall TPlot::MenuSaveImageClick(TObject *Sender) {
  ...
  if ((ext = strrchr(file, '.')) && (!strcmp(ext, ".jpg") || !strcmp(ext, ".jpeg") ||
...
void __fastcall TPlot::MenuTimeClick(TObject *Sender) {
  ...
  if (TimeEna[0] != SpanDialog->TimeEna[0] || TimeEna[1] != SpanDialog->TimeEna[1] ||
      ...
      timediff(TimeEnd, SpanDialog->TimeEnd) != 0.0 || TimeInt != SpanDialog->TimeInt) {
...
void __fastcall TPlot::MenuSrcObsClick(TObject *Sender) {
  ...
  cstat = rtk_uncompress(file.c_str(), tmpfile, sizeof(tmpfile));
...
void __fastcall TPlot::MenuMapViewClick(TObject *Sender) {
  ...
  MapView->Caption = s.sprintf("%s ver.%s %s: Map View", PRGNAME, VER_RTKLIB, PATCH_LEVEL);
  MapView->Show();
}
...
void __fastcall TPlot::BtnConnectClick(TObject *Sender) {
  ...
  if (!ConnectState)
    MenuConnectClick(Sender);
  else
    MenuDisconnectClick(Sender);
}
...
void __fastcall TPlot::DispMouseDown(TObject *Sender, TMouseButton Button, TShiftState Shift, int X,
                                     int Y) {
...
void __fastcall TPlot::DispMouseUp(TObject *Sender, TMouseButton Button, TShiftState Shift, int X,
                                   int Y) {
...
void __fastcall TPlot::MouseMoveTrk(int X, int Y, double dx, double dy, double dxs, double dys) {
...
void __fastcall TPlot::MouseMoveSol(int X, int Y, double dx, double dy, double dxs, double dys) {
  TPoint p1, p2, p(X, Y);
  double x, y, xs, ys;
  int i, sel = !BtnSol1->Down && BtnSol2->Down ? 1 : 0;
...
void __fastcall TPlot::MouseMoveObs(int X, int Y, double dx, double dy, double dxs, double dys) {
  TPoint p1, p2, p(X, Y);
  double x, y, xs, ys;
  int i;
...
void __fastcall TPlot::MouseWheel(TObject *Sender, TShiftState Shift, int WheelDelta,
...
void __fastcall TPlot::CMDialogKey(Messages::TWMKey &Message) {
  ...
  if (Message.CharCode != VK_UP && Message.CharCode != VK_DOWN && Message.CharCode != VK_LEFT &&
      Message.CharCode != VK_RIGHT) {
    TForm::Dispatch(&Message);
  }
}
// callback on key-down events ----------------------------------------------
void __fastcall TPlot::FormKeyDown(TObject *Sender, WORD &Key, TShiftState Shift) {
  ...
  if (PlotType == PLOT_TRK) {
    ...
    GraphT->GetScale(xs, ys);
    if (key == 1) {
      if (!BtnFixHoriz->Down) yc += fact * ys;
    }
    if (key == 2) {
      if (!BtnFixHoriz->Down) yc -= fact * ys;
    }
    if (key == 3) {
      if (!BtnFixHoriz->Down) xc -= fact * xs;
    }
    if (key == 4) {
      if (!BtnFixHoriz->Down) xc += fact * xs;
    }
    ...
    if (key == 1) {
      if (!BtnFixVert->Down) yc1 += fact * ys1;
      yc2 += fact * ys2;
      yc3 += fact * ys3;
    }
    if (key == 2) {
...
void __fastcall TPlot::UpdateEnable(void) {
  ...
  BtnSol12->Enabled =
      !ConnectState && PlotType <= PLOT_SOLA && SolData[0].n > 0 && SolData[1].n > 0;
  QFlag->Visible = PlotType == PLOT_TRK || PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
  ...
  Panel102->Visible = PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA ||
                      PlotType == PLOT_NSAT || PlotType == PLOT_RES || PlotType == PLOT_RESE ||
                      PlotType == PLOT_SNR || PlotType == PLOT_SNRE;
  BtnOn1->Enabled = plot || PlotType == PLOT_SNR || PlotType == PLOT_RES || PlotType == PLOT_RESE ||
                    PlotType == PLOT_SNRE;
  BtnOn2->Enabled = plot || PlotType == PLOT_SNR || PlotType == PLOT_RES || PlotType == PLOT_RESE ||
                    PlotType == PLOT_SNRE;
  BtnOn3->Enabled = plot || PlotType == PLOT_SNR || PlotType == PLOT_RES;

  BtnCenterOri->Visible = PlotType == PLOT_TRK || PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
                          PlotType == PLOT_SOLA || PlotType == PLOT_NSAT;
  BtnCenterOri->Enabled = PlotType != PLOT_NSAT;
  BtnRangeList->Visible = PlotType == PLOT_TRK || PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
                          PlotType == PLOT_SOLA || PlotType == PLOT_NSAT;
  BtnRangeList->Enabled = PlotType != PLOT_NSAT;

  BtnFitHoriz->Visible = PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA ||
  ...
  BtnFitVert->Visible = PlotType == PLOT_TRK || PlotType == PLOT_SOLP || PlotType == PLOT_SOLV ||
                        PlotType == PLOT_SOLA;
  BtnFitVert->Enabled = data;
  BtnShowTrack->Enabled = data;
  BtnFixCent->Visible = PlotType == PLOT_TRK;
  BtnFixCent->Enabled = data;
  BtnFixHoriz->Visible = PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA ||
                         PlotType == PLOT_NSAT || PlotType == PLOT_RES || PlotType == PLOT_OBS ||
                         PlotType == PLOT_DOP || PlotType == PLOT_SNR;
  BtnFixHoriz->Enabled = data;
  BtnFixVert->Visible = PlotType == PLOT_SOLP || PlotType == PLOT_SOLV || PlotType == PLOT_SOLA;
  ...
  BtnShowImg->Visible = PlotType == PLOT_TRK || PlotType == PLOT_SKY || PlotType == PLOT_MPS;
...
void __fastcall TPlot::LoadOpt(void) {
  ...
  MapStrs[0][1] =
      ini->ReadString("mapview", "mapstrs_0_1", "https://tile.openstreetmap.org/{z}/{x}/{y}.png");
...
//---------------------------------------------------------------------------
void __fastcall TPlot::BtnUdListClick(TObject *Sender) { FileList->Update(); }
//---------------------------------------------------------------------------
