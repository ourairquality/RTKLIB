...
void __fastcall TPlot::ReadSol(TStrings *files, int sel) {
  ...
  if ((h = CreateFile(s.c_str(), GENERIC_READ, 0, NULL, OPEN_EXISTING, FILE_ATTRIBUTE_NORMAL, 0)) ==
...
int __fastcall TPlot::ReadObsRnx(TStrings *files, obs_t *obs, nav_t *nav, sta_t *sta) {
  AnsiString s;
  gtime_t ts, te;
  double tint;
  int i, n;
  ...
  for (i = 0; i < files->Count; i++) {
    ...
    if (!strcmp(p, ".obs") || !strcmp(p, ".OBS")) {
      ...
      strcpy(p, ".inav");
      readrnxt(navfile, 1, ts, te, tint, opt, NULL, nav, NULL);
    } else if (!strcmp(p + 3, "o") || !strcmp(p + 3, "d") || !strcmp(p + 3, "O") ||
               !strcmp(p + 3, "D")) {
      n = nav->n;
...
void __fastcall TPlot::UpdateSky(void) {
  ...
  if (!GetObject(SkyImageI->Handle, sizeof(bm1), &bm1) ||
      !GetObject(SkyImageR->Handle, sizeof(bm2), &bm2) || bm1.bmBitsPixel != 24)
    return;
  w1 = bm1.bmWidth;
  h1 = bm1.bmHeight;
  b1 = bm1.bmWidthBytes;
...
void __fastcall TPlot::ReadSkyTag(AnsiString file) {
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    *p = '\0';
    if (strstr(buff, "centx") == buff)
      sscanf(p + 1, "%lf", SkyCent);
    else if (strstr(buff, "centy") == buff)
      sscanf(p + 1, "%lf", SkyCent + 1);
    else if (strstr(buff, "scale") == buff)
      sscanf(p + 1, "%lf", &SkyScale);
    else if (strstr(buff, "roll") == buff)
      sscanf(p + 1, "%lf", SkyFov);
    else if (strstr(buff, "pitch") == buff)
      sscanf(p + 1, "%lf", SkyFov + 1);
    else if (strstr(buff, "yaw") == buff)
      sscanf(p + 1, "%lf", SkyFov + 2);
    else if (strstr(buff, "destcorr") == buff)
      sscanf(p + 1, "%d", &SkyDestCorr);
    else if (strstr(buff, "elmask") == buff)
      sscanf(p + 1, "%d", &SkyElMask);
    else if (strstr(buff, "resample") == buff)
      sscanf(p + 1, "%d", &SkyRes);
    else if (strstr(buff, "flip") == buff)
      sscanf(p + 1, "%d", &SkyFlip);
    else if (strstr(buff, "dest") == buff) {
      sscanf(p + 1, "%lf %lf %lf %lf %lf %lf %lf %lf %lf", SkyDest + 1, SkyDest + 2, SkyDest + 3,
             SkyDest + 4, SkyDest + 5, SkyDest + 6, SkyDest + 7, SkyDest + 8, SkyDest + 9);
    } else if (strstr(buff, "binarize") == buff)
      sscanf(p + 1, "%d", &SkyBinarize);
    else if (strstr(buff, "binthr1") == buff)
      sscanf(p + 1, "%lf", &SkyBinThres1);
    else if (strstr(buff, "binthr2") == buff)
      sscanf(p + 1, "%lf", &SkyBinThres2);
  }
  fclose(fp);
}
// read sky image data ------------------------------------------------------
...
void __fastcall TPlot::ReadMapTag(AnsiString file) {
  ...
  while (fgets(buff, sizeof(buff), fp)) {
    ...
    if (!(p = strchr(buff, '='))) continue;
    *p = '\0';
    if (strstr(buff, "scalex") == buff)
      sscanf(p + 1, "%lf", &MapScaleX);
    else if (strstr(buff, "scaley") == buff)
      sscanf(p + 1, "%lf", &MapScaleY);
    else if (strstr(buff, "scaleeq") == buff)
      sscanf(p + 1, "%d", &MapScaleEq);
    else if (strstr(buff, "lat") == buff)
      sscanf(p + 1, "%lf", &MapLat);
    else if (strstr(buff, "lon") == buff)
      sscanf(p + 1, "%lf", &MapLon);
  }
  fclose(fp);
}
// read shapefile -----------------------------------------------------------
...
void __fastcall TPlot::ReadGpxFile(AnsiString file) {
  ...
  while (fgets(buff, sizeof(buff), fp) && NWayPnt < MAXWAYPNT) {
    if ((p = strstr(buff, "<wpt "))) {
      if (sscanf(p + strlen("<wpt "), "lat=\"%lf\" lon=\"%lf\"", pos, pos + 1) < 2) continue;
...
void __fastcall TPlot::SavePosFile(AnsiString file) {
  ...
  for (i = 0; i < NWayPnt; i++) {
    AnsiString str(PntName[i]);
    fprintf(fp, "%13.9f %14.9f %10.4f %s\n", PntPos[i][0], PntPos[i][1], PntPos[i][2], str.c_str());
  }
  fclose(fp);
}
// save waypoint ------------------------------------------------------------
...
void __fastcall TPlot::ReadStaPos(const char *file, const char *sta, double *rr) {
  FILE *fp;
...
void __fastcall TPlot::SaveSnrMp(AnsiString file) {
  ...
  fprintf(fp, "%% %-*s %6s %8s %8s %9s %10s\n", TimeLabel == 0 ? 13 : 19, tlabel, "SAT", "AZ(deg)",
          "EL(deg)", "SNR(dBHz)", mp);
  ...
  for (i = 0; i < MAXSAT; i++) {
    ...
    for (j = 0; j < Obs.n; j++) {
      ...
      fprintf(fp, "%s %6s %8.1f %8.1f %9.2f %10.4f\n", tstr, sat, Az[j] * R2D, El[j] * R2D,
              Obs.data[j].SNR[k] * SNR_UNIT, !Mp[k] ? 0.0 : Mp[k][j]);
    }
  }
  fclose(fp);
}
...
void __fastcall TPlot::Connect(void) {
  ...
  for (i = 0; i < 2; i++) {
    if (RtStream[i] == STR_NONE)
      continue;
    else if (RtStream[i] == STR_SERIAL)
      path = StrPaths[i][0].c_str();
    else if (RtStream[i] == STR_FILE)
      path = StrPaths[i][2].c_str();
    else if (RtStream[i] <= STR_NTRIPCLI)
      path = StrPaths[i][1].c_str();
    else
      continue;
  ...
  if (Title != "")
    ...
    Caption = s.sprintf("CONNECT %s %s", name[0], name[1]);
...
int __fastcall TPlot::CheckObs(AnsiString file) {
  ...
  if (!strcmp(p, ".z") || !strcmp(p, ".gz") || !strcmp(p, ".zip") || !strcmp(p, ".Z") ||
      !strcmp(p, ".GZ") || !strcmp(p, ".ZIP")) {
    return *(p - 1) == 'o' || *(p - 1) == 'O' || *(p - 1) == 'd' || *(p - 1) == 'D';
  }
  return !strcmp(p, ".obs") || !strcmp(p, ".OBS") || !strcmp(p, ".rnx") || !strcmp(p, ".RNX") ||
...
void __fastcall TPlot::UpdateObs(int nobs) {
  ...
  for (int i = 0, j = 0; i < Obs.n; i = j) {
    ...
    if (RcvPos == 0) {  // single point position
      ...
      msg[0] = '\0';
      opt.err[0] = 900.0;
      pntpos(Obs.data + i, j - i, &Nav, &opt, &sol, azel, NULL, msg, sizeof(msg));
