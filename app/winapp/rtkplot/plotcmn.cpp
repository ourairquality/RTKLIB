...
//---------------------------------------------------------------------------
const char *PTypes[] = {"Gnd Trk",   "Position",  "Velocity",   "Accel",   "NSat",
                        "Residuals", "Resid-EL",  "Sat Vis",    "Skyplot", "DOP/NSat",
                        "SNR/MP/EL", "SNR/MP-EL", "MP-Skyplot", ""};
...
int __fastcall TPlot::ExecCmd(AnsiString cmd) {
  ...
  if (!CreateProcess(NULL, p, NULL, NULL, false, CREATE_NO_WINDOW, NULL, NULL, &si, &info))
    return 0;
...
void __fastcall TPlot::PosToXyz(gtime_t time, const double *rr, int type, double *xyz) {
...
void __fastcall TPlot::CovToXyz(const double *rr, const float *qr, int type, double *xyzs) {
...
void __fastcall TPlot::CalcStats(const double *x, int n, double ref, double &ave, double &std,
                                 double &rms) {
...
void __fastcall TPlot::TimeStr(gtime_t time, int n, int tsys, char str[48]) {
  ...
  if (TimeLabel == 0) {  // www/ssss
    ...
    if (!(t = localtime(&time.time)))
      ...
      snprintf(tstr, sizeof(tstr), "%04d/%02d/%02d %02d:%02d:%02d.%0*d", t->tm_year + 1900,
               t->tm_mon + 1, t->tm_mday, t->tm_hour, t->tm_min, t->tm_sec, n,
               (int)(time.sec * pow(10.0, n)));
    label = " LT";
  }
...
UTF8String __fastcall TPlot::LatLonStr(const double *pos, int ndec) {
  ...
  if (LatLonFmt == 0) {
    s.sprintf("%*.*f" CHARDEG "%s %*.*f" CHARDEG "%s", ndec + 4, ndec, fabs(pos[0] * R2D),
              pos[0] < 0.0 ? "S" : "N", ndec + 5, ndec, fabs(pos[1] * R2D),
              pos[1] < 0.0 ? "W" : "E");
  } else {
    deg2dms(pos[0] * R2D, dms1, ndec - 5);
    deg2dms(pos[1] * R2D, dms2, ndec - 5);
    s.sprintf("%3.0f" CHARDEG "%02.0f'%0*.*f\"%s %4.0f" CHARDEG "%02.0f'%0*.*f\"%s", fabs(dms1[0]),
              dms1[1], ndec - 2, ndec - 5, dms1[2], pos[0] < 0.0 ? "S" : "N", fabs(dms2[0]),
              dms2[1], ndec - 2, ndec - 5, dms2[2], pos[1] < 0.0 ? "W" : "E");
  }
  return s;
}
