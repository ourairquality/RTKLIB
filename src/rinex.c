                          ...
                          sigind_t *index, obsd_t *obs)
{
    ...
    int stat = 1;
    if (obs->sat == 0) {
      // Note for version 2 the system is unknown here and multi-line
      // input might not be skipped properly.
    ...
    for (i=0,j=ver<=2.9999?0:3;i<ind->n;i++,j+=16) {
        ...
        if (!stat) continue; // Only need to consume the input.
        int code = ind->code[i];
        if (code != CODE_NONE) {
            ...
            // Note non-zero pseudorange values for each signal.
            if (ind->type[i] == 0 && val[i] != 0) csigp[code] = 1;
        }
    }
    if (!stat) return 0;
    ...
    for (i=0;i<ind->n;i++) {
        ...
        if (!csigp[code]) continue; // Skip if no pseudo range value for this signal.
        int freqidx = sigindex(obs, sys, code, opt);
        if (freqidx < 0) continue;
        switch (ind->type[i]) {
            case 0: obs->P[freqidx]=val[i];
                    obs->Pstd[freqidx] = std[i] > 0 ? (float)(0.01 * pow(2, std[i] + 5)) : 0.0f;
                    break;
            case 1: obs->L[freqidx]=val[i];
                    obs->LLI[freqidx]=lli[i];
                    obs->Lstd[freqidx] = std[i] > 0 ? (float)(std[i] * 0.004) : 0.0f;
                    break;
            case 2: obs->D[freqidx]=(float)val[i]; break;
            case 3: obs->SNR[freqidx]=(float)val[i]; break;
        }
...
static void saveslips(uint8_t slips[][MAXCODE], obsd_t *data)
{
    for (int i=0;i<NFREQ+NEXOBS;i++) {
        ...
        if (data->LLI[i]&LLI_SLIP) slips[data->sat-1][code]=1;
    }
}
/* restore cycle slips -------------------------------------------------------*/
static void restslips(uint8_t slips[][MAXCODE], obsd_t *data)
{
    for (int i=0;i<NFREQ+NEXOBS;i++) {
        int code = data->code[i];
        if (slips[data->sat-1][code]!=0) data->LLI[i]|=LLI_SLIP;
