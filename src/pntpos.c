...
static double varerr(int sat, int sys, double el, double snr, int frq,
                   ...
                   double *azel, int *vsat, double *resp, int *ns)
{
    ...
    *ns=0;
    for (int i=0;i<n&&i<MAXOBS;i++) {
        ...
        if (opt->ionoopt==IONOOPT_IFLC && obs[i].SNR[f2] < snr) snr = obs[i].SNR[f2];
        var[nv++] += varerr(sat, sys, azel[1 + i * 2], snr, 0, opt, &obs[i]);
