...
static int snrmask(const obsd_t *obs, const double *azel, const prcopt_t *opt, int base)
{
    ...
    if (opt->ionoopt==IONOOPT_IFLC) {
        f2 = seliflc(opt->nf, satsyst(obs->sat, obs->time, NULL));
        if (testsnr(base,f2,azel[1],obs->SNR[f2],&opt->snrmask)) return 0;
    }
    return 1;
}
                     ...
                     double *var)
{
    ...
    sat=obs->sat;
    int sys = satsyst(sat, obs->time, NULL);
    P1=obs->P[0];
                   ...
                   double *azel, int *vsat, double *resp, int *ns)
{
    ...
    *ns=0;
    for (int i=0;i<n&&i<MAXOBS;i++) {
        ...
        int sat=obs[i].sat, sys = satsyst(sat, obs[i].time, NULL);
        if (!sys) continue;
        ...
        /* Excluded satellite? */
        if (satexclude(sat, obs[i].time, vare[i], svh[i], opt)) continue;
        ...
        if (pos[2] > -RE_WGS84 / 2) {
          ...
          r = geodist(rs + i * 6, rpc, e);
          if (r <= 0.0) continue;
          if (satazel(rpc_pos,e,azel+i*2)<opt->elmin) continue;
        } else {
          // Skip antenna delta and PCO and elevation masking on
          // position initialization.
          r = geodist(rs + i * 6, rr, e);
          if (r <= 0.0) continue;
          for (int j = 0; j < 3; j++) rpc_pos[j] = pos[j];
          satazel(rpc_pos,e,azel+i*2);
        }
                    ...
                    sol_t *sol, double *azel, int *vsat, double *resp, char *msg)
{
    ...
    trace(3,"raim_fde: %s n=%2d base=%d\n",time2str(obs[0].time,tstr,0),n,base);
                  ...
                  char *msg)
{
    ...
    if (ssat) {
        ...
        for (int i = 0; i < n; i++) {
          ...
          if (opt->ionoopt == IONOOPT_IFLC) {
            // Use the minimum SNR.
            int sys = satsyst(obs[i].sat, obs[i].time, NULL);
            int f2 = seliflc(opt->nf, sys);
            if (obs[i].SNR[f2] < snr) snr = obs[i].SNR[f2];
          }
          if (base)
            ssat[obs[i].sat - 1].snr_base[0] = (float)snr;
          else
            ssat[obs[i].sat - 1].snr_rover[0] = (float)snr;
        }
    }
    
    if (opt_.mode!=PMODE_SINGLE) { /* for precise positioning */
        if (opt_.ionoopt != IONOOPT_SBAS && opt_.ionoopt != IONOOPT_TEC &&
            opt_.ionoopt != IONOOPT_VTEC)
