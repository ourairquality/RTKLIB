...
static int decode_qzssnav(raw_t *raw) {
  ...
  if (!strstr(raw->opt, "EPHALL")) {
    if ((eph.iode == raw->nav.eph[sat - 1][0].iode) &&
        (eph.iodc == raw->nav.eph[sat - 1][0].iodc)) {
      trace(3, "eph.iode %d raw->nav.eph[sat - 1][0].iode %d\n", eph.iode,
            raw->nav.eph[sat - 1][0].iode);
      trace(3, "eph.iodc %d raw->nav.eph[sat - 1][0].iodc %d\n", eph.iode,
            raw->nav.eph[sat - 1][0].iode);
      return 0;
    }
  }
...
static int decode_sbp(raw_t *raw) {
  ...
  if (raw->outtype) {
    rtksnprintf(raw->msgtype, sizeof(raw->msgtype), "SBP 0x%04X (%4d):", type, raw->len);
  }
