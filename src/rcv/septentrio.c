...
static uint8_t const Meas3_SVIDBase[] = {
    MINPRNGPS, MINPRNGLO, MINPRNGAL, MINPRNBDS,
...
static uint8_t U1(const raw_t *raw, size_t index) {
  RBOUNDSCHECK(raw->buff, sizeof(raw->buff), index);
  RBOUNDSCHECK(raw->buff, raw->len, index);
...
static uint16_t U2(const raw_t *raw, size_t index) {
  RBOUNDSCHECK(raw->buff, sizeof(raw->buff), index + 1);
  RBOUNDSCHECK(raw->buff, raw->len, index + 1);
  uint16_t u;
...
static uint32_t U4(const raw_t *raw, size_t index) {
  RBOUNDSCHECK(raw->buff, sizeof(raw->buff), index + 3);
  RBOUNDSCHECK(raw->buff, raw->len, index + 3);
  uint32_t u;
...
static double R8(const raw_t *raw, size_t index) {
  RBOUNDSCHECK(raw->buff, sizeof(raw->buff), index + 7);
  RBOUNDSCHECK(raw->buff, raw->len, index + 7);
  double r;
  memcpy(&r, raw->buff + index, 8);
  return r;
}
static void STR(const raw_t *raw, size_t index, size_t size, char *string, size_t str_size) {
  RBOUNDSCHECK(raw->buff, sizeof(raw->buff), index + size - 1);
  RBOUNDSCHECK(raw->buff, raw->len, index + size - 1);
  RBOUNDSCHECK(string, str_size, size);
...
static const int Meas3SigIdx2SignalType_Default[7][16] = {
    ...
    // QZS
    {CODE_L1C, CODE_L2L, CODE_L5Q, CODE_L6L, CODE_L1L, CODE_L1Z, CODE_L5P, CODE_L1E,
     CODE_NONE, CODE_NONE, CODE_NONE, CODE_NONE, CODE_NONE, CODE_NONE, CODE_NONE, CODE_NONE},
    // IRN
...
static int decode_measepoch(raw_t *raw) {
  ...
  for (int i = 0, pi = 20; i < n1 && n < MAXOBS && pi + 20 <= raw->len; i++) {
    ...
    int sys = satsyst(sat, raw->time, NULL);
    ...
    if (U1(raw, pi + 15) != 255) {
      ...
      if (idx >= 0) raw->obuf.data[n].SNR[idx] = (float)S1;
    }
    ...
    for (int j = 0; j < n2 && pi + 12 <= raw->len; j++, pi += len2) {
      ...
      if (U1(raw, pi + 2) != 255) {
        ...
        raw->obuf.data[n].SNR[idx] = (float)S2;
      }
      if (D1 != 0.0 && freq1 > 0.0 && freq2 > 0.0 &&
        ...
        raw->obuf.data[n].D[idx] = (float)((float)(D1 * freq2 / freq1) + D2);
      }
    }
    n++;
  }
...
static int decode_measepochextra(raw_t *raw) {
  ...
  for (uint8_t i = 0; i < n_chn; i++) {
    ...
    int sys = satsyst(raw->obuf.data[n].sat, raw->time, NULL);
    if (sig_tbl[sig][0] == 0) {
    ...
    if ((revision >= 3) &&
      ...
      raw->obuf.data[n].SNR[idx] += (float)((misc & 0x7) * 0.03125);
    }
  }
...
static int decode_meas3ranges(raw_t *raw) {
...
#endif
  uint16_t constellations = U2(raw, 16);
  ...
  for (int navsys = 0; navsys < MEAS3_SYS_MAX && n < MAXOBS; navsys++) {
    ...
    for (int svid = 0; svid < MEAS3_SAT_MAX && satCnt < nSats && n < MAXOBS; svid++) {
      ...
      int sys = satsyst(satNo, raw->time, NULL); // For the BDS2 BDS3 split.
      ...
      if ((blockTypeMaster & 1) == 1) {
        ...
        if (masterFreqIndex >= 0 && satNo > 0) {
          ...
          SNRm = CN0 + 24.0;
          raw->obuf.data[n].SNR[masterFreqIndex] = (float)SNRm;
          ...
          if ((blockTypeSlave & 1) == 1) {
            ...
            if (slaveFreqIndex >= 0 && satNo > 0) {
              ...
              if ((navsys == 0) && (codeSlave == CODE_L1W || codeSlave == CODE_L2W))
                ...
                SNRs = CN0 + 24.0;
              raw->obuf.data[n].SNR[slaveFreqIndex] = (float)SNRs;
...
int decode_meas3Doppler(raw_t *raw) {
  ...
  for (int n = 0; n < raw->obuf.n && offset < (uint32_t)raw->len; n++) {
    ...
    int prn, sys = satsyst(raw->obuf.data[n].sat, raw->time, &prn);
...
int decode_meas3CN(raw_t *raw) {
  ...
  for (int n = 0; n < raw->obuf.n && offset / 2 + 16 < (uint32_t)raw->len; n++) {
    ...
    raw->obuf.data[n].SNR[masterFreqIndex] += (float)(mc * 0.0625 - 0.5);
    offset++;
    for (int i = 1; i < MEAS3_SIG_MAX && offset / 2 + 16 < (uint32_t)raw->len; i++) {
      ...
      if (slaveFreqIndex >= 0) {
        ...
        raw->obuf.data[n].SNR[slaveFreqIndex] += (float)(sc * 0.0625 - 0.5);
      }
      offset++;
    }
  }
...
static int decode_glorawcanav(raw_t *raw) {
  ...
  int32_t time = (int32_t)raw->time.time;
...
static int decode_galrawfnav(raw_t *raw) {
  ...
  RBOUNDSCHECK(raw->subfrm[sat - 1] + 128, sizeof(raw->subfrm[sat - 1]) - 128,
                 (type - 1) * 31 + 30);
...
static int decode_galrawinav(raw_t *raw) {
  ...
  for (int i = 0, j = 2; i < 14; i++, j += 8) {
    ...
    RBOUNDSCHECK(raw->subfrm[sat - 1], 128, type * 16 + i);
...
extern int input_sbf(raw_t *raw, unsigned char data) {
  ...
  RBOUNDSCHECK(raw->buff, sizeof(raw->buff), raw->nbyte);
