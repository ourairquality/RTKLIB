...
static int sig2code(int sys, unsigned sigtype, unsigned l2c)
{
    if (sys == SYS_GPS) {
        switch (sigtype) {
...
static int decode_track_stat(uint32_t stat, int *sys, int *code, int *plock, int *clock)
{
    ...
    *clock = (stat >> 12) & 1;
    ...
    unsigned sigtype = (stat >> 21) & 0x1F;
    unsigned l2c = (stat >> 26) & 0x01;
    ...
    if (!(*code = sig2code(*sys, sigtype, l2c))) {
        trace(2, "unicore signal type error: sys=%u sigtype=%u\n", *sys, sigtype);
        return -1;
    }
    return 0;
}
