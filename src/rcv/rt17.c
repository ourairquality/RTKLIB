...
EXPORT int input_rt17(raw_t *Raw, uint8_t Data) {
  ...
  */
  ...
  if (Raw->outtype)
    rtksnprintf(Raw->msgtype, sizeof(Raw->msgtype), "RT17 0x%02X (%4u)", PacketBuffer[2],
                rt17->PacketLength);
  ...
  if (PacketBuffer[2] == RAWDATA) {
    ...
    */
    if (rt17->MessageBytes == 0) {
      ...
      tracet(2,
             "RT17: RAWDATA packet sequence number mismatch or page out of order. %u RAWDATA "
             "packets discarded.\n",
             Page);
  ...
  if (PacketBuffer[2] == GENOUT) {
    ...
    */
    if (rt17->MessageBytes == 0) {
      ...
      tracet(2,
             "RT17: GENOUT packet sequence number mismatch or page out of order. %u GENOUT "
             "packets discarded.\n",
             Page);
      ClearMessageBuffer(rt17);
      ClearPacketBuffer(rt17);
...
static int DecodeGSOF15(raw_t *Raw, uint8_t *p) {
  if (p[1] < 15)
    ...
    rtksnprintf(Raw->sta.recsno, sizeof(Raw->sta.recsno), "%u", U4(p + 2));
...
static int DecodeGSOF41(raw_t *raw, uint8_t *p) {
  if (p[1] < 6)
    tracet(2,
           "RT17: GSOF Base Position and Quality Indicator message record length %d < 6 bytes. "
           "Record discarded.\n",
           p[1]);
  else
    SetWeek(raw, I2(p + 6), ((long double)I4(p + 2)) * 0.001L);
...
static int DecodeIONAndUTCData(raw_t *Raw) {
  ...
  if (rt17->PacketLength < 129) {
    tracet(2,
           "RT17: RETSVDATA packet length %d < 129 bytes. GPS ION / UTC data packet discarded.\n",
           rt17->PacketLength);
    return -1;
  }
  ...
  *rif = p_in[7];
  ...
  while (InputLengthTotal > 0) {
    if ((uint32_t)p_in[7] != *rif)
      tracet(2,
             "RT17: Inconsistent Record Interpretation Flags within a single RAWDATA "
             "message.\n");
