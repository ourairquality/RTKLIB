...
static int is_meas(char sig) {
  return sig == 'c' || sig == 'C' || sig == '1' || sig == '2' || sig == '3' || sig == '5' ||
         sig == 'l';
}
...
/* Set time tag --------------------------------------------------------------*/
static bool settag(obsd_t *data, gtime_t time) {
  char s1[40], s2[40];

  if (data->time.time != 0 && fabsl(timediff(data->time, time)) > 5E-4L) {
...
static int flushobuf(raw_t *raw) {
  ...
  for (i = 0; i < MAXSAT; i++) raw->prCA[i] = raw->dpCA[i] = 0.0L;
  return n > 0 ? 1 : 0;
}
...
static int decode_RT(raw_t *raw) {
  ...
  time = adjday(time, raw->tod * 0.001L);
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " %s", time2str(time, tstr, 3));
  }
  /* Flush observation data buffer */
...
static int decode_RD(raw_t *raw) {
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " %04.0Lf/%02.0Lf/%02.0Lf base=%d", ep[0],
                 ep[1], ep[2], raw->tbase);
  }
  if (raw->tod < 0) {
  ...
  raw->time = timeadd(epoch2time(ep), raw->tod * 0.001L);
...
static int decode_SI(raw_t *raw) {
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " nsat=%2d", raw->obuf.n);
  }
  return 0;
}
...
static int decode_NN(raw_t *raw) {
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " nsat=%2d", ns);
  }
  return 0;
}
...
static int decode_eph(raw_t *raw, int sys) {
  ...
  p += 8;
  eph.M0 = (long double)R8(p) * SC2RAD;
  p += 8;
  eph.OMG0 = (long double)R8(p) * SC2RAD;
  p += 8;
  eph.i0 = (long double)R8(p) * SC2RAD;
  p += 8;
  eph.omg = (long double)R8(p) * SC2RAD;
  p += 8;
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " prn=%3d iode=%3d iodc=%3d toes=%6.0Lf", prn,
  ...
  if (!strstr(raw->opt, "-EPHALL")) {
    if (timediff(raw->nav.eph[sat - 1][set].toe, eph.toe) == 0.0L &&
        raw->nav.eph[sat - 1][set].iode == eph.iode && raw->nav.eph[sat - 1][set].iodc == eph.iodc)
      return 0; /* Unchanged */
  }
  eph.sat = sat;
...
static int decode_NE(raw_t *raw) {
  ...
  if (raw->len >= 85) { /* Firmware v 2.6.0 [2] */
    ...
    p += 8;
    geph.vel[0] = (long double)R4(p) * 1E3L;
    p += 4;
    geph.vel[1] = (long double)R4(p) * 1E3L;
    p += 4;
    geph.vel[2] = (long double)R4(p) * 1E3L;
    p += 4;
    geph.acc[0] = (long double)R4(p) * 1E3L;
    p += 4;
    geph.acc[1] = (long double)R4(p) * 1E3L;
    p += 4;
    geph.acc[2] = (long double)R4(p) * 1E3L;
    p += 4 + 8;
    geph.taun = (long double)R4(p);
    p += 4;
    geph.gamn = (long double)R4(p);
    p += 4;
  } else {
  ...
  if (raw->len >= 93) { /* Firmware v 3.2.0 [1] */
    geph.dtaun = (long double)R4(p);
    p += 4;
    geph.sva = U1(p);
  }
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " prn=%2d frq=%2d tk=%6d tb=%4d", prn,
                 geph.frq, tk, tb);
  }
  ...
  if (fabsl(tt) > 3600.0L) {
    trace(3, "javad NE illegal toe: prn=%2d tt=%6.0Lf\n", prn, tt);
    return 0;
  }
...
static int decode_WE(raw_t *raw) {
  ...
  p += 4;
  for (i = 0; i < 3; i++) {
    seph.pos[i] = (long double)R8(p);
    p += 8;
  }
  for (i = 0; i < 3; i++) {
    seph.vel[i] = (long double)R4(p);
    p += 4;
  }
  for (i = 0; i < 3; i++) {
    seph.acc[i] = (long double)R4(p);
    p += 4;
  }
  seph.af0 = (long double)R4(p);
  p += 4;
  seph.af1 = (long double)R4(p);
  p += 4;
  tow = U4(p);
  p += 4;
  week = U2(p);
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " prn=%3d tod=%6u", prn, tod);
  }
...
static int decode_UO(raw_t *raw) {
  ...
  p += 8;
  raw->nav.utc_gps[1] = (long double)R4(p);
  p += 4;
...
static int decode_IO(raw_t *raw) {
  ...
  p += 4 + 2;
  for (i = 0; i < 8; i++) {
    raw->nav.ion_gps[i] = (long double)R4(p);
    p += 4;
  }
  return 9;
}
...
static int decode_L1nav(uint8_t *buff, int len, int sat, raw_t *raw) {
  ...
  for (i = 0, p = buff; i < 10; i++, p += 4) {
    setbitu(subfrm, sizeof(subfrm), 24 * i, 24, U4(p) >> 6);
  }
  id = getbitu(subfrm, sizeof(subfrm), 43, 3);
  if (id < 1 || id > 5) {
...
static int decode_L2nav(uint8_t *buff, int len, int sat, raw_t *raw) {
  ...
  i = 0;
  preamb = getbitu(msg, sizeof(msg), i, 8);
  i += 8;
  prn = getbitu(msg, sizeof(msg), i, 6);
  i += 6;
  msgid = getbitu(msg, sizeof(msg), i, 6);
  i += 6;
  tow = getbitu(msg, sizeof(msg), i, 17);
  i += 17;
  alert = getbitu(msg, sizeof(msg), i, 1);
  i += 1;
...
static int decode_nd(raw_t *raw, int sys) {
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " prn=%3d time=%7d type=%d", prn, time, type);
  }
  if (!(sat = satno(sys, prn))) {
    trace(2, "javad nd satellite error: sys=%d prn=%d\n", sys, prn);
    return 0;
...
static int decode_lD(raw_t *raw) {
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " prn=%2d frq=%2d time=%7d type=%d", prn, frq,
                 time, type);
  }
  ...
  for (i = 0; i < 4; i++) {
    setbitu(raw->subfrm[sat - 1] + (id - 1) * 10, sizeof(raw->subfrm[0]) - (id - 1) * 10, i * 25,
            i < 3 ? 25 : 2, U4(p + 4 * i) >> (i < 3 ? 0 : 23));
  }
  if (id != 4) return 0;
...
static int decode_WD(raw_t *raw) {
  ...
  if (raw->outtype) {
    rtkcatprintf(raw->msgtype, sizeof(raw->msgtype), " prn=%3d tow=%6d", prn, tow);
  }
...
static int decode_rx(raw_t *raw, char sig) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    ...
    /*                             Ksys  Asys */
    if (sys == SYS_SBS)
      prm = (pr * 1E-11L + 0.125L) * CLIGHT; /* [6] */
    else if (sys == SYS_QZS)
      prm = (pr * 2E-11L + 0.125L) * CLIGHT; /* [3] */
    else if (sys == SYS_CMP)
      prm = (pr * 2E-11L + 0.105L) * CLIGHT; /* [4] */
    else if (sys == SYS_GAL)
      prm = (pr * 2E-11L + 0.085L) * CLIGHT; /* [7] */
    else if (sys == SYS_IRN)
      prm = (pr * 2E-11L + 0.105L) * CLIGHT; /* [6] */
    else
      prm = (pr * 1E-11L + 0.075L) * CLIGHT;
...
static int decode_xR(raw_t *raw, char sig) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    ...
    sat = raw->obuf.data[i].sat;
    if (!(sys = satsys(sat, NULL)) || raw->prCA[sat - 1] == 0.0L) continue;
...
static int decode_xr(raw_t *raw, char sig) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    pr = I2(p);
    p += 2;
    if (pr == (int16_t)0x7FFF) continue;

    sat = raw->obuf.data[i].sat;
    if (!(sys = satsys(sat, NULL)) || raw->prCA[sat - 1] == 0.0L) continue;

    prm = (pr * 1E-11L + 2E-7L) * CLIGHT + raw->prCA[sat - 1];
...
static int decode_px(raw_t *raw, char sig) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    ...
    if ((idx = checkpri(sys, code, raw->opt, idx)) >= 0) {
      if (!settag(raw->obuf.data + i, raw->time)) continue;
      raw->obuf.data[i].L[idx] = cp / 1024.0L;
      raw->obuf.data[i].code[idx] = (uint8_t)code;
    }
  }
  return 0;
...
static int decode_xP(raw_t *raw, char sig) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    rcp = (long double)R4(p);
    p += 4;
    if (rcp == 0.0L) continue;
...
static int decode_xd(raw_t *raw, char sig) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    rdp = I2(p);
    p += 2;
    if (rdp == (int16_t)0x7FFF) continue;

    sat = raw->obuf.data[i].sat;
    if (!(sys = satsys(sat, NULL)) || raw->dpCA[sat - 1] == 0.0L) continue;
    ...
    if ((idx = checkpri(sys, code, raw->opt, idx)) >= 0) {
      ...
      dop = (-rdp + raw->dpCA[sat - 1] * 1E4L) * fn / f1 * 1E-4L;
...
static int decode_Ex(raw_t *raw, char sig) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    ...
    if ((idx = checkpri(sys, code, raw->opt, idx)) >= 0) {
      if (!settag(raw->obuf.data + i, raw->time)) continue;
      raw->obuf.data[i].SNR[idx] = (uint16_t)(cnr / SNR_UNIT + 0.5L);
    }
  }
  return 0;
}
...
static int decode_xE(raw_t *raw, char sig) {
  ...
  for (i = 0; i < raw->obuf.n && i < MAXOBS; i++) {
    ...
    if ((idx = checkpri(sys, code, raw->opt, idx)) >= 0) {
      if (!settag(raw->obuf.data + i, raw->time)) continue;
      raw->obuf.data[i].SNR[idx] = (uint16_t)(cnr * 0.25L / SNR_UNIT + 0.5L);
    }
  }
  return 0;
}
...
static int decode_javad(raw_t *raw) {
  ...
  if (raw->outtype) {
    rtksnprintf(raw->msgtype, sizeof(raw->msgtype), "JAVAD %2.2s (%4d)", p, raw->len);
  }
...
static int sync_javad(uint8_t *buff, uint8_t data) {
  ...
  return (p == '\r' || p == '\n') && ISTXT(buff[0]) && ISTXT(buff[1]) && ISHEX(buff[2]) &&
         ISHEX(buff[3]) && ISHEX(buff[4]);
}
/* Clear buffer --------------------------------------------------------------*/
