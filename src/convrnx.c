                     ...
                     int *mask)
{
        ...
        while ((type=input_strfile(str))>=-1) {
            ...
            if (opt->ltspan && type==1 &&
              ...
              for (int i = 0; i < str->obs->n; i++) {
                int sys = satsyst(str->obs->data[i].sat, str->obs->data[i].time, NULL);
            ...
            if (type==1) { /* observation data */
                for (i=0;i<str->obs->n;i++) {
                    sys = satsyst(str->obs->data[i].sat, str->obs->data[i].time, NULL);
                    if ((sys & opt->navsys) == 0) continue;
    ...
    for (i=0;i<str->nav->ng;i++) {
        if (satsyst(str->nav->geph[i].sat, str->nav->geph[i].tof, &prn) != SYS_GLO) continue;
