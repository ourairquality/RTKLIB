...
static void rtcm_set38bits(rtcm_t *rtcm, int pos, double value) {
...
static bool encode_type1019(rtcm_t *rtcm, int sync) {
  ...
  i += 1;
  rtcm_setbitu(rtcm, i, 1, eph->fit > 0.0 ? 0 : 1);
  i += 1;
  rtcm->nbit = i;
  return true;
}
...
static bool encode_type1044(rtcm_t *rtcm, int sync) {
  ...
  rtcm_setbitu(rtcm, i, 10, eph->iodc);
  i += 10;
  rtcm_setbitu(rtcm, i, 1, eph->fit == 2.0 ? 0 : 1);
  i += 1;
  rtcm->nbit = i;
  return true;
}
