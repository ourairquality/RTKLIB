...
static void gen_obs_gps(rtcm_t *rtcm, const obsd_t *data, int *gcode1, int *pr1, int *ppr1,
  ...
  *pr1 = *amb = 0;
  ...
  int lt1 = (int)locktime(rtcm, data, code0, data->LLI[0]);
  int lt2 = (int)locktime(rtcm, data, code1, data->LLI[1]);
                           ...
                           int provid, int solid)
{
    ...
    if (subtype==0) { /* RTCM SSR */
        ...
        switch (sys) {
          ...
          case SYS_BDS: msgno=(type==7)?1269:1257+type; break; /* draft */
...
static int encode_ssr3(rtcm_t *rtcm, int sys, int subtype, int sync)
{
    ...
    if (subtype>0) { /* IGS SSR */
      switch (sys) {
        ...
        case SYS_BDS: np=6; offp=  0; codes=codes_igs_bds; break;
      ...
      switch (sys) {
        ...
        case SYS_BDS: np=6; offp=  0; codes=codes_bds; break;
...
static void gen_msm_sat(rtcm_t *rtcm, int sys, int rtype, int nsat, const uint8_t *sat_ind,
  ...
  for (int i = 0; i < rtcm->obs.n; i++) {
    ...
    for (int j = 0; j < NFREQ + NEXOBS; j++) {
      ...
      if (!sig) continue;
      // Use satsyst() here to distinguish between BDS2 and BDS3.
      double freq = code2freq(satsyst(data->sat, data->time, NULL), data->code[j], fcn - 7);
...
static void gen_msm_sig(rtcm_t *rtcm, int sys, int nsat, int nsig, int ncell,
  ...
  for (int i = 0; i < rtcm->obs.n; i++) {
    ...
    for (int j = 0; j < NFREQ + NEXOBS; j++) {
      ...
      // Use satsyst() here to distinguish between BDS2 and BDS3.
      double freq = code2freq(satsyst(data->sat, data->time, NULL), code, fcn - 7);
...
int encode_rtcm3(rtcm_t *rtcm, int type, int subtype, int sync)
{
    ...
    switch (type) {
        ...
        case 1270: ret=encode_ssr7(rtcm,SYS_BDS,0,sync); break; /* draft */
