...
static int decode_ssr3(rtcm_t *rtcm, int sys, int subtype)
{
    ...
    switch (sys) {
        ...
        case SYS_BDS: np=6; offp=  1; sigs=ssr_sig_cmp; break;
                         ...
                         const int *ex, const int *half)
{
    ...
    for (i=j=0;i<h->nsat;i++) {
        ...
        int ssys = satsys(sat, NULL); // For BDS2 vs BDS3.
        fcn=0;
        if (ssys==SYS_GLO) {
        ...
        for (k=0;k<h->nsig;k++) {
            ...
            if (sat && index >= 0 && freqidx >= 0) {
                ...
                rtcm->obs.data[index].SNR [freqidx]=cnr[j];
            }
            j++;
        }
    }
...
extern int decode_rtcm3(rtcm_t *rtcm)
{
    ...
    switch (type) {
        ...
        case   14: ret=decode_ssr7(rtcm,SYS_BDS,0); break; /* tentative */
