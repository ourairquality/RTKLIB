...
static void outsolstat(rtk_t *rtk,const nav_t *nav)
{
    ...
    for (i=0;i<MAXSAT;i++) {
        ...
        for (j=0;j<nfreq;j++) {
            k=IB(i+1,j,&rtk->opt);
            fprintf(fp_stat,"$SAT,%d,%.3f,%s,%d,%.1f,%.1f,%.4f,%.4f,%d,%.2f,%d,%d,%d,%d,%d,%d,%.2f,%.6f,%.5f\n",
    ...
    for (int m=0;m<6;m++) {
        ...
        for (int f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
            ...
            for (int j=0;j<ns;j++) {
                ...
                double threshadj=(cmatread(P,rtk->nx,Pc,nc,xi,ii,ii)==SQR(rtk->opt.std[0]))||
                    (cmatread(P,rtk->nx,Pc,nc,xi,jj,jj)==SQR(rtk->opt.std[0]))?10:1;
