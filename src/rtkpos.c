...
// Averge of two angles in degrees.
static double degavg(double a1, double a2)
{
  double d = a1 - a2;
  if (d <= -180) d += 360;
  else if (d > 180) d -= 360;
        ...
        if (est&&rtk->opt.ionoopt==IONOOPT_EST) {
            for (int i=0;i<MAXSAT;i++) {
                ...
                if (rtk->opt.mode >= PMODE_DGPS) {
                  ...
                  az = degavg(az, ssat->azel[1][0] * R2D);
...
#define VAR_SQR_SINEL
    ...
    double e=opt->err[7];
    if (e>0.0) {
        if (code) var+=SQR(e)*SQR(obs->Pstd[frq]);
        else var+=SQR(e)*SQR(obs->Lstd[frq]*0.2);
    }
...
/* Undifferenced phase/code residuals ----------------------------------------
 ...
 Args:  I   base:  1=base,0=rover
        ...
        I   nav  = sat nav data
        I   rr   = rcvr pos (x,y,z), marker position
        I   opt  = options
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (opt->ionoopt == IONOOPT_IFLC) { /* Iono-free linear combination */
      ...
      for (int f = 0; f < nf; f++) {
        ...
        ionocorr(obs[i].time, nav, sat, rpc_pos, azel + i * 2, opt->ionoopt, &dion, &vion);
...
static inline double cmatread(const double *A, int n, const double *Ac, int nc, const int *xi, int i, int j)
{
    int ii=xi[i],jj=xi[j];
    if (ii>=nc||jj>nc)
        return A[i+j*n];
...
static int constbl(rtk_t *rtk, const double *x, const double *P,
                   const double *Pc, int nc, const int *xi, double *v,
                   double *Hc, double *Ri, double *Rj, int index)
{
    const double thres=0.1; /* Threshold for nonlinearity (v.2.3.0) */
    double xb[3],b[3],var=0.0;
...
static int ddres(rtk_t *rtk, const obsd_t *obs, double dt, const double *x,
                 const double *P, const double *Pc, int nc, const int *xi,
                 const int *sat, const double *y, const double *e,
                 const double *azel, const double *freq, const int *iu, const int *ir,
                 int ns, double *v, double *Hc, double *R, int *vflg)
{
    prcopt_t *opt=&rtk->opt;
    double dr[3],posu[3],posr[3],didxi=0.0,didxj=0.0;
    int nv=0,nb[NFREQ*NSYS*2+2]={0},b=0,sysi,sysj,nf=NF(opt);
    ...
    double *Ri=mat(ns*nf*2+2,1),*Rj=mat(ns*nf*2+2,1),*im=mat(ns,1);
    ...
    for (int i=0;i<ns;i++) {
        if (opt->ionoopt==IONOOPT_EST) {
            im[i]=(ionmapf(posu,azel+iu[i]*2,ME_WGS84/1000,HION,1)+
                   ionmapf(posr,azel+ir[i]*2,ME_WGS84/1000,HION,1))/2.0;
        }
    ...
    for (int m=0;m<7;m++) {
        ...
        for (int f=opt->mode>PMODE_DGPS?0:nf;f<nf*2;f++) {
            ...
            for (int j=0;j<ns;j++) {
                ...
                double refvar=varerr(sat[j],sysj,azel[1+iu[j]*2],azel[1+ir[j]*2],
            ...
            for (int j=0;j<ns;j++) {
                ...
                if (Hc) {
                    ...
                    for (int k=0;k<3;k++) {
                        cvwrite(Hi,nc,xi,k,-e[k+frq*3+iu[i]*nf*3]+e[k+frq*3+iu[j]*nf*3]);
                    }
                }
                if (opt->ionoopt==IONOOPT_EST) {
                ...
                if (opt->mode > PMODE_DGPS) {
                  ...
                  int ii = IB(sat[i], frq, opt);
                  int jj = IB(sat[j], frq, opt);
                  if (cmatread(P, rtk->nx, Pc, nc, xi, ii, ii) == SQR(rtk->opt.std[0]) ||
                ...
                if (opt->mode>PMODE_DGPS) {
                    ...
                    xjj = x[jj];
                    Pjj = cmatread(P,rtk->nx,Pc,nc,xi,jj,jj);
                }
  ...
  for (int m = 0; m < 7; m++) {
    ...
    for (int f = 0; f < nf; f++) {
      ...
      for (int sat2 = sat1 + 1; sat2 < MAXSAT; sat2++) {
        if (rtk->x[IB(sat2 + 1, f, &rtk->opt)] == 0.0 ||
...
static void holdamb(rtk_t *rtk, const double *xa)
{
    ...
    for (int m=0;m<7;m++) for (int f=0;f<nf;f++) {
        ...
        for (int i = 0; i < n; i++) rtk->ssat[sat[i]].fix[f] = 3; // Hold.
                  ...
                  const nav_t *nav)
{
    ...
    double *var=mat(1,n), *y=mat(nf*2,n), *e=mat(3*nf,n);
     ...
     * be passed as a vector of the 3 axis. Also always include the other
     * parameters that are enabled as they may be set even if zero now. */
    for (int i = 0; i < NR(opt); i++) {
        xi[i] = nc;
        ix[nc++] = i;
    }
    // Flag the phase bias indices used.
    int *ibf = imat(nx, 1);
    for (int i = 0; i < nx; i++) ibf[i] = 0;
    if (rtk->opt.mode > PMODE_DGPS) {
      // All observations
      for (int i = 0; i < nu + nr; i++) {
        for (int frq = 0; frq < nf; frq++) {
          if (obs[i].code[frq] != CODE_NONE) {
            int j = IB(obs[i].sat, frq, opt);
            if (j > 0) ibf[j] = 1;
          }
        }
      }
    }
    for (int i = NR(opt); i < nx; i++) {
      if (ibf[i] || (x[i] != 0.0 && P[i + i * nx] > 0.0)) {
        xi[i] = nc;
        ix[nc++] = i;
      } else {
    ...
    for (int i=0;i<opt->niter;i++) {
        ...
        if ((nv=ddres(rtk,obs,dt,x,P,Pc,nc,xi,sat,y,e,azel,freq,iu,ir,ns,v,Hc,R,vflg))<4) {
        ...
        /* Kalman filter measurement update, updates x,y,z,sat phase biases, etc
                ...
                xp=x+K*v
                Pp=(I-K*H')*P                  */
        trace(3,"before filter x=");tracemat(3,x,1,NP(opt),13,6);
        /*  Do kalman filter state update on compressed arrays */
        int info=filter_(xc,Pc,Hc,v,R,nc,nv,Ppc);
        if (info) {
            errmsg(rtk,"filter error (info=%d)\n",info);
        ...
        double *Pt=Pc; Pc=Ppc; Ppc=Pt; /* Swap Pc and Ppc */
        trace(3,"after filter x=");tracemat(3,x,1,NP(opt),13,6);
        trace(4,"x(%d)=",i+1); tracemat(4,x,1,NR(opt),13,4);
    }
    free(xc); free(Ppc); free(Hc);
    ...
    if (stat!=SOLQ_NONE&&zdres(0,obs,nu,rs,dts,var,svh,nav,x,opt,y,e,azel,freq)) {
        ...
        int vflg[MAXOBS*NFREQ*2+1];
        nv=ddres(rtk,obs,dt,x,P,Pc,nc,xi,sat,y,e,azel,freq,iu,ir,ns,v,NULL,R,vflg);
        ...
        if (valpos(rtk,v,R,vflg,nv,4.0)) {
            ...
            for (int j=0;j<nc;j++) for (int i=0;i<nc;i++) P[ix[i]+ix[j]*nx]=Pc[i+j*nc];
    ...
    if (stat==SOLQ_FLOAT) {
        ...
        if (manage_amb_LAMBDA(rtk,bias,xa,sat,nf,ns)>1) {
            ...
            if (zdres(0,obs,nu,rs,dts,var,svh,nav,xa,opt,y,e,azel,freq)) {
                ...
                int vflg[MAXOBS*NFREQ*2+1];
                nv=ddres(rtk,obs,dt,xa,P,Pc,nc,xi,sat,y,e,azel,freq,iu,ir,ns,v,NULL,R,vflg);
